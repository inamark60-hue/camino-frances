...
/* ---------- V8.6 · Motor de cielo profesional Canvas + catálogo abierto ---------- */
...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v86')}catch(_){ }
...
function skyParseStars(j){return (j?.features||[]).map((f,i)=>{const eq=skyCoordToEq(f.geometry?.coordinates),p=f.properties||{};if(!eq)return null;const mag=Number(p.mag);return{id:String(f.id??p.id??`s${i}`),name:String(p.name||''),display:String(p.name||p.desig||f.id||''),ra:eq.ra,dec:eq.dec,mag:Number.isFinite(mag)?mag:6.5,bv:Number.isFinite(Number(p.bv))?Number(p.bv):null,con:''}}).filter(Boolean)}
...
function skyCanvasGeom(canvas){const r=canvas.getBoundingClientRect(),w=Math.max(300,Math.round(r.width||360)),h=Math.max(300,Math.round(r.height||520)),dpr=Math.min(2,window.devicePixelRatio||1);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr)}const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);return{ctx,w,h,cx:w/2,cy:h/2,m:Math.min(w,h),f:Math.min(w,h)*.58}}
function skyProjectVector(v,basis,g){if(!basis)return null;const x=dot3(v,basis.right),y=dot3(v,basis.up),z=dot3(v,basis.forward);if(z<=.16)return null;const p={x:g.cx+g.f*x/z,y:g.cy-g.f*y/z,z};if(p.x<-40||p.x>g.w+40||p.y<-40||p.y>g.h+40)return null;return p}
function skyProjectStatic(o,g){if(o.alt<=0)return null;const R=g.m*.46,rr=(90-o.alt)/90*R,a=o.az*Math.PI/180;return{x:g.cx+rr*Math.sin(a),y:g.cy-rr*Math.cos(a),z:1}}
...
function skyCanvasHit(canvas,source){
 if(canvas.dataset.skyBound==='1')return;canvas.dataset.skyBound='1';canvas.addEventListener('pointerup',e=>{const r=canvas.getBoundingClientRect(),x=e.clientX-r.left,y=e.clientY-r.top,hits=skyHitTargets[source]||[];let best=null,dist=Infinity;for(const h of hits){let d=Infinity;if(h.kind==='constellation'){for(const s of h.segments||[])for(let i=1;i<s.length;i++)d=Math.min(d,skyLineDistance(x,y,s[i-1],s[i]));if(h.label)d=Math.min(d,Math.hypot(x-h.label.x,y-h.label.y))}else d=Math.hypot(x-h.x,y-h.y);const lim=h.kind==='constellation'?20:(h.kind==='star'?18:22);if(d<=lim&&d<dist){dist=d;best=h}}if(best)openSkyObjectInfo(best.kind,best.key,source)});
}
...
function skyRenderCanvas(canvas,lat,lon,now,sensor,source='normal'){
 if(!canvas)return{visible:[],active:null,pointing:null};const g=skyCanvasGeom(canvas),ctx=g.ctx,scene=skyBuildScene(lat,lon,now),basis=sensor?deviceSkyBasis(skyOrientationState.alpha,skyOrientationState.beta,skyOrientationState.gamma):null,pointing=basis?skyPointingDirection(basis):null,project=o=>basis?skyProjectVector(o.v,basis,g):skyProjectStatic(o,g);ctx.clearRect(0,0,g.w,g.h);const grad=ctx.createRadialGradient(g.cx,g.cy,0,g.cx,g.cy,Math.max(g.w,g.h)*.75);grad.addColorStop(0,'#102b4c');grad.addColorStop(1,'#030d1b');ctx.fillStyle=grad;ctx.fillRect(0,0,g.w,g.h);
 if(!sensor){ctx.save();ctx.strokeStyle='rgba(110,145,180,.32)';for(const alt of [0,30,60]){const R=g.m*.46*(90-alt)/90;ctx.beginPath();ctx.arc(g.cx,g.cy,R,0,Math.PI*2);ctx.stroke()}ctx.fillStyle='#ffc400';ctx.font='800 12px system-ui';ctx.textAlign='center';ctx.fillText('N',g.cx,g.cy-g.m*.46+16);ctx.fillText('S',g.cx,g.cy+g.m*.46-8);ctx.fillText('E',g.cx+g.m*.46-10,g.cy);ctx.fillText('O',g.cx-g.m*.46+10,g.cy);ctx.restore()}
 ...
 const active=skyPickAutoConstellation(projectedConst,g);skyAutoConstellation=active?.name||null;
 // Líneas de las 88 constelaciones. La más cercana al centro resalta automáticamente.
 for(const c of projectedConst){const hot=active&&c.abbr===active.abbr;ctx.save();ctx.strokeStyle=hot?'#ffd400':'rgba(103,150,198,.65)';ctx.lineWidth=hot?2.8:1.05;ctx.shadowColor=hot?'#ffd400':'transparent';ctx.shadowBlur=hot?8:0;for(const s of c.screenSegments){ctx.beginPath();ctx.moveTo(s[0].x,s[0].y);for(let i=1;i<s.length;i++)ctx.lineTo(s[i].x,s[i].y);ctx.stroke();if(hot){ctx.fillStyle='#ffd400';for(const p of s){ctx.beginPath();ctx.arc(p.x,p.y,3.2,0,Math.PI*2);ctx.fill()}}}ctx.restore()}
 // Miles de estrellas del catálogo hasta magnitud 6.
 const visible=[];const starHits=[];for(const st of scene.stars){const p=project(st);if(!p)continue;visible.push(st);const r=skyStarRadius(st.mag);ctx.beginPath();ctx.fillStyle=skyStarColor(st.bv);ctx.globalAlpha=Math.max(.45,Math.min(1,1.08-(st.mag/9)));ctx.arc(p.x,p.y,r,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;if(st.name&&st.mag<=2.1){ctx.fillStyle='rgba(230,241,255,.9)';ctx.font='10px system-ui';ctx.fillText(st.name,p.x+6,p.y-5);starHits.push({kind:'star',key:st.id,x:p.x,y:p.y})}}
 // Galaxias, nebulosas y cúmulos Messier.
 const dsoHits=[];for(const d of scene.dsos){const p=project(d);if(!p)continue;const s=skyDsoStyle(d.type),size=d.mag<=4?6:4.5;ctx.save();ctx.strokeStyle=s.color;ctx.fillStyle=s.color;ctx.globalAlpha=.9;ctx.lineWidth=1.4;if(s.shape==='ellipse'){ctx.beginPath();ctx.ellipse(p.x,p.y,size*1.5,size*.8,0,0,Math.PI*2);ctx.stroke()}else if(s.shape==='diamond'){ctx.beginPath();ctx.moveTo(p.x,p.y-size);ctx.lineTo(p.x+size,p.y);ctx.lineTo(p.x,p.y+size);ctx.lineTo(p.x-size,p.y);ctx.closePath();ctx.stroke()}else{ctx.beginPath();ctx.arc(p.x,p.y,size,0,Math.PI*2);ctx.stroke()}if(d.mag<=5.5||/^M\s*\d+/i.test(d.name)){ctx.font='800 9px system-ui';ctx.fillText(d.name,p.x+7,p.y+3)}ctx.restore();dsoHits.push({kind:'dso',key:d.id,x:p.x,y:p.y})}
 // Nombre de la constelación a la que apunta el retículo.
 let conHit=[];for(const c of projectedConst){let sx=0,sy=0,n=0;for(const s of c.screenSegments)for(const p of s){sx+=p.x;sy+=p.y;n++}const label=n?{x:sx/n,y:sy/n}:null;if(c===active&&label){const name=c.name;const x=Math.max(12,Math.min(g.w-12,label.x)),y=Math.max(22,Math.min(g.h-18,label.y));ctx.save();ctx.font='900 16px system-ui';ctx.textAlign='center';const tw=ctx.measureText(name).width;ctx.fillStyle='rgba(2,13,27,.78)';ctx.fillRect(x-tw/2-8,y-16,tw+16,24);ctx.fillStyle='#ffd400';ctx.shadowColor='#ffd400';ctx.shadowBlur=7;ctx.fillText(name,x,y);ctx.restore();label.x=x;label.y=y}conHit.push({kind:'constellation',key:c.abbr,segments:c.screenSegments,label})}
 if(sensor)skyDrawPolarGrid(ctx,g,pointing);skyHitTargets[source]=[...starHits,...dsoHits,...conHit];skyCanvasHit(canvas,source);return{visible,active,pointing};
}
...
function skyInfoIntro(source='normal'){const box=skyInfoPanel(source);if(!box)return;box.hidden=false;box.innerHTML=`<div class="sky-object-head"><div><small>INFO DEL CIELO</small><h3>ℹ️ Toca estrellas, constelaciones o galaxias</h3></div><button type="button" class="sky-info-close" aria-label="Cerrar información">×</button></div><p>El visor usa ${esc(skyCatalogSummary())}. Al acercar el centro amarillo a una constelación, sus líneas y estrellas principales se iluminan en amarillo y aparece su nombre. Toca el nombre, una línea, una estrella brillante o un objeto de cielo profundo para abrir aquí su ficha.</p><div class="constellation-facts"><span>⭐ Estrellas hasta magnitud 6</span><span>✨ 88 constelaciones IAU</span><span>🌌 Galaxias/nebulosas/cúmulos Messier</span><span>🧭 Brújula celeste polar AZ/ALT</span></div><small class="license-note">Catálogo astronómico: Celestial Data / d3-celestial, datos J2000 reutilizables con atribución.</small>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true}
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>22. ✨ Estrellas y constelaciones</strong><p>Primero pulsa <b>USAR MI GPS</b> y después <b>ACTIVAR MODO CIELO</b>. Apunta la parte trasera del móvil hacia el cielo: el centro amarillo representa exactamente la dirección observada y arriba se muestran <b>APUNTANDO</b> (azimut) y <b>ALT</b> (altura sobre el horizonte). Con conexión se carga un catálogo abierto con <b>miles de estrellas hasta magnitud 6, las 88 constelaciones IAU y objetos Messier</b> como galaxias, nebulosas y cúmulos; después queda guardado como respaldo. Al acercar el centro amarillo a una constelación, sus líneas y estrellas de figura <b>se iluminan en amarillo</b> y aparece su nombre: tócalo para abrir la ficha dentro de <b>ℹ️ INFO</b>. También puedes tocar estrellas brillantes y objetos de cielo profundo. La brújula del cielo es <b>polar/altazimutal</b>, muestra AZ y ALT y puede moverse con el dedo. Pulsa <b>🌌 GRAN VISTA CIELO</b> para pantalla completa en vertical u horizontal. Las fotos astronómicas se buscan en Wikimedia Commons y solo se muestran cuando consta licencia libre o dominio público, con autor y licencia.</p></div>
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v86','1')}catch{}
 const u=new URL('./',location.href);u.searchParams.set('bcv','85');u.searchParams.set('repair',Date.now().toString());location.replace(u.href);
}
