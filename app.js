...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>b.onclick=e=>{e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  // V10.0: captura táctil global. Los gestos se detectan a nivel de documento y
  // solo se activan si el primer dedo nace dentro del rectángulo del mapa.
  // Así ninguna capa SVG/marker/control del propio mapa puede bloquear Samsung/Chrome.
  ...
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=36?best:-1;};
  const isControlAt=(x,y)=>{const el=document.elementFromPoint(x,y);return !!el?.closest?.('.bc-map-controls,.bc-map-view-controls,.bc-center-route,.bc-layer,.bc-service-filters,.bc-map-legend,.map-compass-overlay,.route-popup');};
  const inside=(x,y)=>{const r=this.el.getBoundingClientRect();return x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom;};
  let gt=null;
  const tpts=touches=>Array.from(touches).map(t=>({x:t.clientX,y:t.clientY,id:t.identifier}));
  const beginPan=p=>{gt={active:true,mode:'pan',start:{...p},last:{...p},cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now()};};
  const beginPinch=(a,b)=>{gt={active:true,mode:'pinch',lastMid:mid(a,b),lastDist:Math.max(1,dist(a,b)),moved:true,t:performance.now()};resetTransforms();};
  const onDocTouchStart=e=>{const ps=tpts(e.touches);if(!ps.length)return;if(!gt?.active){const p=ps[0];if(!inside(p.x,p.y)||isControlAt(p.x,p.y))return;this.userInteracted=true;this.closePopup();if(ps.length===1)beginPan(p);else beginPinch(ps[0],ps[1]);}else if(ps.length>=2)beginPinch(ps[0],ps[1]);if(gt?.active){e.preventDefault();e.stopPropagation();}};
  const onDocTouchMove=e=>{if(!gt?.active)return;const ps=tpts(e.touches);if(!ps.length)return;e.preventDefault();e.stopPropagation();if(ps.length===1){const p=ps[0];if(gt.mode!=='pan')beginPan(p);const dx=p.x-gt.start.x,dy=p.y-gt.start.y;gt.dx=dx;gt.dy=dy;gt.last={...p};if(Math.abs(dx)+Math.abs(dy)>3)gt.moved=true;translatePreview(dx,dy);return;}const a=ps[0],b=ps[1],m=mid(a,b),d=Math.max(1,dist(a,b));if(gt.mode!=='pinch'){beginPinch(a,b);return;}const mdx=m.x-gt.lastMid.x,mdy=m.y-gt.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.1){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/gt.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;gt.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.92&&this.zoom>5){this.zoom--;gt.lastDist=d;this.clearTileNodes();this.render();}gt.lastMid=m;gt.moved=true;};
  const onDocTouchEnd=e=>{if(!gt?.active)return;e.preventDefault();e.stopPropagation();const ps=tpts(e.touches);if(ps.length>=2){beginPinch(ps[0],ps[1]);return;}if(ps.length===1){resetTransforms();beginPan(ps[0]);gt.moved=true;return;}const ch=e.changedTouches?.[0],end=ch?{x:ch.clientX,y:ch.clientY}:gt.last,was=gt;if(was.mode==='pan'){if(was.moved)this.center=world2ll(was.cp.x-was.dx,was.cp.y-was.dy,this.zoom);else if(end&&performance.now()-was.t<750){const i=nearestYellow(end.x,end.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);gt=null;resetTransforms();return;}}}gt=null;resetTransforms();this.render();};
  const onDocTouchCancel=e=>{if(!gt?.active)return;gt=null;resetTransforms();this.render();};
  ...
  this._globalTouchCleanup=()=>{document.removeEventListener('touchstart',onDocTouchStart,true);document.removeEventListener('touchmove',onDocTouchMove,true);document.removeEventListener('touchend',onDocTouchEnd,true);document.removeEventListener('touchcancel',onDocTouchCancel,true);};
  // Ratón/lápiz conserva Pointer Events locales.
  let mp=null;this.el.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||(e.pointerType==='mouse'&&e.button!==0)||e.target.closest('button,a,.route-popup'))return;e.preventDefault();const cp=ll2world(this.center.lat,this.center.lon,this.zoom);mp={id:e.pointerId,x:e.clientX,y:e.clientY,cp,dx:0,dy:0};try{this.el.setPointerCapture(e.pointerId)}catch{}});this.el.addEventListener('pointermove',e=>{if(!mp||e.pointerId!==mp.id)return;e.preventDefault();mp.dx=e.clientX-mp.x;mp.dy=e.clientY-mp.y;translatePreview(mp.dx,mp.dy)});const mpEnd=e=>{if(!mp||e.pointerId!==mp.id)return;this.center=world2ll(mp.cp.x-mp.dx,mp.cp.y-mp.dy,this.zoom);mp=null;resetTransforms();this.render();};this.el.addEventListener('pointerup',mpEnd);this.el.addEventListener('pointercancel',mpEnd);
  this.el.addEventListener('dblclick',e=>{if(e.target.closest('button,a,.route-popup'))return;e.preventDefault();this.setZoom(this.zoom+1)});
