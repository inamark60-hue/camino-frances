...
function renderSky(lat,lon,label){
 ...
 if(info)info.innerHTML=`<b>${esc(label)}</b><span>${lat.toFixed(4)}, ${lon.toFixed(4)} · ${esc(now.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}))}</span><small>${sensor?`${esc(aim)} · centro amarillo = dirección de la parte trasera del móvil`:'Centro = cenit · borde = horizonte · toca ACTIVAR MODO CIELO para seguir el movimiento'}</small>`;
...
async function startSkyTracking(){
 const status=document.getElementById('skyStatus'),btn=document.getElementById('skyTrackBtn');try{await requestCompassPermission();skyTrackActive=true;if(btn){btn.textContent='✓ MODO CIELO ACTIVO';btn.classList.add('active')}
  ...
  if(skyCompassCtrl&&!skyCompassCtrl.active)activateSkyPolarCompass(skyCompassCtrl);if(status){status.className='feature-status ok';status.innerHTML='<b>✓ Modo cielo activo</b><span>Apunta la parte trasera del teléfono hacia el cielo y muévelo despacio. El centro amarillo representa la dirección real a la que apunta el móvil; azimut e inclinación se calculan con los sensores.</span>'}
...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v87')}catch(_){ }
...
function skyBuildScene(lat,lon,now){
 ...
 const stars=SKY_RICH.stars.map(conv),dsos=SKY_RICH.dsos.map(conv),constellations=SKY_RICH.constellations.map(c=>({...c,segments:c.segments.map(seg=>seg.map(conv))})),centers=SKY_RICH.centers.map(conv);skySceneCache={key,stars,dsos,constellations,centers};return skySceneCache;
}
...
function skyUpdateControls(){document.querySelectorAll('[data-sky-zoom-readout]').forEach(el=>el.textContent=`${skyZoomLabel()} · ~${skyApproxFov()}°`);document.querySelectorAll('[data-sky-detail]').forEach(el=>{el.textContent=`⭐ ${skyDetailMode==='simple'?'SIMPLE':skyDetailMode==='complete'?'COMPLETO':'NORMAL'}`;el.dataset.mode=skyDetailMode})}
...
function skyDrawPolarGrid(ctx,g,pointing){
 ctx.save();ctx.strokeStyle='rgba(137,167,196,.24)';ctx.lineWidth=1;for(const r of [g.m*.12,g.m*.24,g.m*.36]){ctx.beginPath();ctx.arc(g.cx,g.cy,r,0,Math.PI*2);ctx.stroke()}ctx.beginPath();ctx.moveTo(g.cx-22,g.cy);ctx.lineTo(g.cx+22,g.cy);ctx.moveTo(g.cx,g.cy-22);ctx.lineTo(g.cx,g.cy+22);ctx.strokeStyle='#ffc400';ctx.lineWidth=1.5;ctx.stroke();ctx.beginPath();ctx.arc(g.cx,g.cy,10,0,Math.PI*2);ctx.stroke();if(pointing){ctx.font='700 11px system-ui';ctx.fillStyle='rgba(255,196,0,.92)';ctx.textAlign='center';ctx.fillText(`${Math.round(pointing.az)}° ${pointing.card} · ALT ${pointing.alt>=0?'+':''}${Math.round(pointing.alt)}°`,g.cx,g.cy+38)}ctx.restore();
}
...
function skyRenderCanvas(canvas,lat,lon,now,sensor,source='normal'){
 ...
 const active=skyPickAutoConstellation(projectedConst,g);skyAutoConstellation=active?.name||null;const nowMs=performance.now();if(active?.abbr!==skyDwell.key){skyDwell={key:active?.abbr||null,since:nowMs}}const dwellReady=!!active&&nowMs-skyDwell.since>=900;
 ...
 // Objetos de cielo profundo: símbolos sí, nombres solo cerca del retículo para evitar nubes de Mxx.
 ...
 // Nombre de la constelación seleccionada, separado del retículo central.
 ...
 if(sensor)skyDrawPolarGrid(ctx,g,pointing);ctx.save();ctx.fillStyle='rgba(255,255,255,.68)';ctx.font='700 9px system-ui';ctx.textAlign='right';ctx.fillText(`${skyZoomLabel()} · campo ~${skyApproxFov()}° · ${skyDetailMode==='simple'?'simple':skyDetailMode==='complete'?'completo':'normal'}`,g.w-8,g.h-8);ctx.restore();skyHitTargets[source]=[...starHits,...dsoHits,...conHit];skyCanvasHit(canvas,source);return{visible,active,pointing};
}
function skyCatalogSummary(){return SKY_RICH.deep?`${SKY_RICH.stars.length.toLocaleString('es-ES')} estrellas · ${SKY_RICH.constellations.length} constelaciones · ${SKY_RICH.dsos.length} objetos Messier`:'catálogo básico sin conexión'}
function renderSky(lat,lon,label){
 ...
 if(info)info.innerHTML=`<b>${esc(label)}</b><span>${lat.toFixed(4)}, ${lon.toFixed(4)} · ${now.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'})}</span><small>${sensor?`MODO CIELO ACTIVO · ${aim}`:'Vista polar del cielo sobre el horizonte'} · ${skyCatalogSummary()}</small>`;const fs=document.getElementById('skyFullStatus');if(fs)fs.textContent=`${label} · ${sensor?aim:'Vista cielo'} · ${now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`;if(visEl){const con=result.active;visEl.innerHTML=con?`<b>✨ Apuntas hacia <button type="button" data-info-con="${esc(con.abbr)}">${esc(con.name)}</button></b><small>Tócala para abrir información, fotos y datos.</small>`:`<b>Mueve el teléfono: al acercar el centro amarillo a una constelación se iluminará en amarillo.</b><small>${skyCatalogSummary()}</small>`;visEl.querySelector('[data-info-con]')?.addEventListener('click',e=>openSkyObjectInfo('constellation',e.currentTarget.dataset.infoCon,'normal'))}
...
function skyInfoIntro(source='normal'){const box=skyInfoPanel(source);if(!box)return;box.hidden=false;box.innerHTML=`<div class="sky-object-head"><div><small>INFO DEL CIELO</small><h3>ℹ️ Toca estrellas, constelaciones o galaxias</h3></div><button type="button" class="sky-info-close" aria-label="Cerrar información">×</button></div><p>El visor usa ${esc(skyCatalogSummary())}. Al acercar el centro amarillo a una constelación, sus líneas y estrellas principales se iluminan en amarillo y aparece su nombre. Toca el nombre, una línea, una estrella brillante o un objeto de cielo profundo para abrir aquí su ficha.</p><div class="constellation-facts"><span>⭐ Estrellas hasta magnitud 6</span><span>✨ 88 constelaciones IAU</span><span>🌌 Galaxias/nebulosas/cúmulos Messier</span><span>🧭 Brújula celeste polar AZ/ALT</span><span>🔭 Zoom ×1 · ×2 · ×4 · ×8</span><span>⭐ Detalle Simple · Normal · Completo</span></div><small class="license-note">Catálogo astronómico: Celestial Data / d3-celestial, datos J2000 reutilizables con atribución. El zoom amplía la carta celeste; no sustituye a un telescopio óptico ni hace visibles objetos que el ojo no puede observar.</small>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true}
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>22. ✨ Estrellas y constelaciones</strong><p>Primero pulsa <b>USAR MI GPS</b> y después <b>ACTIVAR MODO CIELO</b>. Apunta la parte trasera del móvil hacia el cielo: el centro amarillo representa la dirección observada y arriba se muestran <b>APUNTANDO</b> (azimut) y <b>ALT</b> (altura sobre el horizonte). Con conexión se carga un catálogo abierto con <b>miles de estrellas hasta magnitud 6, las 88 constelaciones IAU y objetos Messier</b>. Las constelaciones no seleccionadas se dibujan tenues; cuando el retículo se acerca a una, sus estrellas y líneas brillan en amarillo, aparece su nombre y, tras mantener la puntería un instante, se muestra <b>Toca para información</b>. La ficha interna incluye datos, estrellas principales cuando constan en el catálogo, cómo localizarla desde tu posición, contexto astronómico y fotografías libres de Wikimedia Commons. Usa <b>⭐ DETALLE</b> para elegir Simple, Normal o Completo y reducir o aumentar objetos y etiquetas. En <b>🌌 GRAN VISTA CIELO</b> dispones de zoom tipo telescopio <b>×1 · ×2 · ×4 · ×8</b> con botones ＋/− o pellizcando con dos dedos; doble toque vuelve a ×1. El zoom es una ampliación digital de la carta y no sustituye a un telescopio óptico. La brújula celeste polar/altazimutal es pequeña, transparente, movible y muestra AZ/ALT.</p></div>
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v87','1')}catch{}
