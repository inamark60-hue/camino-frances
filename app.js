...
function smoothCompassHeading(raw){
 ...
 const now=performance.now();compassSampleBuffer.push({v:raw,t:now});
 compassSampleBuffer=compassSampleBuffer.filter(x=>now-x.t<=650).slice(-14);
 const mean=circularMeanCompass(compassSampleBuffer.map(x=>x.v));if(mean==null)return null;
 const northDist=Math.min(mean,360-mean),rawNorthDist=Math.min(raw,360-raw);
 // Bloqueo del Norte con histéresis. 357° y 0° son prácticamente la misma dirección:
 // entra suavemente en 0° dentro de ±4,5° y no libera el Norte hasta apartarse de ±9°.
 const northEnter=4.5,northExit=9;
 if(compassNorthLocked){
  if(rawNorthDist>northExit&&northDist>northExit){compassNorthLocked=false;compassNorthLockCount=0;compassSampleBuffer=[{v:raw,t:now}]}
 }else if(northDist<=northEnter){
  compassNorthLockCount+=1;if(compassNorthLockCount>=2)compassNorthLocked=true;
 }else compassNorthLockCount=0;
 const target=(compassNorthLocked||northDist<=northEnter)?0:mean;
 if(compassNorthLocked){compassSmoothedHeading=0;return 0}
 if(compassSmoothedHeading==null){compassSmoothedHeading=target;return target}
 const delta=((target-compassSmoothedHeading+540)%360)-180,ad=Math.abs(delta);
 // Zona muerta + media circular: evita que la aguja tiemble con el móvil quieto.
 if(ad<1.15)return compassSmoothedHeading;
 const gain=ad<4?.075:ad<12?.13:ad<35?.24:.44;
 compassSmoothedHeading=normalizeCompassDeg(compassSmoothedHeading+delta*gain);
 if(compassNorthLocked&&Math.min(compassSmoothedHeading,360-compassSmoothedHeading)<1.2)compassSmoothedHeading=0;
...
function compassSample(raw,accuracy,source){
 ...
 // Limita el refresco visual a ~12 Hz: suficiente al girar y mucho más estable en reposo.
 if(now-compassLastEmit<80)return;compassLastEmit=now;
...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v92')}catch(_){ }
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v92','1')}catch{}
