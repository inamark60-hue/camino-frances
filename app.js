...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>{const zoomAct=e=>{e.preventDefault();e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));};b.addEventListener('pointerup',zoomAct,{passive:false});b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();});});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  ...
  const translatePreview=(dx,dy)=>{for(const node of [this.tiles,this.svg,this.markers])node.style.transform=`translate(${dx}px,${dy}px)`;};
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=34?best:-1;};
  const point=e=>({x:e.clientX,y:e.clientY});
  const beginPan=p=>{const cp=ll2world(this.center.lat,this.center.lon,this.zoom);gesture={mode:'pan',start:p,last:p,cp,dx:0,dy:0,moved:false,t:performance.now()};};
  const beginPinch=()=>{const a=[...pts.values()].slice(0,2);if(a.length<2)return;gesture={mode:'pinch',lastMid:mid(a[0],a[1]),lastDist:Math.max(1,dist(a[0],a[1])),moved:true};resetTransforms();};
  surface.addEventListener('pointerdown',e=>{if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();e.stopPropagation();this.userInteracted=true;this.closePopup();pts.set(e.pointerId,point(e));try{surface.setPointerCapture(e.pointerId)}catch{}if(pts.size===1)beginPan(point(e));else beginPinch();},{passive:false});
  surface.addEventListener('pointermove',e=>{if(!pts.has(e.pointerId))return;e.preventDefault();e.stopPropagation();pts.set(e.pointerId,point(e));if(pts.size===1){const p=[...pts.values()][0];if(!gesture||gesture.mode!=='pan')beginPan(p);gesture.dx=p.x-gesture.start.x;gesture.dy=p.y-gesture.start.y;gesture.last=p;if(Math.abs(gesture.dx)+Math.abs(gesture.dy)>3)gesture.moved=true;translatePreview(gesture.dx,gesture.dy);return;}const a=[...pts.values()].slice(0,2),m=mid(a[0],a[1]),d=Math.max(1,dist(a[0],a[1]));if(!gesture||gesture.mode!=='pinch'){beginPinch();return;}const mdx=m.x-gesture.lastMid.x,mdy=m.y-gesture.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.1){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/gesture.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;gesture.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.92&&this.zoom>5){this.zoom--;gesture.lastDist=d;this.clearTileNodes();this.render();}gesture.lastMid=m;gesture.moved=true;},{passive:false});
  const finish=e=>{if(!pts.has(e.pointerId))return;e.preventDefault();e.stopPropagation();const end=point(e),was=gesture;pts.delete(e.pointerId);try{surface.releasePointerCapture(e.pointerId)}catch{}if(pts.size>=2){beginPinch();return;}if(pts.size===1){resetTransforms();beginPan([...pts.values()][0]);if(gesture)gesture.moved=true;return;}if(was?.mode==='pan'){if(was.moved)this.center=world2ll(was.cp.x-was.dx,was.cp.y-was.dy,this.zoom);else if(performance.now()-was.t<750){const i=nearestYellow(end.x,end.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);gesture=null;resetTransforms();return;}}}gesture=null;resetTransforms();this.render();};
  surface.addEventListener('pointerup',finish,{passive:false});surface.addEventListener('pointercancel',e=>{if(!pts.has(e.pointerId))return;pts.delete(e.pointerId);gesture=null;resetTransforms();this.render()},{passive:false});
  surface.addEventListener('dblclick',e=>{e.preventDefault();this.setZoom(this.zoom+1)},{passive:false});
  // V10.0.1: respaldo táctil para navegadores Android/Samsung que no entreguen Pointer Events de forma fiable.
  if(!window.PointerEvent){
   let tStart=null,tCenter=null,tDist=0,tMoved=false;
   const tPoint=t=>({x:t.clientX,y:t.clientY});
   surface.addEventListener('touchstart',e=>{e.preventDefault();this.userInteracted=true;this.closePopup();const ts=[...e.touches];if(ts.length===1){tStart=tPoint(ts[0]);tCenter=ll2world(this.center.lat,this.center.lon,this.zoom);tMoved=false}else if(ts.length>=2){const a=tPoint(ts[0]),b=tPoint(ts[1]);tStart={x:(a.x+b.x)/2,y:(a.y+b.y)/2};tCenter=ll2world(this.center.lat,this.center.lon,this.zoom);tDist=Math.max(1,Math.hypot(a.x-b.x,a.y-b.y));tMoved=true}},{passive:false});
   surface.addEventListener('touchmove',e=>{e.preventDefault();const ts=[...e.touches];if(!tStart)return;if(ts.length===1){const p=tPoint(ts[0]),dx=p.x-tStart.x,dy=p.y-tStart.y;tMoved=tMoved||Math.abs(dx)+Math.abs(dy)>3;translatePreview(dx,dy)}else if(ts.length>=2){resetTransforms();const a=tPoint(ts[0]),b=tPoint(ts[1]),m={x:(a.x+b.x)/2,y:(a.y+b.y)/2},d=Math.max(1,Math.hypot(a.x-b.x,a.y-b.y)),cp=tCenter||ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-(m.x-tStart.x),cp.y-(m.y-tStart.y),this.zoom);const ratio=d/(tDist||d),max=this.layer==='topo'?17:18;if(ratio>1.12&&this.zoom<max){this.zoom++;tDist=d;tCenter=ll2world(this.center.lat,this.center.lon,this.zoom);tStart=m;this.clearTileNodes();this.render()}else if(ratio<.88&&this.zoom>5){this.zoom--;tDist=d;tCenter=ll2world(this.center.lat,this.center.lon,this.zoom);tStart=m;this.clearTileNodes();this.render()}}},{passive:false});
   surface.addEventListener('touchend',e=>{e.preventDefault();if(e.touches.length)return;if(tStart&&tMoved){const changed=e.changedTouches?.[0],p=changed?tPoint(changed):tStart,dx=p.x-tStart.x,dy=p.y-tStart.y;this.center=world2ll((tCenter||ll2world(this.center.lat,this.center.lon,this.zoom)).x-dx,(tCenter||ll2world(this.center.lat,this.center.lon,this.zoom)).y-dy,this.zoom)}resetTransforms();tStart=null;tCenter=null;tDist=0;tMoved=false;this.render()},{passive:false});
  }
 ...
 destroy(){window.removeEventListener('resize',this._resizeHandler);try{this._globalTouchCleanup?.()}catch{}this.activePointers.clear();this.clearTileNodes();this.el.innerHTML='';}
}
