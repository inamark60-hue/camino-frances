...
function normalizeStageOsmElement(e,n,route){
 ...
 const details={
  ...
  pilgrim:t.pilgrims||t.pilgrim||t['pilgrim:yes']||'',
...
async function showSpecialServiceList(n,mode){
 ...
 if(mode==='albergues'){
  ...
  host.innerHTML=`<div class="pilgrim-special-head albergue"><div><b>🐚 ALBERGUES DEL CAMINO</b><span>${pack.verified?`${pack.exactCount} albergues/refugios identificados · ${pack.institutionalCount||0} públicos/institucionales`:'Se muestran alojamientos de la ruta para confirmar'}</span></div><button id="specialMapBtn">🎯 VER EN MAPA</button></div>`
   +`<div class="credential-info"><b>🐚 Credencial y Compostela</b><span>Los albergues del Camino suelen poder sellar la credencial; cuando el dato no consta, la app indica “confirmar en recepción”. Conserva los sellos: la Compostela se tramita al llegar a Santiago.</span></div>`
   ...
   +`<p class="data-source-note">Público/institucional solo se marca cuando el nombre, operador o datos abiertos lo permiten identificar. Ordenados por km de marcha; confirma plazas, precio, horario y sello antes de llegar.</p>`;
 }else{
