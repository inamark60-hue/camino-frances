...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v89')}catch(_){ }
...
function skyRenderCanvas(canvas,lat,lon,now,sensor,source='normal'){
 ...
 const active=skyAimEnabled?skyPickAutoConstellation(projectedConst,g):null;skyAutoConstellation=active?.name||null;const nowMs=performance.now();if(active?.abbr!==skyDwell.key){skyDwell={key:active?.abbr||null,since:nowMs}}const dwellReady=!!active&&nowMs-skyDwell.since>=900;
 // Las demás constelaciones quedan tenues; la apuntada resalta con claridad.
 for(const c of projectedConst){const hot=active&&c.abbr===active.abbr;ctx.save();ctx.strokeStyle=hot?'#ffd400':`rgba(103,150,198,${cfg.constAlpha})`;ctx.lineWidth=hot?3.0:.85;ctx.shadowColor=hot?'#ffd400':'transparent';ctx.shadowBlur=hot?(dwellReady?14:8):0;for(const seg of c.screenSegments){ctx.beginPath();ctx.moveTo(seg[0].x,seg[0].y);for(let i=1;i<seg.length;i++)ctx.lineTo(seg[i].x,seg[i].y);ctx.stroke();if(hot){ctx.fillStyle='#ffd400';for(const p of seg){ctx.beginPath();ctx.arc(p.x,p.y,dwellReady?3.8:3.0,0,Math.PI*2);ctx.fill()}}}ctx.restore()}
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v89','1')}catch{}
