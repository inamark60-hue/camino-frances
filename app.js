...
class BCMap{
 ...
 closePopup(){if(this.popup){try{this.popup.remove()}catch{}this.popup=null;}}
 _bind(){
  // V10.0.3: controlador 2D reconstruido sobre el contenedor del mapa.
  // No depende de una capa transparente intermedia: Android/Samsung recibe los toques directamente.
  const controls='button,a,.route-popup,.bc-map-legend,.bc-service-filters,.map-compass-overlay';
  const zoomButtons=[...this.el.querySelectorAll('[data-z]')];
  zoomButtons.forEach(b=>{
    let touchHandledAt=0;
    const act=e=>{e?.preventDefault?.();e?.stopPropagation?.();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));};
    b.addEventListener('pointerup',e=>{if(e.pointerType==='touch'||e.pointerType==='pen'){touchHandledAt=Date.now();act(e)}},{passive:false});
    b.addEventListener('click',e=>{if(Date.now()-touchHandledAt<700){e.preventDefault();e.stopPropagation();return}act(e)});
  });
  ...
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=38?best:-1;};

  // Ratón / lápiz no táctil.
  let mousePan=null;
  this.el.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||e.target.closest(controls)||(e.pointerType==='mouse'&&e.button!==0))return;e.preventDefault();this.userInteracted=true;this.closePopup();const cp=ll2world(this.center.lat,this.center.lon,this.zoom);mousePan={id:e.pointerId,x:e.clientX,y:e.clientY,cp,dx:0,dy:0};try{this.el.setPointerCapture(e.pointerId)}catch{}},{passive:false});
  this.el.addEventListener('pointermove',e=>{if(!mousePan||e.pointerId!==mousePan.id)return;e.preventDefault();mousePan.dx=e.clientX-mousePan.x;mousePan.dy=e.clientY-mousePan.y;translatePreview(mousePan.dx,mousePan.dy)},{passive:false});
  const endMouse=e=>{if(!mousePan||e.pointerId!==mousePan.id)return;e.preventDefault();this.center=world2ll(mousePan.cp.x-mousePan.dx,mousePan.cp.y-mousePan.dy,this.zoom);mousePan=null;resetTransforms();this.render();};
  this.el.addEventListener('pointerup',endMouse,{passive:false});this.el.addEventListener('pointercancel',endMouse,{passive:false});

  // Android/iOS: Touch Events directos en .bc-map, como las versiones 9.x más robustas.
  let ts=null;
  const tp=t=>({x:t.clientX,y:t.clientY}),touchList=e=>[...e.touches].map(tp);
  this.el.addEventListener('touchstart',e=>{
    if(e.target.closest(controls))return;
    const ps=touchList(e);if(!ps.length)return;
    e.preventDefault();this.userInteracted=true;this.closePopup();resetTransforms();
    if(ps.length===1){ts={mode:'pan',start:ps[0],last:ps[0],cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now()};}
    else{ts={mode:'pinch',lastMid:mid(ps[0],ps[1]),lastDist:Math.max(1,dist(ps[0],ps[1])),moved:true};}
  },{passive:false});
  this.el.addEventListener('touchmove',e=>{
    if(!ts||e.target.closest(controls))return;const ps=touchList(e);if(!ps.length)return;e.preventDefault();
    if(ps.length===1){if(ts.mode!=='pan'){ts={mode:'pan',start:ps[0],last:ps[0],cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:true,t:performance.now()};return}const p=ps[0];ts.dx=p.x-ts.start.x;ts.dy=p.y-ts.start.y;ts.last=p;if(Math.abs(ts.dx)+Math.abs(ts.dy)>3)ts.moved=true;translatePreview(ts.dx,ts.dy);return;}
    const a=ps[0],b=ps[1],m=mid(a,b),d=Math.max(1,dist(a,b));if(ts.mode!=='pinch'){ts={mode:'pinch',lastMid:m,lastDist:d,moved:true};resetTransforms();return}const mdx=m.x-ts.lastMid.x,mdy=m.y-ts.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.1){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/ts.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;ts.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.92&&this.zoom>5){this.zoom--;ts.lastDist=d;this.clearTileNodes();this.render();}ts.lastMid=m;ts.moved=true;
  },{passive:false});
  this.el.addEventListener('touchend',e=>{
    if(!ts)return;e.preventDefault();const ps=touchList(e);
    if(ps.length>=2){ts={mode:'pinch',lastMid:mid(ps[0],ps[1]),lastDist:Math.max(1,dist(ps[0],ps[1])),moved:true};resetTransforms();return;}
    if(ps.length===1){const p=ps[0];resetTransforms();ts={mode:'pan',start:p,last:p,cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:true,t:performance.now()};return;}
    const was=ts,ch=e.changedTouches?.[0],end=ch?tp(ch):was.last;ts=null;resetTransforms();
    if(was.mode==='pan'){if(was.moved){this.center=world2ll(was.cp.x-was.dx,was.cp.y-was.dy,this.zoom);this.render();return;}if(end&&performance.now()-was.t<800){const i=nearestYellow(end.x,end.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);return;}}}
    this.render();
  },{passive:false});
  this.el.addEventListener('touchcancel',()=>{ts=null;resetTransforms();this.render()},{passive:false});
  this.el.addEventListener('dblclick',e=>{if(e.target.closest(controls))return;e.preventDefault();this.setZoom(this.zoom+1)});
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.register('sw.js?v=1006',{updateViaCache:'none'})
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v1006','1')}catch{}
 const u=new URL('./',location.href);u.searchParams.set('bcv','1006');u.searchParams.set('repair',Date.now().toString());location.replace(u.href);
}
