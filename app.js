...
let featureReturn=null,compassHandler=null,compassFallbackHandler=null,compassNoDataTimer=null,compassUpdateTarget=null,compassSmoothedHeading=null,compassSource=null,compassAbsoluteSeen=false,compassCalibrationHandler=null,skyTimer=null,skyLocation=null,bikeRouteCurrent=null;
...
/* ---------- V7.8 · Herramientas internas: bici, brújula estabilizada y cielo ---------- */
...
function renderFocusCompass(heading,accuracy=null,source=''){
 const rose=document.getElementById('focusCompassRose'),value=document.getElementById('focusCompassValue'),card=document.getElementById('focusCompassCardinal'),note=document.getElementById('focusCompassNote');if(!rose||!value)return;
 heading=normalizeCompassDeg(heading);rose.style.transform=`rotate(${-heading}deg)`;value.textContent=`${Math.round(heading)}°`;if(card)card.textContent=compassCardinal(heading);if(note)note.textContent=Number.isFinite(accuracy)&&accuracy>25?'⚠ Calibra (precisión baja)':(source||'Rumbo estabilizado');
}
function focusCompassStatus(kind,title,text){
 const note=document.getElementById('focusCompassNote');if(!note)return;note.textContent=kind==='warning'?`⚠ ${title}`:title;note.title=text||'';
}
async function toggleFocusCompass(){
 const panel=document.getElementById('focusCompassOverlay'),btn=document.getElementById('focusCompass');if(!panel||!btn)return;
 if(!panel.hidden){panel.hidden=true;btn.classList.remove('active');btn.textContent='🧭 BRÚJULA';stopCompassSensors();return}
 panel.hidden=false;btn.classList.add('active');btn.textContent='🧭 BRÚJULA ON';
 await startCompassSensor(renderFocusCompass,focusCompassStatus);
}
...
function openFocusMap(n){
 focusOverlay.hidden=false;focusOverlay.className='focus-overlay';focusOverlay.innerHTML=`<div class="focus-top"><strong>BUEN CAMINO · MAPA</strong><button id="focusCompass" class="focus-compass-toggle">🧭 BRÚJULA</button><button id="focusRoute">🟨 Ruta ${routeVisible?'ON':'OFF'}</button><button id="focusCenter">🎯 CENTRAR RUTA</button><button id="focusProfileToggle">⛰ PERFIL</button><button id="focusServices">📌 Servicios</button><button id="focus3D" class="terrain-top-btn">🏔 3D</button><button id="focusRotate">↻ HORIZONTAL</button><button id="focusClose">✕</button></div><div class="focus-body"><div class="focus-map"><div id="focusMapEl" class="bc-map"></div><div id="focusCompassOverlay" class="focus-compass-overlay" hidden aria-live="polite"><button id="focusCompassClose" class="focus-compass-close" type="button" aria-label="Ocultar brújula">×</button><div class="focus-compass-mini"><div id="focusCompassRose" class="focus-compass-rose"><b class="n">N</b><b class="e">E</b><b class="s">S</b><b class="w">O</b><i></i></div><span class="focus-compass-arrow">▲</span><span class="focus-compass-dot"></span></div><div class="focus-compass-reading"><strong id="focusCompassValue">—°</strong><b id="focusCompassCardinal">—</b><small id="focusCompassNote">Activando…</small></div></div><aside id="focusProfilePanel" class="focus-profile-overlay"><div class="elevation-head"><div><span>DESNIVEL</span><strong id="focusReadout">Toca o desliza</strong></div><button id="focusProfileClose" class="profile-close" aria-label="Ocultar perfil">×</button></div><div id="focusProfile" class="elevation-chart"></div></aside></div></div>`;
 document.body.style.overflow='hidden';setTimeout(()=>{focusMap=buildStageMap('focusMapEl',n,false);setupProfile('focusProfile',n,focusMap,'focusReadout')},50);
 const panel=document.getElementById('focusProfilePanel'),toggleProfile=()=>panel.classList.toggle('profile-hidden');document.getElementById('focusRoute').onclick=()=>toggleRoute(document.getElementById('focusRoute'));document.getElementById('focusCenter').onclick=()=>{if(focusMap?.route?.length){focusMap.userInteracted=false;focusMap.fitRoute();toast('🎯 Ruta centrada en mapa grande')}else toast('La ruta todavía se está cargando')};document.getElementById('focusCompass').onclick=toggleFocusCompass;document.getElementById('focusCompassClose').onclick=toggleFocusCompass;document.getElementById('focusProfileToggle').onclick=toggleProfile;document.getElementById('focusProfileClose').onclick=toggleProfile;document.getElementById('focusServices').onclick=()=>showMapServices(n,focusMap);document.getElementById('focus3D').onclick=()=>open3DMap(n);document.getElementById('focusClose').onclick=closeFocusMap;document.getElementById('focusRotate').onclick=requestLandscape;
}
function closeFocusMap(){stopCompassSensors();focusOverlay.classList.remove('force-landscape');focusMap?.destroy?.();focusMap=null;focusOverlay.hidden=true;focusOverlay.innerHTML='';document.body.style.overflow='';}
window.addEventListener('resize',()=>{if(!focusOverlay.hidden){if(window.innerWidth>window.innerHeight&&focusOverlay.classList.contains('force-landscape')){focusOverlay.classList.remove('force-landscape');const b=document.getElementById('focusRotate');if(b)b.textContent='↻ HORIZONTAL';if(compassUpdateTarget)compassSmoothedHeading=null;}setTimeout(()=>focusMap?.render(),180)}});
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>20. 🧭 Brújula</strong><p>Usa rumbo magnético/absoluto del móvil con filtro anti-oscilación. Puedes abrirla como herramienta independiente o, dentro de MAPA GRANDE, pulsar 🧭 BRÚJULA para verla semitransparente encima del plano mientras sigues moviendo y ampliando el mapa. Concede permiso si el sistema lo solicita; el rumbo no se guarda.</p></div>
