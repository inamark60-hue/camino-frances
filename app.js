...
function smoothCompassHeading(raw){
 ...
 if(compassNorthLocked){
  if(rawNorthDist>6.5){compassNorthLocked=false;compassNorthLockCount=0;compassSampleBuffer=[{v:raw,t:now}]}
 }else if(northDist<=2.8){
  compassNorthLockCount+=1;if(compassNorthLockCount>=4)compassNorthLocked=true;
 }else compassNorthLockCount=0;
 const target=compassNorthLocked?0:mean;
...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v91')}catch(_){ }
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v91','1')}catch{}
