...
async function loadNearbyPhoto(lat,lon,host){
 if(!host)return;host.innerHTML='<span class="photo-loading">📷 Buscando fotos cercanas libres…</span>';
 const api='https://commons.wikimedia.org/w/api.php?origin=*&format=json&action=query&generator=geosearch&ggsnamespace=6&ggsradius=1500&ggslimit=8&ggscoord='+encodeURIComponent(lat+'|'+lon)+'&prop=imageinfo&iiprop=url|extmetadata&iiurlwidth=420';
 try{const r=await fetch(api,{cache:'force-cache'});if(!r.ok)throw 0;const j=await r.json(),pages=Object.values(j?.query?.pages||{}),photos=[];for(const page of pages){const ii=page?.imageinfo?.[0],md=ii?.extmetadata||{},license=stripHtmlText(md.LicenseShortName?.value||md.UsageTerms?.value||'');if(!ii?.thumburl||!/(cc\s*by|cc0|public domain|pd-|gfdl|free art)/i.test(license))continue;photos.push({thumb:ii.thumburl,page:ii.descriptionurl||('https://commons.wikimedia.org/wiki/'+encodeURIComponent(String(page.title||'').replace(/ /g,'_'))),title:String(page.title||'').replace(/^File:/,''),artist:stripHtmlText(md.Artist?.value||md.Credit?.value||'Autor no indicado'),license,licenseUrl:md.LicenseUrl?.value||''});if(photos.length>=3)break}if(!photos.length)throw 0;host.innerHTML=`<div class="route-photo-grid">${photos.map(p=>`<figure><a href="${esc(p.page)}" target="_blank" rel="noopener"><img src="${esc(p.thumb)}" alt="Foto cercana del Camino" loading="lazy"></a><figcaption><span>${esc(p.artist)}</span><a href="${esc(p.licenseUrl||p.page)}" target="_blank" rel="noopener">${esc(p.license)}</a></figcaption></figure>`).join('')}</div><small class="route-photo-license">Wikimedia Commons · imágenes de licencia libre o dominio público; respeta la atribución indicada.</small>`;
...
/* ---------- V8.0 · Herramientas internas: a pie, bici, brújula, cielo y mapas ---------- */
...
function stopFeatureSensors(){
 ...
 skyTrackActive=false;skyOrientationState=null;skyCompassCtrl=null;
}
...
function featureShell(icon,title,subtitle,content){
 app.innerHTML=`<section class="feature-view"><header class="feature-head"><button id="featureBack" class="feature-back" type="button" aria-label="Volver a la ventana anterior">← <span>BUEN CAMINO</span></button><div><span>${icon} HERRAMIENTA</span><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div></header>${content}</section>`;
...
async function loadWalkStage(n){
 ...
 host.innerHTML='';mainMap=new BCMap(host,{lat:route[0][0],lon:route[0][1],zoom:11,layer:'topo'});mainMap.stageNo=n;mainMap.setRoute(route,true);mainMap.setRouteVisible(true);walkRouteCurrent=route;
...
function skyStaticPoint(h,cx=180,cy=180,R=154){if(h.alt<=0)return null;const rr=(90-h.alt)/90*R,a=h.az*Math.PI/180;return{x:cx+rr*Math.sin(a),y:cy-rr*Math.cos(a)}}
function skySensorPoint(h,basis,cx=180,cy=180){const v=skyWorldVector(h),x=dot3(v,basis.right),y=dot3(v,basis.up),z=dot3(v,basis.forward);if(z<=.12)return null;const f=170,p={x:cx+f*x/z,y:cy-f*y/z};if(p.x<-35||p.x>395||p.y<-35||p.y>395)return null;return p}
function bindSkyConstellationClicks(){
 document.querySelectorAll('#skySvg [data-con],#skyVisible [data-con]').forEach(el=>{el.style.cursor='pointer';el.onclick=e=>{e.stopPropagation();openConstellationInfo(el.dataset.con)}});
}
function renderSky(lat,lon,label){
 skyLocation={lat,lon,label};const svg=document.getElementById('skySvg'),info=document.getElementById('skyInfo'),visEl=document.getElementById('skyVisible');if(!svg)return;const now=new Date(),positions=new Map(),visible=[],sensor=skyTrackActive&&skyOrientationState,basis=sensor?deviceSkyBasis(skyOrientationState.alpha,skyOrientationState.beta,skyOrientationState.gamma):null;
 for(const st of SKY_STARS){const h=skyHorizontal(st,lat,lon,now),pt=sensor?skySensorPoint(h,basis):skyStaticPoint(h);if(!pt)continue;positions.set(st.name,{...pt,...h,st});visible.push({...pt,...h,st})}
 let base='';if(sensor){base=`<rect x="0" y="0" width="360" height="360" rx="18" class="sky-live-bg"/><circle cx="180" cy="180" r="7" class="sky-reticle"/><line x1="166" y1="180" x2="194" y2="180" class="sky-reticle-line"/><line x1="180" y1="166" x2="180" y2="194" class="sky-reticle-line"/>`}else{base=`<circle cx="180" cy="180" r="154" class="sky-horizon"/><circle cx="180" cy="180" r="103" class="sky-alt"/><circle cx="180" cy="180" r="52" class="sky-alt"/><line x1="180" y1="26" x2="180" y2="334" class="sky-axis"/><line x1="26" y1="180" x2="334" y2="180" class="sky-axis"/><text x="180" y="18" text-anchor="middle" class="sky-dir">N</text><text x="346" y="184" text-anchor="middle" class="sky-dir">E</text><text x="180" y="354" text-anchor="middle" class="sky-dir">S</text><text x="14" y="184" text-anchor="middle" class="sky-dir">O</text>`}
 ...
 const stars=visible.sort((a,b)=>a.st.mag-b.st.mag).map(p=>{const rad=Math.max(2,Math.min(5.4,4.4-p.st.mag*.55)),labelStar=p.st.mag<=1.0||['Polaris','Vega','Deneb','Betelgeuse'].includes(p.st.name);return `<g data-con="${esc(p.st.con)}"><circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${rad.toFixed(1)}" class="sky-star"><title>${esc(p.st.name)} · ${esc(p.st.con)} · altitud ${Math.round(p.alt)}°</title></circle>${labelStar?`<text x="${(p.x+6).toFixed(1)}" y="${(p.y-5).toFixed(1)}" class="sky-star-label">${esc(p.st.name)}</text>`:''}</g>`}).join('');
 svg.innerHTML=base+lines+stars+(sensor?'':'<circle cx="180" cy="180" r="3" class="sky-zenith"/>');
...
function skyOrientationEvent(e,source){
 ...
 skyOrientationState={alpha,beta,gamma};if(skyLocation&&document.getElementById('skySvg'))
...
class BCMap{
 ...
 _build(){this.el.innerHTML='<div class="bc-tiles"></div><svg class="bc-overlay"></svg><div class="bc-markers"></div><div class="bc-service-filters" hidden></div><div class="bc-map-controls"><button data-z="1" aria-label="Acercar">+</button><button data-z="-1" aria-label="Alejar">−</button></div><div class="bc-map-view-controls"><button class="bc-3d-mini" type="button" aria-label="Abrir mapa 3D"><span>3D</span></button><button class="bc-street-mini" type="button" aria-label="Abrir Street View cerca del centro del mapa"><span class="street-man" aria-hidden="true"></span><small>STREET<br>VIEW</small></button><button class="bc-symbols-btn" type="button" aria-label="Ver significado de los símbolos del mapa">◉<small>SÍMBOLOS</small></button></div><aside class="bc-map-legend" hidden><div><b>🗺 SÍMBOLOS DEL MAPA</b><button type="button" class="bc-map-legend-close" aria-label="Cerrar leyenda">×</button></div><span><i class="legend-route"></i> Ruta del Camino</span><span><i class="legend-yellow"></i> Punto amarillo: punto de ruta</span><span>🎯 Centrar toda la ruta</span><span>📍 GPS / posición</span><span>🛏 🍴 💧 Servicios</span><span>⛰ TOPO: curvas de nivel y relieve</span><span><b class="legend-3d">3D</b> Relieve, caminos y edificios 3D</span><span><i class="legend-man"></i> Street View / imagen a nivel de calle</span></aside><button class="bc-center-route" title="Centrar toda la ruta" aria-label="Centrar ruta">🎯<span>RUTA</span></button><button class="bc-layer" title="Cambiar mapa" aria-label="Cambiar tipo de mapa"></button><div class="bc-scale"></div><div class="bc-map-msg">Cargando mapa…</div>';this.tiles=this.el.querySelector('.bc-tiles');this.svg=this.el.querySelector('.bc-overlay');this.markers=this.el.querySelector('.bc-markers');this.scale=this.el.querySelector('.bc-scale');this.msg=this.el.querySelector('.bc-map-msg');this.popup=null;this.serviceFilters=this.el.querySelector('.bc-service-filters');this.updateLayerButton();}
...
async function open3DMap(n){
 ...
 try{
  ...
  terrain3DMap.on('load',()=>{
   ...
   try{if(!terrain3DMap.getSource('topo'))terrain3DMap.addSource('topo',{type:'raster',tiles:['https://a.tile.opentopomap.org/{z}/{x}/{y}.png','https://b.tile.opentopomap.org/{z}/{x}/{y}.png','https://c.tile.opentopomap.org/{z}/{x}/{y}.png'],tileSize:256,maxzoom:17,attribution:'© OpenStreetMap · SRTM · OpenTopoMap'});const firstLabel=(terrain3DMap.getStyle().layers||[]).find(l=>l.type==='symbol')?.id;terrain3DMap.addLayer({id:'topo-base',type:'raster',source:'topo',paint:{'raster-opacity':0.82}},firstLabel);const baseLayers=(terrain3DMap.getStyle().layers||[]).filter(l=>l.id!=='topo-base'&&['background','fill','raster'].includes(l.type)).map(l=>l.id);terrain3DMap._bcBaseLayers=baseLayers;baseLayers.forEach(id=>{try{terrain3DMap.setLayoutProperty(id,'visibility','none')}catch{}})}catch(e){console.warn('Topo 3D',e)}
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>22. ✨ Estrellas y constelaciones</strong><p>Primero pulsa <b>USAR MI GPS</b> y después <b>ACTIVAR MODO CIELO</b>. Apunta la parte trasera del móvil hacia el cielo y muévelo lentamente: la carta responde a los sensores. Toca una constelación o su nombre para ver información y fotografías reales obtenidas de Wikimedia Commons. Las imágenes se muestran solo cuando consta una licencia libre o dominio público, junto con autor y licencia. La brújula pequeña funciona igual que en los mapas: transparente, arrastrable y con cambio de colores al alinear el norte.</p></div>
 <div class="guide-card"><strong>23. 🗺 Símbolos · Street View · 3D</strong><p>En el lateral derecho de los mapas están los controles 3D, el hombre amarillo de STREET VIEW y el botón SÍMBOLOS. SÍMBOLOS explica la leyenda. El 3D combina relieve con caminos/edificios de OpenStreetMap/OpenFreeMap y, al elegir TOPO, mantiene visibles las curvas de nivel. Street View busca primero imágenes abiertas de KartaView y ofrece Google Street View como respaldo.</p></div>
 <div class="guide-card"><strong>24. 📷 Fotos Wikimedia cercanas</strong><p>Al tocar una bola amarilla, BUEN CAMINO intenta mostrar directamente varias fotografías cercanas de Wikimedia Commons. No se paga licencia: se filtran archivos con licencia libre o dominio público y se muestra la atribución/licencia disponible. Algunas licencias libres exigen citar al autor.</p></div>
