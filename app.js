...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v98')}catch(_){ }
...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>b.onclick=e=>{e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  // V9.8: Android/Chrome usa Touch Events nativos para los dedos.
  // Pointer Events quedan reservados a ratón/lápiz para evitar la capa de compatibilidad táctil.
  ...
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=30?best:-1;};
  const isControlTarget=t=>t instanceof Element&&!!t.closest('.bc-map-controls,.bc-map-view-controls,.bc-center-route,.bc-layer,.bc-service-filters,.bc-map-legend,.map-compass-overlay,.route-popup');
  const touchPts=ts=>Array.from(ts).map(t=>({x:t.clientX,y:t.clientY,id:t.identifier}));
  let tg=null;
  const beginTouchPan=p=>{tg={mode:'pan',start:{...p},cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now(),last:{...p}};};
  const beginTouchPinch=(a,b)=>{tg={mode:'pinch',lastMid:mid(a,b),lastDist:Math.max(1,dist(a,b)),moved:true,t:performance.now()};resetTransforms();};
  const onTouchStart=e=>{if(isControlTarget(e.target))return;const ps=touchPts(e.touches);if(!ps.length)return;e.preventDefault();e.stopPropagation();this.userInteracted=true;this.closePopup();if(ps.length===1)beginTouchPan(ps[0]);else beginTouchPinch(ps[0],ps[1]);};
  const onTouchMove=e=>{if(!tg||isControlTarget(e.target))return;const ps=touchPts(e.touches);if(!ps.length)return;e.preventDefault();e.stopPropagation();if(ps.length===1){const p=ps[0];if(tg.mode!=='pan')beginTouchPan(p);const dx=p.x-tg.start.x,dy=p.y-tg.start.y;tg.dx=dx;tg.dy=dy;tg.last={...p};if(Math.abs(dx)+Math.abs(dy)>3)tg.moved=true;translatePreview(dx,dy);return;}const a=ps[0],b=ps[1],m=mid(a,b),d=Math.max(1,dist(a,b));if(tg.mode!=='pinch'){beginTouchPinch(a,b);return;}const mdx=m.x-tg.lastMid.x,mdy=m.y-tg.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.2){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/tg.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.10&&this.zoom<max){this.zoom++;tg.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.90&&this.zoom>5){this.zoom--;tg.lastDist=d;this.clearTileNodes();this.render();}tg.lastMid=m;tg.moved=true;};
  const onTouchEnd=e=>{if(!tg)return;e.preventDefault();e.stopPropagation();const ps=touchPts(e.touches);if(ps.length>=2){beginTouchPinch(ps[0],ps[1]);return;}if(ps.length===1){resetTransforms();beginTouchPan(ps[0]);if(tg)tg.moved=true;return;}const ch=e.changedTouches&&e.changedTouches[0],last=ch?{x:ch.clientX,y:ch.clientY}:tg.last;if(tg.mode==='pan'){if(tg.moved)this.center=world2ll(tg.cp.x-tg.dx,tg.cp.y-tg.dy,this.zoom);else if(last&&performance.now()-tg.t<700){const i=nearestYellow(last.x,last.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);}}}tg=null;resetTransforms();this.render();};
  const onTouchCancel=e=>{if(!tg)return;e.preventDefault();tg=null;resetTransforms();this.render();};
  this.el.addEventListener('touchstart',onTouchStart,{passive:false,capture:true});
  this.el.addEventListener('touchmove',onTouchMove,{passive:false,capture:true});
  this.el.addEventListener('touchend',onTouchEnd,{passive:false,capture:true});
  this.el.addEventListener('touchcancel',onTouchCancel,{passive:false,capture:true});

  // Ratón/lápiz: comportamiento de arrastre simple, independiente del táctil.
  let mp=null;
  this.el.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||isControlTarget(e.target)||(e.pointerType==='mouse'&&e.button!==0))return;e.preventDefault();this.userInteracted=true;this.closePopup();mp={id:e.pointerId,start:{x:e.clientX,y:e.clientY},cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now()};try{this.el.setPointerCapture(e.pointerId)}catch{}},{passive:false});
  this.el.addEventListener('pointermove',e=>{if(e.pointerType==='touch'||!mp||mp.id!==e.pointerId)return;e.preventDefault();mp.dx=e.clientX-mp.start.x;mp.dy=e.clientY-mp.start.y;if(Math.abs(mp.dx)+Math.abs(mp.dy)>3)mp.moved=true;translatePreview(mp.dx,mp.dy)},{passive:false});
  const finishMouse=e=>{if(e.pointerType==='touch'||!mp||mp.id!==e.pointerId)return;e.preventDefault();if(mp.moved)this.center=world2ll(mp.cp.x-mp.dx,mp.cp.y-mp.dy,this.zoom);else if(performance.now()-mp.t<700){const i=nearestYellow(e.clientX,e.clientY);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);}}mp=null;resetTransforms();this.render();};
  this.el.addEventListener('pointerup',finishMouse,{passive:false});this.el.addEventListener('pointercancel',e=>{if(mp&&mp.id===e.pointerId){mp=null;resetTransforms();this.render()}},{passive:false});
  this.el.addEventListener('dblclick',e=>{if(isControlTarget(e.target))return;e.preventDefault();this.setZoom(this.zoom+1)},{passive:false});
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v98','1')}catch{}
 const u=new URL('./',location.href);u.searchParams.set('bcv','91');u.searchParams.set('repair',Date.now().toString());location.replace(u.href);
}
