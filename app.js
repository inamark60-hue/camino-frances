...
let currentView='home', currentStageNo=null, mainMap=null, focusMap=null, terrain3DMap=null, terrain3DOverlay=null, routeVisible=localStorage.getItem('bc-route')!=='0';
...
function specialServiceItems(all,mode){
 if(mode==='albergues'){
  ...
  return {items:(exact.length?exact:fallback).map(x=>({...x,specialIcon:'🥾',specialBadge:exact.length?'🥾 Albergue del Camino':'🛏 Alojamiento de la ruta'})),verified:exact.length>0,exactCount:exact.length};
 }
 if(mode==='menu'){
...
async function showSpecialServiceList(n,mode){
 ...
 if(mode==='albergues'){
  ...
  host.innerHTML=`<div class="pilgrim-special-head albergue"><div><b>🥾 ALBERGUES DEL CAMINO</b><span>${pack.verified?`${pack.exactCount} albergues/refugios identificados`:'No se distinguen albergues explícitos: se muestran alojamientos de la ruta para confirmar'}</span></div><button id="specialMapBtn">📍 VER EN MAPA</button></div>`
   +items.slice(0,70).map(serviceCard).join('')
   +`<p class="data-source-note">Ordenados por km de marcha. Los datos proceden de puntos abiertos del Camino y OpenStreetMap; confirma plazas, horarios y condiciones antes de llegar.</p>`;
 }else{
...
function setActive(k){
 ...
 if(k==='home'){
  ...
  top.classList.remove('earth-home-btn');
  top.innerHTML='<b>?</b> DUDAS';
  top.title='Dudas y consejos del peregrino';
  top.onclick=openDoubts;
 }
}
