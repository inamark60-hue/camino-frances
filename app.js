...
function quickButtons(){return [
 ...
 ['🟨','Sellos','credencial peregrino'],['🛡','Policía','policía guardia civil'],['🏛','Ayuntamiento','ayuntamiento'],['⚠','Emergencias','emergencia']
...
<section class="hero"><h1>Camino Francés<br>cartografía de peregrino</h1><p>33 etapas, mapa topográfico, GPS, servicios y datos abiertos. Pensada exclusivamente para Android.</p><span class="pill">GRATUITA · PWA · V4.2 PRO</span></section>
...
function renderDetailBody(s,tab){const body=document.getElementById('detailBody');
 if(tab==='resumen'){
   ...
   body.innerHTML=`<section class="section"><div class="stat-grid"><div class="stat">🚶<strong>${s.km.toFixed(1)} km</strong><span>Distancia plan</span></div><div class="stat">◷<strong>${s.h}</strong><span>Tiempo</span></div><div class="stat">⛰<strong>+${s.gain} m</strong><span>Desnivel</span></div><div class="stat">▥<strong>${s.difficulty}</strong><span>Dificultad</span></div></div>${stage1}<h2>Preparación de etapa</h2><div class="checklist"><span>💧 Agua y comida</span><span>🌦 Meteorología</span><span>🔋 Batería / powerbank</span><span>🩹 Botiquín</span><span>🪪 Credencial</span><span>🧥 Capa y abrigo</span></div><h2>Mapa topográfico</h2><div class="map-toolbar inline-toolbar"><button id="routeBtn" class="map-chip ${routeVisible?'active':''}">🟨 Ruta ${routeVisible?'ON':'OFF'}</button></div><div id="stageMap" class="mapbox"></div><div class="map-note">Base topográfica con curvas de nivel: OpenTopoMap. Ruta: línea guía fina + puntos amarillos para no tapar senderos, cruces ni cotas.</div></section>`;setTimeout(async()=>{await makeMap(s.n,'stageMap',false);bindMapToolbar(s.n)},0)
 } else if(tab==='mapa'){
   body.innerHTML=`<div class="map-toolbar"><button id="topoBtn" class="map-chip active">⛰ Topográfico</button><button id="osmBtn" class="map-chip">🛣 Calles</button><button id="routeBtn" class="map-chip ${routeVisible?'active':''}">🟨 Ruta ${routeVisible?'ON':'OFF'}</button><button id="poisBtn" class="map-chip">📍 Servicios</button></div><div id="stageMapTall" class="mapbox tall"></div><section class="section"><button class="location-cta full" id="locateOnMap">📍 MOSTRAR MI UBICACIÓN</button><div id="mapStatus" class="route-source">Cargando trazado detallado · puntos de Camino…</div><div id="poiLegend" class="poi-legend"></div></section>`;setTimeout(async()=>{await makeMap(s.n,'stageMapTall',false,true);document.getElementById('locateOnMap').onclick=locateUser;bindMapToolbar(s.n)},0)
...
function serviceCard(x){return `<article class="service-card"><div><span class="badge">${esc(x.type)}</span><h3>${esc(x.name)}</h3><p>${esc(x.sub)}</p><p>${esc(x.note)}</p></div><div><div class="price">${esc(x.price)}</div><button class="fav ${favorites.has(x.id)?'on':''}" data-fav="${x.id}">♥</button></div></article>`}
...
function routeLayers(map,segs){
  ...
  segs.forEach(seg=>{
    // Línea guía muy fina y semitransparente: mantiene continuidad sin tapar el sendero.
    L.polyline(seg,{renderer,color:'#17345f',weight:1.35,opacity:.38,lineCap:'round',lineJoin:'round',interactive:false}).addTo(g);
    // Puntos amarillos del Camino. El muestreo limita la densidad para no ocultar curvas de nivel ni cruces.
    const targetDots=240;
    const step=Math.max(1,Math.ceil(seg.length/targetDots));
    for(let i=0;i<seg.length;i+=step){
      L.circleMarker(seg[i],{renderer,radius:2.35,color:'#17345f',weight:.75,fillColor:'#ffc400',fillOpacity:.92,interactive:false}).addTo(g);
    }
    if(seg.length>1){
      L.circleMarker(seg[seg.length-1],{renderer,radius:2.35,color:'#17345f',weight:.75,fillColor:'#ffc400',fillOpacity:.92,interactive:false}).addTo(g);
    }
  });
  g.addTo(map);
  return g
...
async function makeMap(n,id,fullRoute=false,withPois=false){if(mapInstance){try{mapInstance.remove()}catch{}mapInstance=null}const el=document.getElementById(id);if(!el||typeof L==='undefined'){if(el)el.innerHTML='<div class="empty">El mapa necesita conexión la primera vez.</div>';return}el.innerHTML='';mapInstance=L.map(id,{zoomControl:true,preferCanvas:true});mapLayers=baseLayers(mapInstance);let segs=[];let source='';try{await loadOpenPilgrimages();if(fullRoute&&remoteRoute){segs=geoJSONSegments(remoteRoute);source='Open Pilgrimages · trazado completo de alta resolución'}else if(n){segs=stageSegmentsFromRemote(n);if(segs.length)source='Open Pilgrimages · geometría OSM de alta resolución'} }catch{}
 ...
 if(withPois&&n)await showStagePois(n);setTimeout(()=>mapInstance&&mapInstance.invalidateSize(),120)}

function mapView(){currentView='map';setActive('map');app.innerHTML=`<div class="map-toolbar"><button id="topoBtn" class="map-chip active">⛰ Topográfico</button><button id="osmBtn" class="map-chip">🛣 Calles</button><button id="routeBtn" class="map-chip ${routeVisible?'active':''}">🟨 Ruta ${routeVisible?'ON':'OFF'}</button><button id="poisBtn" class="map-chip">📍 Servicios</button></div><div id="mainMap" class="mapbox tall"></div><section class="section"><button class="location-cta full" id="mapLocate">📍 MOSTRAR MI UBICACIÓN</button><p class="map-note">Mapa completo del Camino Francés. OpenTopoMap aporta relieve y curvas de nivel. El recorrido se dibuja con puntos y una guía fina para mantener visible la cartografía.</p><div id="poiLegend" class="poi-legend"></div></section>`;setTimeout(async()=>{await makeMap(null,'mainMap',true,false);document.getElementById('mapLocate').onclick=locateUser;bindMapToolbar(null)},0)}
...
function serviceSearch(type){if(type==='emergencia'){location.href='tel:112';return}if(type==='meteorología'){window.open('https://www.aemet.es/','_blank','noopener');return}const q=encodeURIComponent(type+' Camino de Santiago');window.open('https://www.openstreetmap.org/search?query='+q,'_blank','noopener')}
