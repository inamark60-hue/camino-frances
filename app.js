...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v96')}catch(_){ }
...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>b.onclick=e=>{e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  // V9.6: capa táctil dedicada. Evita que SVG, teselas o marcadores roben gestos en Android/Chrome.
  const gesture=this.el.querySelector('.bc-gesture-layer');
  ...
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=26?best:-1;};
  let ts=null;const touchPoint=t=>({x:t.clientX,y:t.clientY});
  gesture.addEventListener('touchstart',e=>{e.preventDefault();this.userInteracted=true;this.closePopup();const a=[...e.touches];if(a.length===1){const p=touchPoint(a[0]);ts={mode:'pan',start:p,cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now()};}else if(a.length>=2){const p1=touchPoint(a[0]),p2=touchPoint(a[1]);ts={mode:'pinch',lastMid:mid(p1,p2),lastDist:Math.max(1,dist(p1,p2)),moved:true,t:performance.now()};resetTransforms();}},{passive:false,capture:true});
  gesture.addEventListener('touchmove',e=>{e.preventDefault();if(!ts)return;const a=[...e.touches];if(a.length===1&&ts.mode==='pan'){const p=touchPoint(a[0]),dx=p.x-ts.start.x,dy=p.y-ts.start.y;ts.dx=dx;ts.dy=dy;if(Math.abs(dx)+Math.abs(dy)>3)ts.moved=true;translatePreview(dx,dy);return;}if(a.length>=2){const p1=touchPoint(a[0]),p2=touchPoint(a[1]),m=mid(p1,p2),d=Math.max(1,dist(p1,p2));if(ts.mode!=='pinch'){ts={mode:'pinch',lastMid:m,lastDist:d,moved:true,t:performance.now()};resetTransforms();return;}const mdx=m.x-ts.lastMid.x,mdy=m.y-ts.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.2){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/ts.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.10&&this.zoom<max){this.zoom++;ts.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.90&&this.zoom>5){this.zoom--;ts.lastDist=d;this.clearTileNodes();this.render();}ts.lastMid=m;ts.moved=true;}},{passive:false,capture:true});
  const touchEnd=e=>{e.preventDefault();if(!ts)return;const remain=[...e.touches];if(remain.length>=2){const p1=touchPoint(remain[0]),p2=touchPoint(remain[1]);ts={mode:'pinch',lastMid:mid(p1,p2),lastDist:Math.max(1,dist(p1,p2)),moved:true,t:performance.now()};return;}if(remain.length===1){resetTransforms();const p=touchPoint(remain[0]);ts={mode:'pan',start:p,cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:true,t:performance.now()};return;}if(ts.mode==='pan'){if(ts.moved)this.center=world2ll(ts.cp.x-ts.dx,ts.cp.y-ts.dy,this.zoom);else if(performance.now()-ts.t<500){const i=nearestYellow(ts.start.x,ts.start.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);}}}ts=null;resetTransforms();this.render();};
  gesture.addEventListener('touchend',touchEnd,{passive:false,capture:true});gesture.addEventListener('touchcancel',e=>{e.preventDefault();ts=null;resetTransforms();this.render();},{passive:false,capture:true});
  let mousePan=null;gesture.addEventListener('pointerdown',e=>{if(e.pointerType==='touch')return;e.preventDefault();this.userInteracted=true;this.closePopup();gesture.setPointerCapture?.(e.pointerId);mousePan={id:e.pointerId,start:{x:e.clientX,y:e.clientY},cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false};});
  gesture.addEventListener('pointermove',e=>{if(!mousePan||mousePan.id!==e.pointerId)return;e.preventDefault();mousePan.dx=e.clientX-mousePan.start.x;mousePan.dy=e.clientY-mousePan.start.y;if(Math.abs(mousePan.dx)+Math.abs(mousePan.dy)>3)mousePan.moved=true;translatePreview(mousePan.dx,mousePan.dy);});
  const mouseEnd=e=>{if(!mousePan||mousePan.id!==e.pointerId)return;e.preventDefault();if(mousePan.moved)this.center=world2ll(mousePan.cp.x-mousePan.dx,mousePan.cp.y-mousePan.dy,this.zoom);else{const i=nearestYellow(e.clientX,e.clientY);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);}}mousePan=null;resetTransforms();this.render();};gesture.addEventListener('pointerup',mouseEnd);gesture.addEventListener('pointercancel',mouseEnd);gesture.addEventListener('dblclick',e=>{e.preventDefault();this.setZoom(this.zoom+1)});window.addEventListener('resize',this._resizeHandler,{passive:true});}
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v96','1')}catch{}
