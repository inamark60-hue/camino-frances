...
function home(){cleanupMaps();currentView='home';currentStageNo=null;setActive('home');app.innerHTML=`<section class="home-intro"><div><strong>33 etapas · mapas · servicios · gratis</strong><span>Camino Francés</span></div><div class="home-version-badge">V7.5 PRO</div><div class="home-share-row"><button id="whatsappShare" class="whatsapp-share">🟢 WHATSAPP</button><button id="homeComments" class="comments-main">💬 COMENTARIOS</button></div><button id="appGuideBtn" class="app-guide-main">📖 GUÍA APP · CÓMO USAR BUEN CAMINO</button><div class="home-counters compact"><div>👁 <strong>${appVisitCount}</strong><span>visitas</span></div><div>👍 <strong>${likedStages.size}</strong><span>me gusta</span></div><div>💬 <strong>${comments.length}</strong><span>comentarios</span></div></div><small class="counter-note">Contadores de este dispositivo</small></section><section class="section home-first"><div class="section-head"><h2>Las 33 etapas</h2><button class="link-btn" data-nav="stages">Ver todas</button></div>${STAGES.slice(0,5).map(stageCard).join('')}</section><section class="section"><h2>Accesos rápidos</h2><div class="service-grid">${SERVICE_TYPES.slice(0,9).map(x=>`<button class="service-btn" data-quick="${x[1]}"><span>${x[0]}</span>${x[2]}</button>`).join('')}</div></section><section class="section support-card compact-support"><div><b>💛 Donación solidaria</b><span>Creador: ${esc(DONATION_CREATOR)}</span></div><button class="donate-main" data-donate>Donar</button></section>`;bindCommon();document.getElementById('whatsappShare').onclick=shareAppWhatsApp;document.getElementById('homeComments').onclick=openComments;document.getElementById('appGuideBtn').onclick=openAppGuide;}
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>1. 🏠 Inicio</strong><p>Desde Inicio puedes entrar en las etapas, Mi Camino, Favoritos y los accesos rápidos a servicios. También encontrarás Google Earth, la información del creador y la donación solidaria directa a Save the Children.</p></div>
 ...
 <div class="guide-card"><strong>18. 📲 Instalar BUEN CAMINO</strong><p>En Android, abre la web con Chrome y usa “Instalar aplicación” o “Añadir a pantalla de inicio”. En iPhone, abre la web en Safari y utiliza Compartir → Añadir a pantalla de inicio.</p></div>
