...
let featureReturn=null,compassHandler=null,compassFallbackHandler=null,compassNoDataTimer=null,skyTimer=null,skyLocation=null,bikeRouteCurrent=null;
...
/* ---------- V7.7 · Herramientas internas: bici, brújula y cielo ---------- */
...
function rememberFeatureReturn(){featureReturn={view:currentView,stage:currentStageNo,tab:currentDetailTab};}
function stopFeatureSensors(){
 if(compassHandler){window.removeEventListener('deviceorientationabsolute',compassHandler,true);window.removeEventListener('deviceorientation',compassHandler,true);compassHandler=null;}
 ...
 clearTimeout(compassNoDataTimer);compassNoDataTimer=null;
...
function returnFromFeature(){
 ...
 if(r?.view?.startsWith('detail-')){detail(Number(r.stage)||Number((r.view.match(/\d+/)||[])[0])||1,r.tab||'resumen');return}
 if(r?.view==='stages'){stages();return}if(r?.view==='map'){mapView();return}if(r?.view==='favorites'){favoritesView();return}if(r?.view==='mycamino'){myCamino();return}home();
}
function featureShell(icon,title,subtitle,content){
 app.innerHTML=`<section class="feature-view"><header class="feature-head"><button id="featureBack" class="feature-back" type="button" aria-label="Volver a la ventana anterior">← <span>VOLVER</span></button><div><span>${icon} HERRAMIENTA</span><h1>${esc(title)}</h1><p>${esc(subtitle)}</p></div></header>${content}</section>`;
...
function openFeature(kind,stage=null){
 ...
 if(kind==='bike')openBikeFeature(stage);else if(kind==='compass')openCompassFeature(stage);else openSkyFeature(stage);
}
function featureStageOptions(selected=1){return STAGES.map(s=>`<option value="${s.n}" ${s.n===selected?'selected':''}>${s.n}. ${esc(s.from)} → ${esc(s.to)}</option>`).join('')}
...
async function loadBikeStage(n){
 ...
 if(!host.isConnected)return;host.innerHTML='';mainMap=new BCMap(host,{lat:route[0][0],lon:route[0][1],zoom:11,layer:'topo'});mainMap.stageNo=n;mainMap.setRoute(route,true);mainMap.setRouteVisible(true);bikeRouteCurrent=route;
 const s=STAGES[n-1],km=routeKm(route);if(meta)meta.innerHTML=`<strong>${km.toFixed(1)} km</strong><span>${esc(s.from)} → ${esc(s.to)}</span><small>${esc(source)}</small>`;
...
function updateCompassFace(heading,accuracy=null){
 ...
 heading=((Number(heading)%360)+360)%360;dial.style.transform=`rotate(${-heading}deg)`;value.textContent=`${Math.round(heading)}°`;if(card)card.textContent=compassCardinal(heading);if(status){status.className='feature-status ok';status.innerHTML=`<b>Brújula activa</b><span>Rumbo del dispositivo${Number.isFinite(accuracy)?` · precisión aprox. ${Math.round(accuracy)}°`:''}. Si oscila, calibra el móvil moviéndolo suavemente en forma de 8.</span>`}
}
async function startCompass(){
 stopFeatureSensors();const status=document.getElementById('compassStatus');if(status){status.className='feature-status loading';status.innerHTML='<b>Activando sensores…</b><span>Mantén el teléfono aproximadamente horizontal.</span>'}
 try{if(typeof DeviceOrientationEvent!=='undefined'&&typeof DeviceOrientationEvent.requestPermission==='function'){const p=await DeviceOrientationEvent.requestPermission();if(p!=='granted')throw new Error('Permiso no concedido')}
  let received=false;compassHandler=e=>{let h=null;if(Number.isFinite(Number(e.webkitCompassHeading)))h=Number(e.webkitCompassHeading);else if(Number.isFinite(Number(e.alpha)))h=(360-Number(e.alpha))%360;if(h==null)return;received=true;updateCompassFace(h,Number(e.webkitCompassAccuracy));};window.addEventListener('deviceorientationabsolute',compassHandler,true);window.addEventListener('deviceorientation',compassHandler,true);clearTimeout(compassNoDataTimer);compassNoDataTimer=setTimeout(()=>{if(!received&&status?.isConnected){status.className='feature-status warning';status.innerHTML='<b>El móvil no está entregando rumbo</b><span>Prueba desde la PWA instalada, revisa permisos de movimiento/orientación o usa un dispositivo con magnetómetro.</span>'}},2600);
 }catch(e){if(status){status.className='feature-status warning';status.innerHTML=`<b>No se pudo activar la brújula</b><span>${esc(e?.message||'Revisa permisos de movimiento y orientación del dispositivo.')}</span>`}}
}
function openCompassFeature(stage=null){
 const s=stage?STAGES[Number(stage)-1]:null;featureShell('🧭','Brújula',s?`Etapa ${stage} · ${s.from} → ${s.to}`:'Orientación dentro de BUEN CAMINO',`<section class="feature-body compass-body"><div class="compass-wrap" aria-label="Brújula"><div id="compassDial" class="compass-dial"><span class="north">N</span><span class="east">E</span><span class="south">S</span><span class="west">O</span><i class="tick t1"></i><i class="tick t2"></i><i class="tick t3"></i><i class="tick t4"></i></div><div class="compass-pointer">▲</div><div class="compass-center"></div></div><div class="compass-readout"><strong id="compassValue">—°</strong><span id="compassCardinal">—</span></div><button id="startCompassBtn" class="feature-primary" type="button">🧭 ACTIVAR BRÚJULA</button><div id="compassStatus" class="feature-status"><b>Sensor detenido</b><span>La brújula se activa solo al pulsar el botón y no guarda el rumbo.</span></div><div class="feature-safety"><b>⚠ Orientación de apoyo</b><p>Los sensores del teléfono pueden verse afectados por fundas magnéticas, vehículos, estructuras metálicas o una mala calibración. Para seguir el Camino prevalecen la señalización y el mapa/GPS.</p></div></section>`);document.getElementById('startCompassBtn').onclick=startCompass;
}
...
function renderSky(lat,lon,label){
 skyLocation={lat,lon,label};const svg=document.getElementById('skySvg'),info=document.getElementById('skyInfo'),visEl=document.getElementById('skyVisible');if(!svg)return;const now=new Date(),cx=180,cy=180,R=154,positions=new Map(),visible=[];
 for(const st of SKY_STARS){const h=skyHorizontal(st,lat,lon,now);if(h.alt<=0)continue;const rr=(90-h.alt)/90*R,a=h.az*Math.PI/180,x=cx+rr*Math.sin(a),y=cy-rr*Math.cos(a);positions.set(st.name,{x,y,...h,st});visible.push({x,y,...h,st})}
 let lines='';for(const [con,pairs] of Object.entries(SKY_LINES)){for(const [a,b] of pairs){const p1=positions.get(a),p2=positions.get(b);if(p1&&p2)lines+=`<line x1="${p1.x.toFixed(1)}" y1="${p1.y.toFixed(1)}" x2="${p2.x.toFixed(1)}" y2="${p2.y.toFixed(1)}" class="sky-line" data-con="${esc(con)}"/>`}}
 const stars=visible.sort((a,b)=>a.st.mag-b.st.mag).map(p=>{const rad=Math.max(1.7,Math.min(4.8,4.1-p.st.mag*.55)),labelStar=p.st.mag<=1.0||['Polaris','Vega','Deneb','Betelgeuse'].includes(p.st.name);return `<g><circle cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${rad.toFixed(1)}" class="sky-star"><title>${esc(p.st.name)} · ${esc(p.st.con)} · altitud ${Math.round(p.alt)}°</title></circle>${labelStar?`<text x="${(p.x+5).toFixed(1)}" y="${(p.y-4).toFixed(1)}" class="sky-star-label">${esc(p.st.name)}</text>`:''}</g>`}).join('');
 svg.innerHTML=`<circle cx="180" cy="180" r="154" class="sky-horizon"/><circle cx="180" cy="180" r="103" class="sky-alt"/><circle cx="180" cy="180" r="52" class="sky-alt"/><line x1="180" y1="26" x2="180" y2="334" class="sky-axis"/><line x1="26" y1="180" x2="334" y2="180" class="sky-axis"/><text x="180" y="18" text-anchor="middle" class="sky-dir">N</text><text x="346" y="184" text-anchor="middle" class="sky-dir">E</text><text x="180" y="354" text-anchor="middle" class="sky-dir">S</text><text x="14" y="184" text-anchor="middle" class="sky-dir">O</text>${lines}${stars}<circle cx="180" cy="180" r="3" class="sky-zenith"/>`;
 if(info)info.innerHTML=`<b>${esc(label)}</b><span>${lat.toFixed(4)}, ${lon.toFixed(4)} · ${esc(now.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}))}</span><small>Centro = cenit · borde = horizonte · N arriba</small>`;
 const cons=[...new Set(visible.map(x=>x.st.con))].filter(c=>(SKY_LINES[c]||[]).some(([a,b])=>positions.has(a)&&positions.has(b)));if(visEl)visEl.innerHTML=cons.length?`<b>Constelaciones con trazos sobre el horizonte</b><div>${cons.map(c=>`<span>${esc(c)}</span>`).join('')}</div>`:'<b>No hay suficientes estrellas de las constelaciones incluidas sobre el horizonte en este momento.</b>';
}
...
function cleanupMaps(){stopFeatureSensors();mainMap?.destroy?.();mainMap=null;focusMap?.destroy?.();focusMap=null;if(!focusOverlay.hidden)closeFocusMap();close3DMap();}
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>5. 🟡 Puntos amarillos</strong><p>Toca un punto amarillo para abrir Google Maps, Google Earth, OpenStreetMap, Fotos y Servicios cerca de ese lugar.</p></div>
 ...
 <div class="guide-card"><strong>19. 🚴 Ruta bici</strong><p>Disponible en Inicio y en las etapas. Calcula dentro de BUEN CAMINO una ruta ciclista orientativa con BRouter/OpenStreetMap. Si el cálculo externo no está disponible, muestra el trazado del Camino claramente identificado como referencia, sin afirmar que todos sus tramos sean ciclables.</p></div>
 <div class="guide-card"><strong>20. 🧭 Brújula</strong><p>Usa los sensores de orientación del móvil. Pulsa ACTIVAR BRÚJULA y concede permiso si el sistema lo solicita. El rumbo no se guarda y puede requerir calibración del teléfono.</p></div>
 <div class="guide-card"><strong>21. ✨ Estrellas y constelaciones</strong><p>Muestra una carta celeste interna calculada para una etapa o para tu posición GPS, con estrellas brillantes y trazos de constelaciones. El centro representa el cenit y el borde el horizonte.</p></div>
