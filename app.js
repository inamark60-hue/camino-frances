...
function skySolarSystem(date=new Date()){
 ...
 const mags={Mercurio:.2,Venus:-4.1,Marte:-.3,Jupiter:-2.2,Saturno:.7,Urano:5.7,Neptuno:7.8},display={Jupiter:'Júpiter'};for(const name of ['Mercurio','Venus','Marte','Jupiter','Saturno','Urano','Neptuno']){const e=skyPlanetEcliptic(name,d,sun),eq=skyEclToEq(e.x,e.y,e.z,d);bodies.push({id:'solar-'+name.toLowerCase(),name:display[name]||name,...SKY_SOLAR_INFO[name],...eq,mag:mags[name],distance:e.distance,note:`Distancia heliocéntrica aprox. ${e.distance.toFixed(2)} UA`})}return bodies;
}
...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v88')}catch(_){ }
...
function skyBuildScene(lat,lon,now){
 ...
 const stars=SKY_RICH.stars.map(conv),dsos=SKY_RICH.dsos.map(conv),constellations=SKY_RICH.constellations.map(c=>({...c,segments:c.segments.map(seg=>seg.map(conv))})),centers=SKY_RICH.centers.map(conv),solar=skySolarSystem(now).map(conv);skySceneCache={key,stars,dsos,constellations,centers,solar};return skySceneCache;
}
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v88','1')}catch{}
