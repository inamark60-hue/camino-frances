...
function storageGet(store,key,fallback=null){try{const v=store?.getItem?.(key);return v==null?fallback:v}catch(e){console.warn('Almacenamiento no disponible',key,e);return fallback}}
function storageSet(store,key,value){try{store?.setItem?.(key,value);return true}catch(e){console.warn('No se pudo guardar',key,e);return false}}
function storageJson(store,key,fallback){try{const raw=storageGet(store,key,null);if(raw==null)return fallback;const v=JSON.parse(raw);return v==null?fallback:v}catch(e){console.warn('Dato local reparado',key,e);return fallback}}
let currentView='home', currentStageNo=null, currentDetailTab='resumen', mainMap=null, focusMap=null, terrain3DMap=null, terrain3DOverlay=null, routeVisible=storageGet(localStorage,'bc-route','1')!=='0';
let featureReturn=null,compassHandler=null,compassFallbackHandler=null,compassNoDataTimer=null,compassUpdateTarget=null,compassSmoothedHeading=null,compassSource=null,compassAbsoluteSeen=false,compassCalibrationHandler=null,compassSampleBuffer=[],compassNorthLockCount=0,compassNorthLocked=false,compassLastEmit=0,skyTimer=null,skyLocation=null,bikeRouteCurrent=null,walkRouteCurrent=null,skyOrientationHandler=null,skyFallbackOrientationHandler=null,skyGpsWatch=null,skyCompassCtrl=null,skyTrackActive=false,skyOrientationState=null,skyFullOverlay=null,skyFullCompassCtrl=null,skyFullReturnScroll=0,skyNormalCompassResume=false,skyBodyOverflow='',skyCatalogPromise=null,skyCatalogDeep=false,skySceneCache=null,skyHitTargets={normal:[],full:[]},skyAutoConstellation=null,skyZoom=Math.max(1,Math.min(8,Number(storageGet(sessionStorage,'bc-sky-zoom','1'))||1)),skyDetailMode=['simple','normal','complete'].includes(storageGet(localStorage,'bc-sky-detail','normal'))?storageGet(localStorage,'bc-sky-detail','normal'):'normal',skyAimEnabled=storageGet(localStorage,'bc-sky-aim','1')!=='0',skyDwell={key:null,since:0},skyRenderRaf=0,skySearchTarget=null;
let mainMapCompass=null,focusMapCompass=null,mapCompassActive=null,mapCompassResume=null,mapCompassPosition=(()=>{try{const p=storageJson(localStorage,'bc-map-compass-pos',null);return p&&Number.isFinite(p.x)&&Number.isFinite(p.y)?{x:Math.max(0,Math.min(1,p.x)),y:Math.max(0,Math.min(1,p.y))}:{x:.82,y:.08}}catch(_){return{x:.82,y:.08}}})();
const favorites=new Set(Array.isArray(storageJson(localStorage,'bc-favs',[]))?storageJson(localStorage,'bc-favs',[]):[]);
const doneStages=new Set((Array.isArray(storageJson(localStorage,'bc-done',[]))?storageJson(localStorage,'bc-done',[]):[]).map(Number));
const likedStages=new Set((Array.isArray(storageJson(localStorage,'bc-likes',[]))?storageJson(localStorage,'bc-likes',[]):[]).map(Number));
const stageVisitCounts=(()=>{const v=storageJson(localStorage,'bc-stage-visits',{});return v&&typeof v==='object'&&!Array.isArray(v)?v:{}})();
const sessionStageViews=new Set((Array.isArray(storageJson(sessionStorage,'bc-stage-viewed',[]))?storageJson(sessionStorage,'bc-stage-viewed',[]):[]).map(Number));
let appVisitCount=Number(storageGet(localStorage,'bc-app-visits','0'))||0;
if(storageGet(sessionStorage,'bc-app-visit-counted',null)!=='1'){appVisitCount+=1;storageSet(localStorage,'bc-app-visits',String(appVisitCount));storageSet(sessionStorage,'bc-app-visit-counted','1');}
const comments=(()=>{const v=storageJson(localStorage,'bc-comments',[]);return Array.isArray(v)?v:[]})();
...
const save=()=>{storageSet(localStorage,'bc-favs',JSON.stringify([...favorites]));storageSet(localStorage,'bc-done',JSON.stringify([...doneStages]));storageSet(localStorage,'bc-likes',JSON.stringify([...likedStages]));storageSet(localStorage,'bc-stage-visits',JSON.stringify(stageVisitCounts));storageSet(localStorage,'bc-comments',JSON.stringify(comments));};
...
async function fetchCommonsNearbyPhotos(lat,lon,radius=2200){
 const api='https://commons.wikimedia.org/w/api.php?origin=*&format=json&action=query&generator=geosearch&ggsnamespace=6&ggsradius='+encodeURIComponent(radius)+'&ggslimit=40&ggscoord='+encodeURIComponent(lat+'|'+lon)+'&prop=imageinfo&iiprop=url|extmetadata|mime&iiurlwidth=720';
 const ctrl=new AbortController(),timer=setTimeout(()=>ctrl.abort(),12000);
 try{
  const r=await fetch(api,{cache:'no-store',signal:ctrl.signal});if(!r.ok)throw new Error('Commons '+r.status);
  const j=await r.json(),pages=Object.values(j?.query?.pages||{}),photos=[];
  for(const page of pages){
   const ii=page?.imageinfo?.[0],md=ii?.extmetadata||{},license=stripHtmlText(md.LicenseShortName?.value||md.UsageTerms?.value||''),mime=String(ii?.mime||'');
   if(!ii?.thumburl||!/^image\//i.test(mime)||!/(cc\s*by|cc0|public domain|pd-|gfdl|free art|fal)/i.test(license))continue;
...
async function loadNearbyPhoto(lat,lon,host){
 if(!host)return;host.innerHTML='<div class="photo-loading"><b>📷 Fotos cercanas</b><span>Buscando imágenes libres en Wikimedia Commons…</span></div>';
 try{
  let photos=await fetchCommonsNearbyPhotos(lat,lon,2400);
  if(photos.length<6){try{photos=[...photos,...await fetchCommonsNearbyPhotos(lat,lon,7500)]}catch{}}
  const seen=new Set();photos=photos.filter(p=>p?.thumb&&!seen.has(p.key)&&(seen.add(p.key),true)).slice(0,12);
  if(!photos.length)throw new Error('No hay fotos libres cercanas');
  host.innerHTML=`<div class="route-photo-scroll" role="region" tabindex="0" aria-label="Galería de fotos cercanas de Wikimedia Commons">${photos.map((p,i)=>`<figure><a href="${esc(p.page)}" target="_blank" rel="noopener"><img src="${esc(p.thumb)}" alt="Foto cercana ${i+1} del Camino" loading="lazy" referrerpolicy="no-referrer"></a><figcaption><b>${esc(p.title||'Foto cercana')}</b><span>${esc(p.artist)}</span><a href="${esc(p.licenseUrl||p.page)}" target="_blank" rel="noopener">${esc(p.license)}</a></figcaption></figure>`).join('')}</div><small class="route-photo-swipe">← Desliza con el dedo · ${photos.length} fotos →</small><small class="route-photo-license">Wikimedia Commons · solo archivos declarados con licencia libre o dominio público. Conserva la atribución indicada.</small>`;
 }catch(err){
  host.innerHTML=`<div class="photo-error"><b>📷 No se pudieron cargar las fotos ahora</b><span>${esc(err?.name==='AbortError'?'La consulta tardó demasiado. Comprueba la conexión y vuelve a intentarlo.':'Wikimedia Commons no respondió o no hay fotos libres cercanas.')}</span><div><button type="button" class="photo-retry">↻ REINTENTAR</button><a href="${commonsNearbyUrl(lat,lon)}" target="_blank" rel="noopener">ABRIR COMMONS</a></div></div>`;
  host.querySelector('.photo-retry')?.addEventListener('click',()=>loadNearbyPhoto(lat,lon,host));
 }
}
function closeRoutePointSheet(){document.getElementById('routePointSheet')?.remove()}
function openRoutePointSheet(lat,lon,stageNo=1,meta={}){
 closeRoutePointSheet();const sheet=document.createElement('div');sheet.id='routePointSheet';sheet.className='route-point-sheet';
 const title=meta.title||'Punto de ruta',detail=meta.detail||'',stage=STAGES[Math.max(0,(Number(stageNo)||1)-1)];
 sheet.innerHTML=`<div class="route-point-backdrop" data-close-point></div><section class="route-point-card" role="dialog" aria-modal="true" aria-label="Información del punto amarillo"><header><div><b>🟨 ${esc(title)}</b><small>${stage?`Etapa ${stage.n} · ${esc(stage.from)} → ${esc(stage.to)}`:''}</small></div><button type="button" data-close-point aria-label="Cerrar">×</button></header>${detail?`<div class="route-point-meta">${esc(detail)}</div>`:''}<div class="route-point-coords">🧭 ${Number(lat).toFixed(6)}, ${Number(lon).toFixed(6)}</div><div class="route-point-actions"><a href="${mapsUrl(lat,lon)}" target="_blank" rel="noopener">📍 Maps</a><a href="${earthUrl(lat,lon)}" target="_blank" rel="noopener">🌍 Earth</a><a href="${osmUrl(lat,lon)}" target="_blank" rel="noopener">🗺 OSM</a><button type="button" class="route-point-photos-btn">📷 Fotos</button><button type="button" class="route-point-nearby">📌 Servicios cerca</button></div><h3>📷 Fotos cercanas libres</h3><div class="route-photo route-point-photo"></div></section>`;
 document.body.appendChild(sheet);sheet.querySelectorAll('[data-close-point]').forEach(x=>x.addEventListener('click',closeRoutePointSheet));
 sheet.querySelector('.route-point-photos-btn')?.addEventListener('click',()=>sheet.querySelector('.route-point-photo')?.scrollIntoView({behavior:'smooth',block:'nearest'}));
 sheet.querySelector('.route-point-nearby')?.addEventListener('click',()=>{closeRoutePointSheet();openNearbyServices(lat,lon,stageNo).catch(()=>toast('No se pudieron abrir los servicios cercanos'))});
 loadNearbyPhoto(lat,lon,sheet.querySelector('.route-point-photo'));return sheet;
}
...
function stopCompassSensors(){
 ...
 clearTimeout(compassNoDataTimer);compassNoDataTimer=null;compassUpdateTarget=null;compassSmoothedHeading=null;compassSource=null;compassAbsoluteSeen=false;compassSampleBuffer=[];compassNorthLockCount=0;compassNorthLocked=false;compassLastEmit=0;
}
...
function circularMeanCompass(samples){
 if(!samples?.length)return null;let sx=0,sy=0;
 for(const v of samples){const r=v*Math.PI/180;sx+=Math.cos(r);sy+=Math.sin(r)}
 if(Math.abs(sx)<1e-8&&Math.abs(sy)<1e-8)return samples.at(-1);
 return normalizeCompassDeg(Math.atan2(sy,sx)*180/Math.PI);
}
function smoothCompassHeading(raw){
 raw=normalizeCompassDeg(raw);if(raw==null)return null;
 // Filtro circular continuo y simétrico: nunca fija la aguja al Norte.
 // El delta siempre toma el camino corto, también al cruzar 359° ↔ 0°.
 ...
 // Una zona muerta mínima elimina el ruido fino del magnetómetro sin crear histéresis.
 if(ad<0.32)return compassSmoothedHeading;
 const gain=ad<1.5?.16:ad<5?.23:ad<15?.34:ad<45?.52:.72;
...
function compassSample(raw,accuracy,source){
 ...
 if(heading==null)return;const now=performance.now();
 // Refresco a ~20 Hz: el filtro circular hace el suavizado y el cruce del Norte queda continuo.
 if(now-compassLastEmit<50)return;compassLastEmit=now;
 compassSource=source||compassSource;compassUpdateTarget(heading,Number.isFinite(Number(accuracy))?Number(accuracy):null,compassSource);
}
...
async function startCompassSensor(onUpdate,onStatus){
 stopCompassSensors();compassUpdateTarget=onUpdate;compassSmoothedHeading=null;compassAbsoluteSeen=false;compassSampleBuffer=[];compassNorthLockCount=0;compassNorthLocked=false;compassLastEmit=0;
...
function updateCompassFace(heading,accuracy=null,source=''){
 ...
 heading=normalizeCompassDeg(heading);dial.style.transform=`rotate(${-heading}deg)`;value.textContent=`${Math.round(heading)%360}°`;if(card)card.textContent=compassCardinal(heading);
 if(status){const low=Number.isFinite(accuracy)&&accuracy>25,sig=`${low}|${source}|${Number.isFinite(accuracy)?Math.round(accuracy):''}`;if(status.dataset.sig!==sig){status.dataset.sig=sig;status.className=`feature-status ${low?'warning':'ok'}`;status.innerHTML=`<b>${low?'Precisión baja':'Brújula estabilizada'}</b><span>${esc(source||'Rumbo absoluto')} · ${Number.isFinite(accuracy)?`precisión aprox. ±${Math.round(accuracy)}°. `:''}Filtro circular continuo activo.${low?' Calibra el móvil con un movimiento suave en forma de 8.':''}</span>`}}
}
...
const SKY_SOLAR_INFO={
 Sol:{symbol:'☀',color:'#ffd85a',text:'El Sol es la estrella del Sistema Solar. Su posición aparente cambia con la hora, la fecha y la ubicación del peregrino.'},
 Luna:{symbol:'☾',color:'#eef3ff',text:'La Luna es el satélite natural de la Tierra. BUEN CAMINO calcula su posición aproximada y la fracción iluminada para el momento actual.'},
 Mercurio:{symbol:'☿',color:'#c9c4b8',text:'Mercurio es el planeta más cercano al Sol. Suele observarse bajo, cerca del horizonte, poco antes del amanecer o después del atardecer.'},
 Venus:{symbol:'♀',color:'#ffe4a3',text:'Venus es el planeta más brillante visto desde la Tierra y suele destacar al amanecer o al anochecer.'},
 Marte:{symbol:'♂',color:'#ff8a63',text:'Marte es un planeta rocoso reconocible por su tono rojizo.'},
 Jupiter:{symbol:'♃',color:'#f0d3a5',text:'Júpiter es el planeta más grande del Sistema Solar y uno de los objetos planetarios más brillantes del cielo.'},
 Saturno:{symbol:'♄',color:'#f6df92',text:'Saturno es un gigante gaseoso conocido por su sistema de anillos, visible con ayuda óptica adecuada.'},
 Urano:{symbol:'♅',color:'#9fe8ec',text:'Urano es un gigante helado. A simple vista es muy difícil de distinguir y normalmente requiere buenas condiciones o ayuda óptica.'},
 Neptuno:{symbol:'♆',color:'#77a9ff',text:'Neptuno es el planeta principal más lejano del Sol. No es visible a simple vista y requiere ayuda óptica.'}
};
function skyDeg(v){return((Number(v)%360)+360)%360}
function skySin(v){return Math.sin(v*Math.PI/180)}function skyCos(v){return Math.cos(v*Math.PI/180)}
function skyAtan2(y,x){return Math.atan2(y,x)*180/Math.PI}function skyAsin(v){return Math.asin(Math.max(-1,Math.min(1,v)))*180/Math.PI}
function skyKepler(M,e){const mr=skyDeg(M)*Math.PI/180;let E=mr+e*Math.sin(mr)*(1+e*Math.cos(mr));for(let i=0;i<5;i++)E-= (E-e*Math.sin(E)-mr)/(1-e*Math.cos(E));return E}
function skyOrbitElements(name,d){
 // Los elementos orbitales originales usan las claves inglesas Mercury/Mars.
 // La interfaz trabaja en español; normalizamos aquí para que una traducción
 // nunca vuelva a detener el renderizado completo del cielo.
 const aliases={Mercurio:'Mercury',Marte:'Mars','Júpiter':'Jupiter'};
 ...
 const t={
  ...
  Jupiter:[100.4542+2.76854e-5*d,1.3030-1.557e-7*d,273.8777+1.64505e-5*d,5.20256,.048498+4.469e-9*d,19.8950+.0830853001*d],
  Saturno:[113.6634+2.38980e-5*d,2.4886-1.081e-7*d,339.3939+2.97661e-5*d,9.55475,.055546-9.499e-9*d,316.9670+.0334442282*d],
  Urano:[74.0005+1.3978e-5*d,.7733+1.9e-8*d,96.6612+3.0565e-5*d,19.18171-1.55e-8*d,.047318+7.45e-9*d,142.5905+.011725806*d],
  Neptuno:[131.7806+3.0173e-5*d,1.7700-2.55e-7*d,272.8461-6.027e-6*d,30.05826+3.313e-8*d,.008606+2.15e-9*d,260.2471+.005995147*d]
 }[name];return t?{N:t[0],i:t[1],w:t[2],a:t[3],e:t[4],M:skyDeg(t[5])}:null;
}
function skyOrbitRect(el){const E=skyKepler(el.M,el.e),xv=el.a*(Math.cos(E)-el.e),yv=el.a*Math.sqrt(1-el.e*el.e)*Math.sin(E),v=skyAtan2(yv,xv),r=Math.hypot(xv,yv),lon=skyDeg(v+el.w),x=r*(skyCos(el.N)*skyCos(lon)-skySin(el.N)*skySin(lon)*skyCos(el.i)),y=r*(skySin(el.N)*skyCos(lon)+skyCos(el.N)*skySin(lon)*skyCos(el.i)),z=r*skySin(lon)*skySin(el.i);return{x,y,z,r,lon:skyDeg(skyAtan2(y,x)),lat:skyAtan2(z,Math.hypot(x,y))}}
function skySunEcliptic(d){const w=282.9404+4.70935e-5*d,e=.016709-1.151e-9*d,M=skyDeg(356.0470+.9856002585*d),E=skyKepler(M,e),xv=Math.cos(E)-e,yv=Math.sqrt(1-e*e)*Math.sin(E),v=skyAtan2(yv,xv),r=Math.hypot(xv,yv),lon=skyDeg(v+w);return{x:r*skyCos(lon),y:r*skySin(lon),z:0,r,lon,M,w}}
function skyEclToEq(x,y,z,d){const ob=23.4393-3.563e-7*d,xe=x,ye=y*skyCos(ob)-z*skySin(ob),ze=y*skySin(ob)+z*skyCos(ob),ra=skyDeg(skyAtan2(ye,xe))/15,dec=skyAtan2(ze,Math.hypot(xe,ye));return{ra,dec}}
function skyPlanetEcliptic(name,d,sun){const el=skyOrbitElements(name,d),r0=skyOrbitRect(el);let lon=r0.lon,lat=r0.lat;if(name==='Jupiter'||name==='Saturno'||name==='Urano'){const Mj=skyOrbitElements('Jupiter',d).M,Ms=skyOrbitElements('Saturno',d).M,Mu=skyOrbitElements('Urano',d).M;if(name==='Jupiter')lon+=-.332*skySin(2*Mj-5*Ms-67.6)-.056*skySin(2*Mj-2*Ms+21)+.042*skySin(3*Mj-5*Ms+21)-.036*skySin(Mj-2*Ms)+.022*skyCos(Mj-Ms)+.023*skySin(2*Mj-3*Ms+52)-.016*skySin(Mj-5*Ms-69);if(name==='Saturno'){lon+=.812*skySin(2*Mj-5*Ms-67.6)-.229*skyCos(2*Mj-4*Ms-2)+.119*skySin(Mj-2*Ms-3)+.046*skySin(2*Mj-6*Ms-69)+.014*skySin(Mj-3*Ms+32);lat+=-.020*skyCos(2*Mj-4*Ms-2)+.018*skySin(2*Mj-6*Ms-49)}if(name==='Urano')lon+=.040*skySin(Ms-2*Mu+6)+.035*skySin(Ms-3*Mu+33)-.015*skySin(Mj-Mu+20)}lon=skyDeg(lon);const xh=r0.r*skyCos(lon)*skyCos(lat),yh=r0.r*skySin(lon)*skyCos(lat),zh=r0.r*skySin(lat);return{x:xh+sun.x,y:yh+sun.y,z:zh,distance:r0.r}}
function skyMoonEcliptic(d,sun){const N=skyDeg(125.1228-.0529538083*d),i=5.1454,w=skyDeg(318.0634+.1643573223*d),a=60.2666,e=.0549,M=skyDeg(115.3654+13.0649929509*d),r0=skyOrbitRect({N,i,w,a,e,M});let lon=r0.lon,lat=r0.lat,Lm=skyDeg(N+w+M),Ls=skyDeg(sun.w+sun.M),D=skyDeg(Lm-Ls),F=skyDeg(Lm-N);lon+=-1.274*skySin(M-2*D)+.658*skySin(2*D)-.186*skySin(sun.M)-.059*skySin(2*M-2*D)-.057*skySin(M-2*D+sun.M)+.053*skySin(M+2*D)+.046*skySin(2*D-sun.M)+.041*skySin(M-sun.M)-.035*skySin(D)-.031*skySin(M+sun.M)-.015*skySin(2*F-2*D)+.011*skySin(M-4*D);lat+=-.173*skySin(F-2*D)-.055*skySin(M-F-2*D)-.046*skySin(M+F-2*D)+.033*skySin(F+2*D)+.017*skySin(2*M+F);lon=skyDeg(lon);return{x:r0.r*skyCos(lon)*skyCos(lat),y:r0.r*skySin(lon)*skyCos(lat),z:r0.r*skySin(lat),lon,lat,distance:r0.r}}
function skyAngularSep(a,b){const r=Math.PI/180,d1=a.dec*r,d2=b.dec*r,ra=(a.ra-b.ra)*15*r;return Math.acos(Math.max(-1,Math.min(1,Math.sin(d1)*Math.sin(d2)+Math.cos(d1)*Math.cos(d2)*Math.cos(ra))))/r}
function skySolarSystem(date=new Date()){
 const d=julianDate(date)-2451543.5,sun=skySunEcliptic(d),sunEq=skyEclToEq(sun.x,sun.y,0,d),bodies=[];
 bodies.push({id:'solar-sun',name:'Sol',...SKY_SOLAR_INFO.Sol,...sunEq,mag:-26.74,distance:sun.r,note:`Distancia aprox. ${sun.r.toFixed(3)} UA`});
 const moonE=skyMoonEcliptic(d,sun),moonEq=skyEclToEq(moonE.x,moonE.y,moonE.z,d),elong=skyAngularSep(sunEq,moonEq),illum=(1-Math.cos(elong*Math.PI/180))/2;bodies.push({id:'solar-moon',name:'Luna',...SKY_SOLAR_INFO.Luna,...moonEq,mag:-12.7,distance:moonE.distance*6378.14,phase:illum,note:`Iluminación aprox. ${Math.round(illum*100)}%`});
 const mags={Mercurio:.2,Venus:-4.1,Marte:-.3,Jupiter:-2.2,Saturno:.7,Urano:5.7,Neptuno:7.8},display={Jupiter:'Júpiter'};for(const name of ['Mercurio','Venus','Marte','Jupiter','Saturno','Urano','Neptuno']){try{const e=skyPlanetEcliptic(name,d,sun);if(!e||![e.x,e.y,e.z].every(Number.isFinite))continue;const eq=skyEclToEq(e.x,e.y,e.z,d);if(!Number.isFinite(eq.ra)||!Number.isFinite(eq.dec))continue;bodies.push({id:'solar-'+name.toLowerCase(),name:display[name]||name,...SKY_SOLAR_INFO[name],...eq,mag:mags[name],distance:e.distance,note:`Distancia heliocéntrica aprox. ${e.distance.toFixed(2)} UA`})}catch(err){console.warn('Cielo: planeta omitido',name,err)}}return bodies;
}
...
function skyScreenAngle(){
 let a=Number(screen.orientation?.angle);
 if(!Number.isFinite(a)){
  const legacy=Number(window.orientation);
  // window.orientation usa signo contrario en navegadores antiguos.
  a=Number.isFinite(legacy)?-legacy:0;
 }
 return normalizeCompassDeg(a)||0;
}
function mix3(a,b,wa,wb){return[a[0]*wa+b[0]*wb,a[1]*wa+b[1]*wb,a[2]*wa+b[2]*wb]}
function norm3(v){const n=Math.hypot(v[0],v[1],v[2])||1;return[v[0]/n,v[1]/n,v[2]/n]}
function deviceSkyBasis(alpha,beta,gamma){
 // Matriz Z-X'-Y'' de la especificación W3C. El marco terrestre usado aquí es
 // X=Este, Y=Norte, Z=arriba. NO se suma el ángulo de pantalla a alpha: eso
 // giraba también la dirección de la cámara y era la causa del visor incorrecto.
 const r=Math.PI/180,a=Number(alpha)*r,b=Number(beta)*r,g=Number(gamma)*r,ca=Math.cos(a),sa=Math.sin(a),cb=Math.cos(b),sb=Math.sin(b),cg=Math.cos(g),sg=Math.sin(g);
 const dx=[ca*cg-sa*sb*sg,cg*sa+ca*sb*sg,-cb*sg];
 const dy=[-cb*sa,ca*cb,sb];
 const dz=[cg*sa*sb+ca*sg,sa*sg-ca*cg*sb,cb*cg];
 // La cámara trasera mira en -Z del dispositivo. El giro de pantalla solo debe
 // cambiar qué vector queda a la derecha/arriba de la imagen, no el rumbo físico.
 const q=skyScreenAngle()*r,c=Math.cos(q),sn=Math.sin(q);
 const right=norm3(mix3(dx,dy,c,sn));
 const up=norm3(mix3(dx,dy,-sn,c));
 const forward=norm3([-dz[0],-dz[1],-dz[2]]);
 return{right,up,forward};
}
function skyPointingDirection(basis){
 if(!basis?.forward)return null;const [east,north,up]=basis.forward;
 const az=normalizeCompassDeg(Math.atan2(east,north)*180/Math.PI),alt=Math.asin(Math.max(-1,Math.min(1,up)))*180/Math.PI;
 return{az,alt,card:compassCardinal(az)};
}
...
function renderSky(lat,lon,label){
 skyLocation={lat,lon,label};const info=document.getElementById('skyInfo'),visEl=document.getElementById('skyVisible'),now=new Date(),sensor=skyTrackActive&&skyOrientationState;let visible=[],pointing=null;
 if(sensor){const b=deviceSkyBasis(skyOrientationState.alpha,skyOrientationState.beta,skyOrientationState.gamma);pointing=skyPointingDirection(b)}
 ...
 const aim=pointing?`APUNTANDO ${Math.round(pointing.az)}° ${pointing.card} · ALT ${pointing.alt>=0?'+':''}${Math.round(pointing.alt)}°`:'';
 if(info)info.innerHTML=`<b>${esc(label)}</b><span>${lat.toFixed(4)}, ${lon.toFixed(4)} · ${esc(now.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'}))}</span><small>${sensor?`${esc(aim)} · mira = dirección de la parte trasera del móvil`:'Centro = cenit · borde = horizonte · toca ACTIVAR MODO CIELO para seguir el movimiento'}</small>`;
 const fullInfo=document.getElementById('skyFullStatus');if(fullInfo)fullInfo.textContent=pointing?`${aim} · ${now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`:`${label} · Vista celeste · ${now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`;
...
function skyOrientationEvent(e,source){
 ...
 skyOrientationState={alpha,beta,gamma};if(skyLocation&&(document.getElementById('skyCanvas')||document.getElementById('skyFullCanvas')))
...
async function startSkyTracking(){
 const status=document.getElementById('skyStatus'),btn=document.getElementById('skyTrackBtn');try{await requestCompassPermission();skyTrackActive=true;if(btn){btn.textContent='✓ MODO CIELO ACTIVO';btn.classList.add('active')}
  ...
  if(skyCompassCtrl&&!skyCompassCtrl.active)activateSkyPolarCompass(skyCompassCtrl);if(status){status.className='feature-status ok';status.innerHTML='<b>✓ Modo cielo activo</b><span>Apunta la parte trasera del teléfono hacia el cielo y muévelo despacio. La marca discreta de MIRA representa la dirección real a la que apunta el móvil; azimut e inclinación se calculan con los sensores.</span>'}
...
async function commonsConstellationPhotos(name){
 const info=CONSTELLATION_INFO[name]||{latin:(skyFindConst(name)?.latin||name)},q=`${info.latin} constellation night sky photograph`,u='https://commons.wikimedia.org/w/api.php?origin=*&format=json&action=query&generator=search&gsrnamespace=6&gsrlimit=8&gsrsearch='+encodeURIComponent(q)+'&prop=imageinfo&iiprop=url|extmetadata&iiurlwidth=640';
...
/* ---------- V8.7 · Motor de cielo profesional + detalle + zoom telescopio ---------- */
const SKY_REMOTE_BASES=[
 'https://cdn.jsdelivr.net/gh/dieghernan/celestial_data@main/data/',
 'https://dieghernan.github.io/celestial_data/data/',
 'https://raw.githubusercontent.com/dieghernan/celestial_data/main/data/'
];
const SKY_CONST_NAMES={
 And:['Andrómeda','Andromeda'],Ant:['Máquina Neumática','Antlia'],Aps:['Ave del Paraíso','Apus'],Aqr:['Acuario','Aquarius'],Aql:['Águila','Aquila'],Ara:['Altar','Ara'],Ari:['Aries','Aries'],Aur:['Auriga','Auriga'],Boo:['Boyero','Boötes'],Cae:['Cincel','Caelum'],Cam:['Jirafa','Camelopardalis'],Cnc:['Cáncer','Cancer'],CVn:['Perros de Caza','Canes Venatici'],CMa:['Can Mayor','Canis Major'],CMi:['Can Menor','Canis Minor'],Cap:['Capricornio','Capricornus'],Car:['Quilla','Carina'],Cas:['Casiopea','Cassiopeia'],Cen:['Centauro','Centaurus'],Cep:['Cefeo','Cepheus'],Cet:['Ballena','Cetus'],Cha:['Camaleón','Chamaeleon'],Cir:['Compás','Circinus'],Col:['Paloma','Columba'],Com:['Cabellera de Berenice','Coma Berenices'],CrA:['Corona Austral','Corona Australis'],CrB:['Corona Boreal','Corona Borealis'],Crv:['Cuervo','Corvus'],Crt:['Copa','Crater'],Cru:['Cruz del Sur','Crux'],Cyg:['Cisne','Cygnus'],Del:['Delfín','Delphinus'],Dor:['Dorado','Dorado'],Dra:['Dragón','Draco'],Equ:['Caballito','Equuleus'],Eri:['Erídano','Eridanus'],For:['Horno','Fornax'],Gem:['Géminis','Gemini'],Gru:['Grulla','Grus'],Her:['Hércules','Hercules'],Hor:['Reloj','Horologium'],Hya:['Hidra','Hydra'],Hyi:['Hidra Austral','Hydrus'],Ind:['Indio','Indus'],Lac:['Lagarto','Lacerta'],Leo:['Leo','Leo'],LMi:['León Menor','Leo Minor'],Lep:['Liebre','Lepus'],Lib:['Libra','Libra'],Lup:['Lobo','Lupus'],Lyn:['Lince','Lynx'],Lyr:['Lira','Lyra'],Men:['Mesa','Mensa'],Mic:['Microscopio','Microscopium'],Mon:['Unicornio','Monoceros'],Mus:['Mosca','Musca'],Nor:['Escuadra','Norma'],Oct:['Octante','Octans'],Oph:['Ofiuco','Ophiuchus'],Ori:['Orión','Orion'],Pav:['Pavo','Pavo'],Peg:['Pegaso','Pegasus'],Per:['Perseo','Perseus'],Phe:['Fénix','Phoenix'],Pic:['Pintor','Pictor'],Psc:['Piscis','Pisces'],PsA:['Pez Austral','Piscis Austrinus'],Pup:['Popa','Puppis'],Pyx:['Brújula','Pyxis'],Ret:['Retículo','Reticulum'],Sge:['Flecha','Sagitta'],Sgr:['Sagitario','Sagittarius'],Sco:['Escorpio','Scorpius'],Scl:['Escultor','Sculptor'],Sct:['Escudo','Scutum'],Ser:['Serpiente','Serpens'],Sex:['Sextante','Sextans'],Tau:['Tauro','Taurus'],Tel:['Telescopio','Telescopium'],TrA:['Triángulo Austral','Triangulum Australe'],Tri:['Triángulo','Triangulum'],Tuc:['Tucán','Tucana'],UMa:['Osa Mayor','Ursa Major'],UMi:['Osa Menor','Ursa Minor'],Vel:['Vela','Vela'],Vir:['Virgo','Virgo'],Vol:['Pez Volador','Volans'],Vul:['Zorra','Vulpecula']
};
const SKY_TYPE_NAMES={g:'Galaxia',s:'Galaxia espiral',s0:'Galaxia lenticular',sd:'Galaxia enana',e:'Galaxia elíptica',i:'Galaxia irregular',gg:'Cúmulo de galaxias',oc:'Cúmulo abierto',gc:'Cúmulo globular',en:'Nebulosa de emisión',bn:'Nebulosa',rn:'Nebulosa de reflexión',pn:'Nebulosa planetaria',snr:'Remanente de supernova',sfr:'Región de formación estelar',dn:'Nebulosa oscura',pos:'Objeto celeste'};
let SKY_RICH={stars:SKY_STARS.map((s,i)=>({...s,id:`fallback-${i}`,display:s.name,bv:null})),constellations:[],centers:[],dsos:[],source:'Catálogo básico integrado',deep:false};
function skyConstMeta(abbr){const p=SKY_CONST_NAMES[String(abbr||'').trim()]||[String(abbr||'Constelación'),String(abbr||'')];return{name:p[0],latin:p[1],abbr:String(abbr||'')}}
function skyCoordToEq(coord){const lon=Number(coord?.[0]),dec=Number(coord?.[1]);if(!Number.isFinite(lon)||!Number.isFinite(dec))return null;const deg=lon<0?lon+360:lon;return{ra:deg/15,dec}}
function skyFallbackConstellations(){const byName=new Map(SKY_STARS.map(s=>[s.name,s]));return Object.entries(SKY_LINES).map(([name,pairs])=>{const info=CONSTELLATION_INFO[name]||{},segments=pairs.map(pair=>pair.map(n=>byName.get(n)).filter(Boolean).map(s=>({ra:s.ra,dec:s.dec}))).filter(s=>s.length>1);return{abbr:info.abbr||name,name,latin:info.latin||name,segments}})}
SKY_RICH.constellations=skyFallbackConstellations();
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v100')}catch(_){ }
 for(const base of SKY_REMOTE_BASES){const url=base+file;try{let r=cache?await cache.match(url):null;if(!r||!r.ok){r=await fetch(url,{mode:'cors',cache:'force-cache'});if(r.ok&&cache)cache.put(url,r.clone()).catch(()=>{})}if(r&&r.ok)return await r.json()}catch(_){}}
 throw new Error('Catálogo celeste no disponible');
}
function skyParseStars(j){return (j?.features||[]).map((f,i)=>{const eq=skyCoordToEq(f.geometry?.coordinates),p=f.properties||{};if(!eq)return null;const mag=Number(p.mag);return{id:String(f.id??p.id??`s${i}`),name:String(p.name||''),display:String(p.name||p.desig||f.id||''),ra:eq.ra,dec:eq.dec,mag:Number.isFinite(mag)?mag:6.5,bv:Number.isFinite(Number(p.bv))?Number(p.bv):null,con:String(p.con||p.constellation||p.const||'')}}).filter(Boolean)}
function skyParseConstellations(j){return (j?.features||[]).map(f=>{const abbr=String(f.id||f.properties?.id||f.properties?.abbr||''),m=skyConstMeta(abbr),g=f.geometry||{},raw=g.type==='LineString'?[g.coordinates]:(g.type==='MultiLineString'?g.coordinates:[]),segments=raw.map(seg=>seg.map(skyCoordToEq).filter(Boolean)).filter(seg=>seg.length>1);return{...m,segments}}).filter(c=>c.segments.length)}
function skyParseCenters(j){return (j?.features||[]).map(f=>{const eq=skyCoordToEq(f.geometry?.coordinates);if(!eq)return null;return{...skyConstMeta(f.id||f.properties?.id||f.properties?.abbr||''),...eq}}).filter(Boolean)}
function skyParseDsos(j){return (j?.features||[]).map((f,i)=>{const eq=skyCoordToEq(f.geometry?.coordinates),p=f.properties||{};if(!eq)return null;const mag=Number(p.mag);return{id:String(f.id||p.desig||`d${i}`),name:String(p.name||p.desig||f.id||`Objeto ${i+1}`),type:String(p.type||'pos').toLowerCase(),mag:Number.isFinite(mag)?mag:99,dim:String(p.dim||''),morph:String(p.morph||''),ra:eq.ra,dec:eq.dec}}).filter(Boolean)}
async function loadSkyCatalog(){
 if(SKY_RICH.deep)return SKY_RICH;if(skyCatalogPromise)return skyCatalogPromise;
 skyCatalogPromise=(async()=>{try{const [stars,lines,centers,dsos]=await Promise.all([skyFetchDataFile('stars.6.min.geojson'),skyFetchDataFile('constellations.lines.min.geojson'),skyFetchDataFile('constellations.min.geojson'),skyFetchDataFile('messier.min.geojson')]);const parsedStars=skyParseStars(stars),parsedLines=skyParseConstellations(lines),parsedCenters=skyParseCenters(centers),parsedDsos=skyParseDsos(dsos);if(parsedStars.length<1000||parsedLines.length<50)throw new Error('Catálogo incompleto');SKY_RICH={stars:parsedStars,constellations:parsedLines,centers:parsedCenters,dsos:parsedDsos,source:'Celestial Data · d3-celestial · J2000',deep:true};skyCatalogDeep=true;skySceneCache=null;const st=document.getElementById('skyStatus');if(st?.isConnected){st.className='feature-status ok';st.innerHTML=`<b>✓ Catálogo celeste completo</b><span>${parsedStars.length.toLocaleString('es-ES')} estrellas · ${parsedLines.length} constelaciones · ${parsedDsos.length} objetos Messier. Datos abiertos cargados y guardados para reutilización.</span>`}if(skyLocation)renderSky(skyLocation.lat,skyLocation.lon,skyLocation.label);return SKY_RICH}catch(e){console.warn('Cielo: catálogo ampliado no disponible',e);return SKY_RICH}finally{skyCatalogPromise=null}})();return skyCatalogPromise;
}
function skyEqHorizontal(obj,lat,lon,date){return skyHorizontal(obj,lat,lon,date)}
function skyBuildScene(lat,lon,now){
 const minute=Math.floor(now.getTime()/60000),key=`${lat.toFixed(4)}|${lon.toFixed(4)}|${minute}|${SKY_RICH.deep?'d':'b'}`;if(skySceneCache?.key===key)return skySceneCache;
 const conv=o=>{const h=skyEqHorizontal(o,lat,lon,now);return{...o,alt:h.alt,az:h.az,v:skyWorldVector(h)}};
 const stars=SKY_RICH.stars.map(conv),dsos=SKY_RICH.dsos.map(conv),constellations=SKY_RICH.constellations.map(c=>({...c,segments:c.segments.map(seg=>seg.map(conv))})),centers=SKY_RICH.centers.map(conv);let solar=[];try{solar=skySolarSystem(now).map(conv).filter(o=>Number.isFinite(o.alt)&&Number.isFinite(o.az)&&Array.isArray(o.v)&&o.v.every(Number.isFinite))}catch(err){console.warn('Cielo: capa Sistema Solar omitida sin detener la carta',err)}skySceneCache={key,stars,dsos,constellations,centers,solar};return skySceneCache;
}
function skyZoomLabel(){const rounded=Math.round(skyZoom*10)/10;return `🔭 ×${Number.isInteger(rounded)?rounded:rounded.toFixed(1)}`}
function skyApproxFov(){return Math.round(2*Math.atan(.5/(.58*skyZoom))*180/Math.PI)}
function skyScheduleRender(){if(skyRenderRaf)return;skyRenderRaf=requestAnimationFrame(()=>{skyRenderRaf=0;if(skyLocation&&(document.getElementById('skyCanvas')||document.getElementById('skyFullCanvas')))renderSky(skyLocation.lat,skyLocation.lon,skyLocation.label)})}
function skyUpdateControls(){document.querySelectorAll('[data-sky-zoom-readout]').forEach(el=>el.textContent=`${skyZoomLabel()} · ~${skyApproxFov()}°`);document.querySelectorAll('[data-sky-detail]').forEach(el=>{el.textContent=`⭐ ${skyDetailMode==='simple'?'SIMPLE':skyDetailMode==='complete'?'COMPLETO':'NORMAL'}`;el.dataset.mode=skyDetailMode});document.querySelectorAll('[data-sky-aim]').forEach(el=>{el.textContent=`👁 MIRA ${skyAimEnabled?'ON':'OFF'}`;el.dataset.mode=skyAimEnabled?'on':'off';el.setAttribute('aria-pressed',skyAimEnabled?'true':'false')});document.querySelectorAll('[data-sky-search]').forEach(el=>{const n=skySearchTarget?.name||'';el.textContent=n?`🔎 ${n.length>12?n.slice(0,11)+'…':n}`:'🔎 BUSCAR';el.classList.toggle('active',!!n);el.setAttribute('aria-label',n?`Buscar otro objeto. Objetivo actual: ${n}`:'Buscar planeta, estrella o constelación')})}
function skySetZoom(value){skyZoom=Math.max(1,Math.min(8,Number(value)||1));storageSet(sessionStorage,'bc-sky-zoom',String(skyZoom));skyUpdateControls();skyScheduleRender()}
function skyZoomStep(dir){const levels=[1,2,4,8],cur=skyZoom;if(dir>0){skySetZoom(levels.find(v=>v>cur+.05)||8)}else{skySetZoom([...levels].reverse().find(v=>v<cur-.05)||1)}}
function skyCycleDetail(){const modes=['simple','normal','complete'],i=modes.indexOf(skyDetailMode);skyDetailMode=modes[(i+1)%modes.length];storageSet(localStorage,'bc-sky-detail',skyDetailMode);skyUpdateControls();skyScheduleRender()}
function skyToggleAim(){skyAimEnabled=!skyAimEnabled;storageSet(localStorage,'bc-sky-aim',skyAimEnabled?'1':'0');if(!skyAimEnabled){skyAutoConstellation=null;skyDwell={key:null,since:0}}skyUpdateControls();skyScheduleRender()}
function skySearchNorm(v){return String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim()}
function skySearchItems(){
 const out=[],seen=new Set(),push=x=>{const id=`${x.kind}|${x.key}`;if(!x.name||seen.has(id))return;seen.add(id);out.push(x)};
 for(const b of skySolarSystem(new Date()))push({kind:'solar',key:b.id,name:b.name,sub:'Sistema Solar',icon:b.symbol||'●',rank:0});
 for(const c of SKY_RICH.constellations)push({kind:'constellation',key:c.abbr,name:c.name,sub:`Constelación · ${c.latin||c.abbr}`,icon:'✨',rank:1});
 for(const st of SKY_RICH.stars){const n=String(st.name||'').trim();if(!n)continue;push({kind:'star',key:st.id,name:n,sub:`Estrella${st.con?` · ${skyConstMeta(st.con).name}`:''} · mag ${Number(st.mag).toFixed(1)}`,icon:'⭐',rank:2,mag:Number(st.mag)})}
 return out;
}
function skyClearSearch(){skySearchTarget=null;skyUpdateControls();skyScheduleRender();document.getElementById('skySearchOverlay')?.remove();toast('🔎 Búsqueda celeste desactivada')}
function openSkySearch(source='normal'){
 document.getElementById('skySearchOverlay')?.remove();
 const overlay=document.createElement('section');overlay.id='skySearchOverlay';overlay.className='sky-search-overlay';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.setAttribute('aria-label','Buscar objeto en el cielo');
 overlay.innerHTML=`<div class="sky-search-card"><div class="sky-search-head"><div><small>BUSCADOR CELESTE</small><h3>🔎 Planetas · estrellas · constelaciones</h3></div><button id="skySearchClose" type="button" aria-label="Cerrar">×</button></div><label for="skySearchInput">¿Qué quieres encontrar?</label><input id="skySearchInput" type="search" inputmode="search" autocomplete="off" placeholder="Ej.: Júpiter, Polaris, Orión…"><div id="skySearchResults" class="sky-search-results"></div>${skySearchTarget?`<button id="skySearchClear" class="sky-search-clear" type="button">✕ Quitar objetivo · ${esc(skySearchTarget.name)}</button>`:''}<small class="sky-search-help">Selecciona un objeto. En MODO CIELO aparecerá una flecha que indica izquierda/derecha y arriba/abajo hasta encontrarlo.</small></div>`;
 document.body.appendChild(overlay);const input=overlay.querySelector('#skySearchInput'),host=overlay.querySelector('#skySearchResults');
 const draw=()=>{const q=skySearchNorm(input.value),all=skySearchItems();let arr;if(q){arr=all.filter(x=>skySearchNorm(`${x.name} ${x.sub}`).includes(q)).sort((a,b)=>a.rank-b.rank||(a.mag??99)-(b.mag??99)||a.name.localeCompare(b.name,'es')).slice(0,60)}else{const wanted=['Sol','Luna','Mercurio','Venus','Marte','Júpiter','Saturno','Urano','Neptuno','Polaris','Sirius','Vega','Arcturus','Betelgeuse'];arr=all.filter(x=>x.rank===0||wanted.includes(x.name)||(x.rank===1&&['Orión','Osa Mayor','Casiopea','Leo','Sagitario'].includes(x.name))).slice(0,35)}host.innerHTML=arr.length?arr.map(x=>`<button type="button" data-kind="${esc(x.kind)}" data-key="${esc(x.key)}" data-name="${esc(x.name)}"><span>${esc(x.icon)} <b>${esc(x.name)}</b></span><small>${esc(x.sub)}</small></button>`).join(''):'<div class="empty">No se encontró ese nombre en el catálogo cargado.</div>';host.querySelectorAll('button[data-kind]').forEach(b=>b.onclick=()=>{skySearchTarget={kind:b.dataset.kind,key:b.dataset.key,name:b.dataset.name};overlay.remove();skyUpdateControls();skyScheduleRender();toast(`🔎 Objetivo: ${skySearchTarget.name}${skyTrackActive?'':' · activa MODO CIELO para seguir la flecha'}`)})};
 input.addEventListener('input',draw);overlay.querySelector('#skySearchClose').onclick=()=>overlay.remove();overlay.addEventListener('click',e=>{if(e.target===overlay)overlay.remove()});overlay.querySelector('#skySearchClear')?.addEventListener('click',skyClearSearch);draw();setTimeout(()=>input.focus(),40);
}
function skyVectorHorizontal(v){const n=norm3(v),alt=skyAsin(n[2]),az=skyDeg(skyAtan2(n[0],n[1]));return{v:n,alt,az}}
function skyResolveSearchTarget(scene){if(!skySearchTarget)return null;const t=skySearchTarget;if(t.kind==='solar'){const o=scene.solar.find(x=>x.id===t.key||x.name===t.name);return o?{...o,searchName:t.name}:null}if(t.kind==='star'){const o=scene.stars.find(x=>x.id===t.key||x.name===t.name);return o?{...o,searchName:t.name}:null}if(t.kind==='constellation'){const c=scene.centers.find(x=>x.abbr===t.key||x.name===t.name);if(c)return{...c,searchName:t.name};const line=scene.constellations.find(x=>x.abbr===t.key||x.name===t.name);if(!line)return null;let sum=[0,0,0],n=0;for(const seg of line.segments)for(const p of seg){sum[0]+=p.v[0];sum[1]+=p.v[1];sum[2]+=p.v[2];n++}if(!n)return null;return{...skyVectorHorizontal(sum),searchName:t.name,abbr:line.abbr,name:line.name}}return null}
function skySignedDelta(a,b){return((a-b+540)%360)-180}
function skyDrawSearchGuide(ctx,g,target,pointing,project,basis){if(!target)return;const p=project(target),name=target.searchName||target.name||'Objetivo';ctx.save();if(!pointing){if(p){ctx.strokeStyle='#ffd400';ctx.lineWidth=2.5;ctx.shadowColor='#ffd400';ctx.shadowBlur=10;ctx.beginPath();ctx.arc(p.x,p.y,14,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='rgba(3,13,27,.84)';ctx.font='900 11px system-ui';const tw=ctx.measureText(`🔎 ${name}`).width;ctx.fillRect(Math.max(4,p.x-tw/2-7),Math.max(4,p.y-36),tw+14,20);ctx.fillStyle='#ffe15b';ctx.textAlign='center';ctx.fillText(`🔎 ${name}`,p.x,Math.max(18,p.y-22))}ctx.restore();return}
 const hd=skySignedDelta(target.az,pointing.az),vd=target.alt-pointing.alt,sep=Math.acos(Math.max(-1,Math.min(1,dot3(norm3(target.v),norm3(skyWorldVector(pointing))))))*180/Math.PI,found=sep<=4.5;
 if(found){const tx=p?.x??g.cx,ty=p?.y??g.cy;ctx.strokeStyle='#ffd400';ctx.lineWidth=3;ctx.shadowColor='#ffd400';ctx.shadowBlur=15;ctx.beginPath();ctx.arc(tx,ty,16,0,Math.PI*2);ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='rgba(3,13,27,.88)';ctx.font='900 12px system-ui';ctx.textAlign='center';ctx.fillRect(g.cx-90,g.cy+48,180,25);ctx.fillStyle='#ffe15b';ctx.fillText(`✓ ${name} ENCONTRADO`,g.cx,g.cy+65);ctx.restore();return}
 let dx,dy;if(p&&Number.isFinite(p.x)&&Number.isFinite(p.y)){dx=p.x-g.cx;dy=p.y-g.cy}else if(basis){dx=dot3(target.v,basis.right);dy=-dot3(target.v,basis.up)}else{dx=hd;dy=-vd}let len=Math.hypot(dx,dy)||1,ux=dx/len,uy=dy/len;const R=Math.min(88,g.m*.22),ax=g.cx+ux*R,ay=g.cy+uy*R,perpX=-uy,perpY=ux;ctx.fillStyle='#ffd400';ctx.shadowColor='#ffd400';ctx.shadowBlur=10;ctx.beginPath();ctx.moveTo(ax+ux*15,ay+uy*15);ctx.lineTo(ax-ux*9+perpX*8,ay-uy*9+perpY*8);ctx.lineTo(ax-ux*9-perpX*8,ay-uy*9-perpY*8);ctx.closePath();ctx.fill();ctx.shadowBlur=0;const htxt=Math.abs(hd)<3?'rumbo OK':`${Math.round(Math.abs(hd))}° ${hd>0?'derecha':'izquierda'}`,vtxt=Math.abs(vd)<3?'altura OK':`${Math.round(Math.abs(vd))}° ${vd>0?'arriba':'abajo'}`;const txt=target.alt<0?`🔎 ${name} · bajo el horizonte · ${htxt} · ${vtxt}`:`🔎 ${name} · ${htxt} · ${vtxt}`;ctx.font='900 10px system-ui';const tw=Math.min(g.w-24,ctx.measureText(txt).width+18);ctx.fillStyle='rgba(3,13,27,.88)';ctx.fillRect((g.w-tw)/2,g.h-42,tw,25);ctx.fillStyle='#ffe15b';ctx.textAlign='center';ctx.fillText(txt,g.cx,g.h-25);ctx.restore()}
function skyDetailConfig(){return skyDetailMode==='simple'?{starMag:4.5,constAlpha:.20,dso:false,dsoMag:0,labelRadius:70}:skyDetailMode==='complete'?{starMag:6.5,constAlpha:.48,dso:true,dsoMag:99,labelRadius:220}:{starMag:5.6,constAlpha:.32,dso:true,dsoMag:8.0,labelRadius:115}}
function skyCanvasGeom(canvas){const r=canvas.getBoundingClientRect(),w=Math.max(300,Math.round(r.width||360)),h=Math.max(300,Math.round(r.height||520)),dpr=Math.min(2,window.devicePixelRatio||1);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr)}const ctx=canvas.getContext('2d');ctx.setTransform(dpr,0,0,dpr,0,0);return{ctx,w,h,cx:w/2,cy:h/2,m:Math.min(w,h),zoom:skyZoom,f:Math.min(w,h)*.58*skyZoom}}
function skyProjectVector(v,basis,g){if(!basis)return null;const x=dot3(v,basis.right),y=dot3(v,basis.up),z=dot3(v,basis.forward);if(z<=.10)return null;const p={x:g.cx+g.f*x/z,y:g.cy-g.f*y/z,z};if(p.x<-50||p.x>g.w+50||p.y<-50||p.y>g.h+50)return null;return p}
function skyProjectStatic(o,g){if(o.alt<=0)return null;const R=g.m*.46*g.zoom,rr=(90-o.alt)/90*R,a=o.az*Math.PI/180;const p={x:g.cx+rr*Math.sin(a),y:g.cy-rr*Math.cos(a),z:1};if(p.x<-50||p.x>g.w+50||p.y<-50||p.y>g.h+50)return null;return p}
function skyStarColor(bv){if(!Number.isFinite(bv))return'#eef6ff';if(bv<-.15)return'#a9c8ff';if(bv<.35)return'#dbe9ff';if(bv<.8)return'#fffdf0';if(bv<1.25)return'#ffe6a0';return'#ffc47a'}
function skyStarRadius(mag){return Math.max(.55,Math.min(4.8,3.7*Math.pow(10,-.12*(Number(mag)||0))))}
function skyLineDistance(px,py,a,b){const dx=b.x-a.x,dy=b.y-a.y,l=dx*dx+dy*dy;if(!l)return Math.hypot(px-a.x,py-a.y);const t=Math.max(0,Math.min(1,((px-a.x)*dx+(py-a.y)*dy)/l));return Math.hypot(px-(a.x+t*dx),py-(a.y+t*dy))}
function skyDsoStyle(type){if(['g','s','s0','sd','e','i','gg'].includes(type))return{color:'#b8a6ff',shape:'ellipse',icon:'🌌'};if(['en','bn','rn','pn','snr','sfr','dn'].includes(type))return{color:'#65d7ff',shape:'diamond',icon:'☁'};return{color:'#ffd35a',shape:'circle',icon:'✦'}}
function skyDrawViewFinder(ctx,g,pointing){
 if(!skyAimEnabled)return;ctx.save();const s=18,L=8;ctx.strokeStyle='rgba(255,215,70,.92)';ctx.lineWidth=1.8;ctx.lineCap='round';for(const [sx,sy] of [[-1,-1],[1,-1],[-1,1],[1,1]]){const x=g.cx+sx*s,y=g.cy+sy*s;ctx.beginPath();ctx.moveTo(x,y-sy*L);ctx.lineTo(x,y);ctx.lineTo(x-sx*L,y);ctx.stroke()}if(pointing){ctx.font='700 10px system-ui';ctx.fillStyle='rgba(255,225,105,.90)';ctx.textAlign='center';ctx.fillText(`${Math.round(pointing.az)}° ${pointing.card} · ALT ${pointing.alt>=0?'+':''}${Math.round(pointing.alt)}°`,g.cx,g.cy+36)}ctx.restore();
}
function skyPickAutoConstellation(projected,g){let best=null,bestD=Infinity;for(const c of projected){for(const s of c.screenSegments){for(let i=1;i<s.length;i++){const d=skyLineDistance(g.cx,g.cy,s[i-1],s[i]);if(d<bestD){bestD=d;best=c}}}}const limit=Math.min(110,g.m*.22);return bestD<=limit?best:null}
function skyCanvasHit(canvas,source){
 if(canvas.dataset.skyBound==='1')return;canvas.dataset.skyBound='1';const pts=new Map();let pinch=null,tap=null;
 const local=e=>{const r=canvas.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top}};
 const hitAt=(x,y)=>{const hits=skyHitTargets[source]||[];let best=null,dist=Infinity;for(const h of hits){let d=Infinity;if(h.kind==='constellation'){for(const seg of h.segments||[])for(let i=1;i<seg.length;i++)d=Math.min(d,skyLineDistance(x,y,seg[i-1],seg[i]));if(h.label)d=Math.min(d,Math.hypot(x-h.label.x,y-h.label.y))}else d=Math.hypot(x-h.x,y-h.y);const lim=h.kind==='constellation'?24:(h.kind==='star'?18:22);if(d<=lim&&d<dist){dist=d;best=h}}if(best)openSkyObjectInfo(best.kind,best.key,source)};
 canvas.addEventListener('pointerdown',e=>{const p=local(e);pts.set(e.pointerId,p);tap={id:e.pointerId,x:p.x,y:p.y,t:performance.now(),moved:false};try{canvas.setPointerCapture(e.pointerId)}catch{}if(pts.size===2){const a=[...pts.values()];pinch={dist:Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y),zoom:skyZoom};tap.moved=true}}, {passive:false});
 canvas.addEventListener('pointermove',e=>{if(!pts.has(e.pointerId))return;const p=local(e),old=pts.get(e.pointerId);pts.set(e.pointerId,p);if(tap&&tap.id===e.pointerId&&Math.hypot(p.x-tap.x,p.y-tap.y)>9)tap.moved=true;if(pts.size>=2){e.preventDefault();const a=[...pts.values()].slice(0,2),d=Math.hypot(a[0].x-a[1].x,a[0].y-a[1].y);if(!pinch)pinch={dist:d,zoom:skyZoom};else if(pinch.dist>8)skySetZoom(pinch.zoom*d/pinch.dist)}}, {passive:false});
 const finish=e=>{const p=local(e),wasTap=tap&&tap.id===e.pointerId&&!tap.moved&&pts.size===1&&performance.now()-tap.t<650;pts.delete(e.pointerId);if(pts.size<2)pinch=null;if(wasTap)hitAt(p.x,p.y);if(tap?.id===e.pointerId)tap=null};
 canvas.addEventListener('pointerup',finish,{passive:false});canvas.addEventListener('pointercancel',e=>{pts.delete(e.pointerId);pinch=null;tap=null},{passive:false});
 canvas.addEventListener('wheel',e=>{e.preventDefault();skySetZoom(skyZoom*(e.deltaY<0?1.18:.85))},{passive:false});
 canvas.addEventListener('dblclick',e=>{e.preventDefault();skySetZoom(1)});
}
function updateSkyPolarCompass(ctrl,pointing){if(!ctrl?.panel?.isConnected||!ctrl.active||!pointing)return;ctrl.panel.classList.add('sky-polar-compass');ctrl.rose.style.transform=`rotate(${-pointing.az}deg)`;ctrl.value.textContent=`${Math.round(pointing.az)}°`;ctrl.cardinal.textContent=pointing.card;ctrl.note.textContent=`BRÚJULA CELESTE · ALT ${pointing.alt>=0?'+':''}${Math.round(pointing.alt)}°`;ctrl.panel.classList.toggle('north-aligned',Math.min(pointing.az,360-pointing.az)<=7)}
function skyRenderCanvas(canvas,lat,lon,now,sensor,source='normal'){
 if(!canvas)return{visible:[],active:null,pointing:null};const g=skyCanvasGeom(canvas),ctx=g.ctx,scene=skyBuildScene(lat,lon,now),basis=sensor?deviceSkyBasis(skyOrientationState.alpha,skyOrientationState.beta,skyOrientationState.gamma):null,pointing=basis?skyPointingDirection(basis):null,project=o=>basis?skyProjectVector(o.v,basis,g):skyProjectStatic(o,g),cfg=skyDetailConfig();ctx.clearRect(0,0,g.w,g.h);const grad=ctx.createRadialGradient(g.cx,g.cy,0,g.cx,g.cy,Math.max(g.w,g.h)*.75);grad.addColorStop(0,'#102b4c');grad.addColorStop(1,'#030d1b');ctx.fillStyle=grad;ctx.fillRect(0,0,g.w,g.h);
 if(!sensor){ctx.save();ctx.strokeStyle='rgba(110,145,180,.26)';for(const alt of [0,30,60]){const R=g.m*.46*g.zoom*(90-alt)/90;if(R<Math.max(g.w,g.h)*1.2){ctx.beginPath();ctx.arc(g.cx,g.cy,R,0,Math.PI*2);ctx.stroke()}}ctx.fillStyle='#ffc400';ctx.font='800 12px system-ui';ctx.textAlign='center';ctx.fillText('N',g.cx,16);ctx.fillText('S',g.cx,g.h-10);ctx.fillText('E',g.w-12,g.cy);ctx.fillText('O',12,g.cy);ctx.restore()}
 const projectedConst=[];for(const c of scene.constellations){const screenSegments=[];for(const seg of c.segments){const ss=[];for(const p of seg){const q=project(p);if(q)ss.push(q);else if(ss.length>1){screenSegments.push(ss.splice(0))}else ss.length=0}if(ss.length>1)screenSegments.push(ss)}if(screenSegments.length)projectedConst.push({...c,screenSegments})}
 const active=skyAimEnabled?skyPickAutoConstellation(projectedConst,g):null,searchTarget=skyResolveSearchTarget(scene);skyAutoConstellation=active?.name||null;const nowMs=performance.now();if(active?.abbr!==skyDwell.key){skyDwell={key:active?.abbr||null,since:nowMs}}const dwellReady=!!active&&nowMs-skyDwell.since>=900;
 // Las demás constelaciones quedan tenues; la apuntada resalta con claridad.
 for(const c of projectedConst){const hot=(active&&c.abbr===active.abbr)||(skySearchTarget?.kind==='constellation'&&(skySearchTarget.key===c.abbr||skySearchTarget.name===c.name));ctx.save();ctx.strokeStyle=hot?'#ffd400':`rgba(103,150,198,${cfg.constAlpha})`;ctx.lineWidth=hot?3.0:.85;ctx.shadowColor=hot?'#ffd400':'transparent';ctx.shadowBlur=hot?(dwellReady?14:8):0;for(const seg of c.screenSegments){ctx.beginPath();ctx.moveTo(seg[0].x,seg[0].y);for(let i=1;i<seg.length;i++)ctx.lineTo(seg[i].x,seg[i].y);ctx.stroke();if(hot){ctx.fillStyle='#ffd400';for(const p of seg){ctx.beginPath();ctx.arc(p.x,p.y,dwellReady?3.8:3.0,0,Math.PI*2);ctx.fill()}}}ctx.restore()}
 const labelBoxes=[];const canLabel=(x,y,text,font='10px system-ui')=>{ctx.font=font;const w=ctx.measureText(text).width+10,h=16,box={x:x-3,y:y-h+3,w,h};if(box.x<0||box.x+w>g.w||box.y<0||box.y+h>g.h)return false;for(const b of labelBoxes)if(!(box.x+box.w<b.x||b.x+b.w<box.x||box.y+box.h<b.y||b.y+b.h<box.y))return false;labelBoxes.push(box);return true};
 // Estrellas: la densidad depende del nivel de detalle; etiquetas solo para las más brillantes y sin solaparse.
 const visible=[],starHits=[];for(const st of scene.stars){if(st.mag>cfg.starMag)continue;const p=project(st);if(!p)continue;visible.push(st);const r=skyStarRadius(st.mag)*Math.min(1.35,1+Math.log2(g.zoom)*.09);ctx.beginPath();ctx.fillStyle=skyStarColor(st.bv);ctx.globalAlpha=Math.max(.35,Math.min(1,1.08-(st.mag/9)));ctx.arc(p.x,p.y,r,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;const labelMag=skyDetailMode==='complete'?2.1:(skyDetailMode==='normal'?1.5:1.0);if(st.name&&st.mag<=labelMag&&canLabel(p.x+6,p.y-5,st.name)){ctx.fillStyle='rgba(230,241,255,.88)';ctx.font='10px system-ui';ctx.fillText(st.name,p.x+6,p.y-5)}if(st.name||st.display)starHits.push({kind:'star',key:st.id,x:p.x,y:p.y})}
 // Objetos de cielo profundo: símbolos sí, nombres solo cerca del visor para evitar nubes de Mxx.
 const dsoHits=[];if(cfg.dso)for(const d of scene.dsos){if(Number.isFinite(d.mag)&&d.mag<90&&d.mag>cfg.dsoMag)continue;const p=project(d);if(!p)continue;const style=skyDsoStyle(d.type),size=d.mag<=4?5.5:4;ctx.save();ctx.strokeStyle=style.color;ctx.fillStyle=style.color;ctx.globalAlpha=.78;ctx.lineWidth=1.2;if(style.shape==='ellipse'){ctx.beginPath();ctx.ellipse(p.x,p.y,size*1.5,size*.8,0,0,Math.PI*2);ctx.stroke()}else if(style.shape==='diamond'){ctx.beginPath();ctx.moveTo(p.x,p.y-size);ctx.lineTo(p.x+size,p.y);ctx.lineTo(p.x,p.y+size);ctx.lineTo(p.x-size,p.y);ctx.closePath();ctx.stroke()}else{ctx.beginPath();ctx.arc(p.x,p.y,size,0,Math.PI*2);ctx.stroke()}const dc=Math.hypot(p.x-g.cx,p.y-g.cy),showName=dc<=cfg.labelRadius;if(showName&&canLabel(p.x+7,p.y+3,d.name,'800 9px system-ui')){ctx.font='800 9px system-ui';ctx.fillText(d.name,p.x+7,p.y+3)}ctx.restore();dsoHits.push({kind:'dso',key:d.id,x:p.x,y:p.y})}
 // Sol, Luna y planetas: siempre visibles cuando entran en el campo y tocables.
 const solarHits=[];for(const body of scene.solar||[]){const p=project(body);if(!p)continue;const isSun=body.name==='Sol',isMoon=body.name==='Luna',bright=['Venus','Júpiter'].includes(body.name),mid=['Marte','Saturno'].includes(body.name),rad=isSun?15:isMoon?13:bright?11:mid?10:9,searched=skySearchTarget?.kind==='solar'&&(skySearchTarget.key===body.id||skySearchTarget.name===body.name);ctx.save();ctx.shadowColor=body.color;ctx.shadowBlur=isSun?24:isMoon?18:searched?18:11;ctx.fillStyle=body.color;ctx.beginPath();ctx.arc(p.x,p.y,rad,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle=searched?'#ffd400':'rgba(255,255,255,.72)';ctx.lineWidth=searched?3:1.2;ctx.beginPath();ctx.arc(p.x,p.y,rad+3,0,Math.PI*2);ctx.stroke();ctx.fillStyle=isSun?'#7b4d00':'#07172b';ctx.font=`900 ${isSun?15:14}px serif`;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(body.symbol||'●',p.x,p.y+.5);ctx.textBaseline='alphabetic';const label=`${body.symbol} ${body.name}`;if(canLabel(p.x+rad+7,p.y-7,label,'900 12px system-ui')){ctx.textAlign='left';ctx.font='900 12px system-ui';ctx.fillStyle=body.color;ctx.fillText(label,p.x+rad+7,p.y-7)}ctx.restore();solarHits.push({kind:'solar',key:body.id,x:p.x,y:p.y})}
 // Nombre de la constelación seleccionada, separado de la marca de mira.
 const conHit=[];for(const c of projectedConst){let sx=0,sy=0,n=0;for(const seg of c.screenSegments)for(const p of seg){sx+=p.x;sy+=p.y;n++}let label=n?{x:sx/n,y:sy/n}:null;if((c===active||(skySearchTarget?.kind==='constellation'&&(skySearchTarget.key===c.abbr||skySearchTarget.name===c.name)))&&label){const name=c.name,dx=label.x-g.cx,dy=label.y-g.cy,len=Math.hypot(dx,dy)||1,offset=56,labelX=Math.max(70,Math.min(g.w-70,g.cx+(dx/len)*offset)),labelY=Math.max(34,Math.min(g.h-44,g.cy+(dy/len)*offset));ctx.save();ctx.textAlign='center';ctx.font='900 17px system-ui';const tw=ctx.measureText(name).width;ctx.fillStyle='rgba(2,13,27,.82)';ctx.fillRect(labelX-tw/2-9,labelY-18,tw+18,dwellReady?42:27);ctx.fillStyle='#ffd400';ctx.shadowColor='#ffd400';ctx.shadowBlur=dwellReady?11:6;ctx.fillText(name,labelX,labelY);if(dwellReady){ctx.shadowBlur=0;ctx.font='800 9px system-ui';ctx.fillStyle='#fff1a0';ctx.fillText('Toca para información',labelX,labelY+15)}ctx.restore();label={x:labelX,y:labelY}}conHit.push({kind:'constellation',key:c.abbr,segments:c.screenSegments,label})}
 skyDrawSearchGuide(ctx,g,searchTarget,pointing,project,basis);if(sensor)skyDrawViewFinder(ctx,g,pointing);ctx.save();ctx.fillStyle='rgba(255,255,255,.68)';ctx.font='700 9px system-ui';ctx.textAlign='right';ctx.fillText(`${skyZoomLabel()} · campo ~${skyApproxFov()}° · ${skyDetailMode==='simple'?'simple':skyDetailMode==='complete'?'completo':'normal'}`,g.w-8,g.h-8);ctx.restore();skyHitTargets[source]=[...solarHits,...starHits,...dsoHits,...conHit];skyCanvasHit(canvas,source);return{visible,active,pointing};
}
function skyCatalogSummary(){return SKY_RICH.deep?`${SKY_RICH.stars.length.toLocaleString('es-ES')} estrellas · ${SKY_RICH.constellations.length} constelaciones · ${SKY_RICH.dsos.length} objetos Messier · Sistema Solar`:'catálogo básico sin conexión · Sistema Solar'}
function renderSky(lat,lon,label){
 skyLocation={lat,lon,label};const info=document.getElementById('skyInfo'),visEl=document.getElementById('skyVisible'),now=new Date(),sensor=skyTrackActive&&skyOrientationState;let result={visible:[],active:null,pointing:null};const normal=document.getElementById('skyCanvas'),full=document.getElementById('skyFullCanvas');if(normal)result=skyRenderCanvas(normal,lat,lon,now,sensor,'normal');if(full)result=skyRenderCanvas(full,lat,lon,now,sensor,'full');const pointing=result.pointing,aim=pointing?`APUNTANDO ${Math.round(pointing.az)}° ${pointing.card} · ALT ${pointing.alt>=0?'+':''}${Math.round(pointing.alt)}°`:'';
 if(info)info.innerHTML=`<b>${esc(label)}</b><span>${lat.toFixed(4)}, ${lon.toFixed(4)} · ${now.toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'})}</span><small>${sensor?`MODO CIELO ACTIVO · ${aim}`:'Vista polar del cielo sobre el horizonte'} · ${skyCatalogSummary()}</small>`;const fs=document.getElementById('skyFullStatus');if(fs)fs.textContent=`${label} · ${sensor?aim:'Vista cielo'} · ${now.toLocaleTimeString('es-ES',{hour:'2-digit',minute:'2-digit'})}`;if(visEl){const con=result.active;visEl.innerHTML=!skyAimEnabled?`<b>👁 MIRA OFF · Vista general del cielo</b><small>Toca directamente estrellas, planetas, constelaciones o cielo profundo para abrir su información.</small>`:con?`<b>✨ Apuntas hacia <button type="button" data-info-con="${esc(con.abbr)}">${esc(con.name)}</button></b><small>Tócala para abrir información, fotos y datos.</small>`:`<b>MIRA ON · mueve el teléfono: al aproximar la marca discreta a una constelación se iluminará en amarillo.</b><small>${skyCatalogSummary()}</small>`;visEl.querySelector('[data-info-con]')?.addEventListener('click',e=>openSkyObjectInfo('constellation',e.currentTarget.dataset.infoCon,'normal'))}
 if(pointing){updateSkyPolarCompass(skyCompassCtrl,pointing);updateSkyPolarCompass(skyFullCompassCtrl,pointing)}
}
function skyFindStar(key){return SKY_RICH.stars.find(s=>s.id===key||s.name===key||s.display===key)||SKY_STARS.find(s=>s.name===key)}
function skyFindDso(key){return SKY_RICH.dsos.find(d=>d.id===key||d.name===key)}
function skyFindSolar(key){return skySolarSystem(new Date()).find(x=>x.id===key||x.name===key)}
function skyFindConst(key){const abbr=String(key||'');return SKY_RICH.constellations.find(c=>c.abbr===abbr||c.name===abbr)||(()=>{const m=Object.entries(SKY_CONST_NAMES).find(([,v])=>v[0]===abbr||v[1]===abbr);return m?{...skyConstMeta(m[0]),segments:[]}:null})()}
async function skyWikiExtract(query,host){if(!host)return;try{const u='https://es.wikipedia.org/w/api.php?origin=*&format=json&action=query&generator=search&gsrlimit=1&gsrsearch='+encodeURIComponent(query)+'&prop=extracts|info&exintro=1&explaintext=1&exsentences=4&inprop=url',r=await fetch(u,{cache:'force-cache'});if(!r.ok)throw 0;const j=await r.json(),p=Object.values(j?.query?.pages||{})[0],txt=String(p?.extract||'').trim();if(txt&&host.isConnected)host.innerHTML=`<p>${esc(txt)}</p>${p?.fullurl?`<a href="${esc(p.fullurl)}" target="_blank" rel="noopener">Ampliar en Wikipedia ↗</a>`:''}`}catch(_){}}
function skyInfoIntro(source='normal'){const box=skyInfoPanel(source);if(!box)return;box.hidden=false;box.innerHTML=`<div class="sky-object-head"><div><small>INFO DEL CIELO</small><h3>ℹ️ Toca estrellas, planetas, constelaciones o galaxias</h3></div><button type="button" class="sky-info-close" aria-label="Cerrar información">×</button></div><p>El visor usa ${esc(skyCatalogSummary())}. Con MIRA ON, una pequeña marca de visor selecciona la constelación hacia la que apuntas; con MIRA OFF puedes recorrer el cielo sin selección automática. Toca el nombre, una línea, una estrella, el Sol, la Luna, un planeta o un objeto de cielo profundo para abrir aquí su ficha.</p><div class="constellation-facts"><span>⭐ Estrellas hasta magnitud 6</span><span>✨ 88 constelaciones IAU</span><span>☀ Sol · ☾ Luna · 7 planetas</span><span>🌌 Galaxias/nebulosas/cúmulos Messier</span><span>🧭 Brújula celeste polar AZ/ALT</span><span>👁 MIRA ON/OFF</span><span>🔭 Zoom ×1 · ×2 · ×4 · ×8</span><span>⭐ Detalle Simple · Normal · Completo</span><span>🔎 Buscador con flecha guía</span></div><small class="license-note">Catálogo astronómico: Celestial Data / d3-celestial, datos J2000 reutilizables con atribución. El zoom amplía la carta celeste; no sustituye a un telescopio óptico ni hace visibles objetos que el ojo no puede observar.</small>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true}
async function openSkyObjectInfo(kind,key,source='normal'){
 const box=skyInfoPanel(source);if(!box)return;box.hidden=false;const close=`<button type="button" class="sky-info-close" aria-label="Cerrar información">×</button>`;
 if(kind==='star'){const st=skyFindStar(key);if(!st)return skyInfoIntro(source);const h=skyLocation?skyHorizontal(st,skyLocation.lat,skyLocation.lon,new Date()):null,name=st.name||st.display||`Estrella ${st.id}`;box.innerHTML=`<div class="sky-object-head"><div><small>ESTRELLA</small><h3>⭐ ${esc(name)}</h3><span>${esc(st.id||'')}</span></div>${close}</div><div class="constellation-facts"><span>◉ Magnitud: ${Number(st.mag).toFixed(2)}</span>${h?`<span>↗ Altitud: ${Math.round(h.alt)}°</span><span>🧭 Acimut: ${Math.round(h.az)}° ${compassCardinal(h.az)}</span>`:''}<span>α ${Number(st.ra).toFixed(3)} h · δ ${Number(st.dec).toFixed(2)}°</span></div><div class="sky-wiki"><p>${esc(STAR_INFO[name]||'Estrella del catálogo celeste. Su posición aparente se calcula para la hora y ubicación actuales.')}</p></div><div class="constellation-links"><a href="https://commons.wikimedia.org/wiki/Special:MediaSearch?type=image&search=${encodeURIComponent(name+' star astronomy')}" target="_blank" rel="noopener">Fotos libres Commons ↗</a></div>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true;skyWikiExtract(`${name} estrella`,box.querySelector('.sky-wiki'));if(source==='normal')box.scrollIntoView({behavior:'smooth',block:'nearest'});return}
 if(kind==='solar'){const b=skyFindSolar(key);if(!b)return skyInfoIntro(source);const h=skyLocation?skyHorizontal(b,skyLocation.lat,skyLocation.lon,new Date()):null,extra=b.name==='Luna'&&Number.isFinite(b.phase)?`<span>◐ Iluminación: ${Math.round(b.phase*100)}%</span>`:'';box.innerHTML=`<div class="sky-object-head"><div><small>SISTEMA SOLAR</small><h3>${esc(b.symbol)} ${esc(b.name)}</h3><span>Posición calculada para ahora</span></div>${close}</div><p>${esc(b.text||'Objeto del Sistema Solar.')}</p><div class="constellation-facts"><span>◉ Magnitud orientativa: ${Number(b.mag).toFixed(1)}</span>${extra}${h?`<span>↗ Altitud: ${Math.round(h.alt)}°</span><span>🧭 Acimut: ${Math.round(h.az)}° ${compassCardinal(h.az)}</span>`:''}${b.note?`<span>📐 ${esc(b.note)}</span>`:''}</div><div class="sky-wiki"><p>La posición se calcula localmente a partir de la fecha y hora del dispositivo; es apropiada para localizar el objeto en la carta, no para navegación astronómica de precisión.</p></div><div class="constellation-links"><a href="https://commons.wikimedia.org/wiki/Special:MediaSearch?type=image&search=${encodeURIComponent(b.name+' astronomy')}" target="_blank" rel="noopener">Fotos libres Commons ↗</a></div>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true;skyWikiExtract(`${b.name} astronomía`,box.querySelector('.sky-wiki'));return}
 if(kind==='dso'){const d=skyFindDso(key);if(!d)return skyInfoIntro(source);const h=skyLocation?skyHorizontal(d,skyLocation.lat,skyLocation.lon,new Date()):null,t=SKY_TYPE_NAMES[d.type]||'Objeto de cielo profundo';box.innerHTML=`<div class="sky-object-head"><div><small>${esc(t.toUpperCase())}</small><h3>🌌 ${esc(d.name)}</h3><span>${esc(d.id)}</span></div>${close}</div><div class="constellation-facts"><span>Tipo: ${esc(t)}</span>${Number.isFinite(d.mag)&&d.mag<90?`<span>◉ Magnitud: ${d.mag.toFixed(1)}</span>`:''}${d.dim?`<span>↔ Tamaño: ${esc(d.dim)}</span>`:''}${h?`<span>↗ Altitud: ${Math.round(h.alt)}°</span><span>🧭 Acimut: ${Math.round(h.az)}° ${compassCardinal(h.az)}</span>`:''}</div><div class="sky-wiki"><p>Objeto Messier/cielo profundo visible en la dirección indicada. La visibilidad real depende de oscuridad, fase lunar, contaminación lumínica y equipo óptico.</p></div><div class="constellation-links"><a href="https://commons.wikimedia.org/wiki/Special:MediaSearch?type=image&search=${encodeURIComponent(d.name+' astronomy')}" target="_blank" rel="noopener">Fotos reales Commons ↗</a></div>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true;skyWikiExtract(`${d.name} astronomía`,box.querySelector('.sky-wiki'));return}
 const c=skyFindConst(key);if(!c)return skyInfoIntro(source);const base=CONSTELLATION_INFO[c.name],desc=base?.text||`${c.name} (${c.latin}) es una de las constelaciones reconocidas por la Unión Astronómica Internacional. BUEN CAMINO dibuja su figura con las líneas del catálogo celeste y la resalta cuando el visor se aproxima a ella.`,scene=skyLocation?skyBuildScene(skyLocation.lat,skyLocation.lon,new Date()):null,center=scene?.centers?.find(x=>x.abbr===c.abbr),namedStars=SKY_RICH.stars.filter(x=>x.con===c.abbr&&(x.name||x.display)).sort((a,b)=>a.mag-b.mag).slice(0,5).map(x=>x.name||x.display).filter(Boolean),locate=center?`Ahora desde ${skyLocation.label}: apunta aprox. a ${Math.round(center.az)}° ${compassCardinal(center.az)} y ALT ${center.alt>=0?'+':''}${Math.round(center.alt)}°.`:'Mueve el móvil hasta que el nombre de la constelación aparezca en amarillo alrededor del visor.';box.innerHTML=`<div class="sky-object-head"><div><small>CONSTELACIÓN</small><h3>✨ ${esc(c.name)}</h3><span>${esc(c.latin)} · ${esc(c.abbr)}</span></div>${close}</div><p>${esc(desc)}</p><div class="constellation-facts"><span>✨ Código IAU: ${esc(c.abbr)}</span>${base?.season?`<span>🗓 Mejor época: ${esc(base.season)}</span>`:''}${base?.bright?`<span>⭐ Referencia brillante: ${esc(base.bright)}</span>`:''}${namedStars.length?`<span>⭐ Estrellas principales: ${esc(namedStars.join(' · '))}</span>`:''}<span>🧭 Cómo localizarla: ${esc(locate)}</span><span>🟡 Se ilumina al apuntar hacia ella</span></div><h4 class="sky-info-subtitle">Historia, astronomía y contexto</h4><div class="sky-wiki"><p>Cargando información adicional…</p></div><h4 class="sky-info-subtitle">Fotografías reales con licencia libre</h4><div class="constellation-photos"><div class="photo-loading">📷 Buscando fotografías reales con licencia libre…</div></div><small class="license-note">Las fotografías proceden de Wikimedia Commons y solo se muestran cuando la API declara una licencia libre o dominio público.</small>`;box.querySelector('.sky-info-close').onclick=()=>box.hidden=true;skyWikiExtract(`${c.name} constelación`,box.querySelector('.sky-wiki'));const host=box.querySelector('.constellation-photos');try{const photos=await commonsConstellationPhotos(c.name);if(!host?.isConnected)return;if(!photos.length)throw 0;host.innerHTML=photos.map(p=>`<figure><a href="${esc(p.page)}" target="_blank" rel="noopener"><img src="${esc(p.thumb)}" alt="${esc(c.name)}" loading="lazy"></a><figcaption><b>${esc(p.title)}</b><span>${esc(p.artist)}</span><a href="${esc(p.licenseUrl||p.page)}" target="_blank" rel="noopener">${esc(p.license)} ↗</a></figcaption></figure>`).join('')}catch{if(host)host.innerHTML='<div class="empty">No se encontró ahora una fotografía con licencia libre claramente identificada.</div>'}if(source==='normal')box.scrollIntoView({behavior:'smooth',block:'nearest'});
}
async function activateSkyPolarCompass(ctrl){if(!ctrl?.panel?.isConnected)return;if(mapCompassActive&&mapCompassActive!==ctrl)deactivateMapCompass(mapCompassActive,true);mapCompassActive=ctrl;ctrl.active=true;ctrl.panel.hidden=false;ctrl.btn.classList.add('active');ctrl.btn.textContent='🧭 BRÚJULA ON';requestAnimationFrame(()=>positionMapCompass(ctrl));if(skyTrackActive&&skyOrientationState){const b=deviceSkyBasis(skyOrientationState.alpha,skyOrientationState.beta,skyOrientationState.gamma);updateSkyPolarCompass(ctrl,skyPointingDirection(b));return}await startCompassSensor((h,a,s)=>{updateMapCompass(ctrl,h,a,s);if(ctrl.note?.isConnected)ctrl.note.textContent='BRÚJULA CELESTE · activa MODO CIELO para ALT'},(k,t,x)=>mapCompassStatus(ctrl,k,t,x))}
function attachSkyPolarCompass(host,btn){const ctrl=attachMapCompass({el:host},btn);if(ctrl){ctrl.panel.classList.add('sky-polar-compass');ctrl.panel.setAttribute('aria-label','Brújula celeste polar de azimut y altura. Arrastra con el dedo para moverla.');ctrl.note.textContent='BRÚJULA CELESTE · AZ/ALT';btn.onclick=e=>{e?.stopPropagation?.();ctrl.active?deactivateMapCompass(ctrl,true):activateSkyPolarCompass(ctrl)};ctrl.panel.querySelector('.map-compass-close').onclick=e=>{e.stopPropagation();deactivateMapCompass(ctrl,true)}}return ctrl}
...
function openSkyFullView(){
 ...
 const overlay=document.createElement('section');overlay.className='sky-fullscreen-overlay';overlay.setAttribute('aria-label','Gran vista del cielo');overlay.innerHTML=`<header class="sky-full-top"><div class="sky-full-nav"><button id="skyFullBack" type="button">← VOLVER</button><button id="skyFullStage" type="button">👣 ETAPA</button><button id="skyFullHome" type="button">🏠 INICIO</button></div><div class="sky-full-tools"><button id="skyFullCompass" class="map-compass-toggle" type="button">🧭 BRÚJULA</button><button id="skyFullAim" data-sky-aim type="button">👁 MIRA ON</button><button id="skyFullSearch" data-sky-search type="button">🔎 BUSCAR</button><button id="skyFullInfoBtn" type="button">ℹ INFO</button></div><small id="skyFullStatus">Gran vista cielo</small></header><div id="skyFullWrap" class="sky-full-canvas"><canvas id="skyFullCanvas" role="img" aria-label="Gran vista interactiva del cielo con miles de estrellas, constelaciones y objetos de cielo profundo. Pellizca con dos dedos para ampliar."></canvas><div class="sky-telescope-controls" aria-label="Zoom telescopio"><button id="skyZoomPlus" type="button" aria-label="Aumentar zoom">＋</button><span data-sky-zoom-readout>🔭 ×1</span><button id="skyZoomMinus" type="button" aria-label="Reducir zoom">−</button><button id="skyZoomReset" class="wide" type="button">↺ ×1</button></div><button id="skyFullDetail" class="sky-detail-float" data-sky-detail type="button">⭐ NORMAL</button><section id="skyFullInfoPanel" class="sky-full-info-panel" hidden></section></div>`;document.body.appendChild(overlay);skyFullOverlay=overlay;document.body.classList.add('sky-full-open');document.body.style.overflow='hidden';
 document.getElementById('skyFullBack').onclick=()=>closeSkyFullView(true);document.getElementById('skyFullStage').onclick=skyGoStage;document.getElementById('skyFullHome').onclick=skyGoHome;document.getElementById('skyFullInfoBtn').onclick=()=>{const p=skyInfoPanel('full');p?.hidden?skyInfoIntro('full'):p.hidden=true};document.getElementById('skyFullAim').onclick=skyToggleAim;document.getElementById('skyFullSearch').onclick=()=>openSkySearch('full');document.getElementById('skyZoomPlus').onclick=()=>skyZoomStep(1);document.getElementById('skyZoomMinus').onclick=()=>skyZoomStep(-1);document.getElementById('skyZoomReset').onclick=()=>skySetZoom(1);document.getElementById('skyFullDetail').onclick=skyCycleDetail;skyUpdateControls();const wrap=document.getElementById('skyFullWrap');skyFullCompassCtrl=attachSkyPolarCompass(wrap,document.getElementById('skyFullCompass'));if(skyNormalCompassResume||skyTrackActive)activateSkyPolarCompass(skyFullCompassCtrl);requestAnimationFrame(()=>{if(skyLocation)renderSky(skyLocation.lat,skyLocation.lon,skyLocation.label)});
}

function openSkyFeature(stage=null){
 const n=Number(stage)||1,s=STAGES[n-1];featureShell('✨','Estrellas y constelaciones',stage?`Etapa ${n} · ${s.from} → ${s.to}`:'Cielo del Camino Francés',`<section class="feature-body sky-feature-body"><div class="sky-local-nav" aria-label="Navegación de cielo"><button id="skyLocalBack" type="button">← VOLVER</button><button id="skyLocalStage" type="button">👣 ETAPA</button><button id="skyLocalHome" type="button">🏠 INICIO</button></div><div class="feature-control-card sky-controls"><label for="skyStageSelect">POSICIÓN DE REFERENCIA</label><select id="skyStageSelect" class="feature-select">${featureStageOptions(n)}</select><div class="sky-action-grid"><button id="skyGpsBtn" class="feature-primary secondary" type="button">📍 USAR MI GPS</button><button id="skyTrackBtn" class="feature-primary" type="button">✨ ACTIVAR MODO CIELO</button><button id="skyCompassBtn" class="feature-primary secondary map-compass-toggle" type="button">🧭 BRÚJULA</button><button id="skyFullBtn" class="feature-primary sky-full-btn" type="button">🌌 GRAN VISTA CIELO</button><button id="skyInfoBtn" class="feature-primary secondary sky-info-btn" type="button">ℹ️ INFO CIELO</button><button id="skyAimBtn" class="feature-primary secondary sky-aim-btn" data-sky-aim type="button">👁 MIRA ON</button><button id="skySearchBtn" class="feature-primary secondary sky-search-btn" data-sky-search type="button">🔎 BUSCAR CIELO</button><button id="skyDetailBtn" class="feature-primary secondary sky-detail-btn" data-sky-detail type="button">⭐ NORMAL</button></div></div><div id="skyStatus" class="feature-status loading"></div><div id="skyInfo" class="sky-info"></div><div id="skyWrap" class="sky-wrap"><canvas id="skyCanvas" role="img" aria-label="Mapa del cielo interactivo con miles de estrellas, 88 constelaciones y objetos de cielo profundo. Pellizca con dos dedos para ampliar."></canvas><div class="sky-zoom-hint"><span data-sky-zoom-readout>🔭 ×1</span><small>Pellizca con dos dedos · doble toque: ×1</small></div></div><div id="skyVisible" class="sky-visible"></div><section id="skyObjectInfoPanel" class="constellation-panel sky-object-panel" hidden></section><div class="feature-safety"><b>✨ Cómo usar el cielo</b><p>1) Pulsa <b>USAR MI GPS</b>. 2) Pulsa <b>ACTIVAR MODO CIELO</b>. 3) Apunta la parte trasera del móvil hacia el cielo y muévelo lentamente. Con <b>MIRA ON</b>, una marca discreta de visor indica la dirección a la que estás apuntando; con <b>MIRA OFF</b> desaparece para explorar el cielo en general. la app muestra además APUNTANDO (azimut) y ALT (altura sobre el horizonte) para poder comprobarlo. 4) Pulsa <b>GRAN VISTA CIELO</b> para ocupar toda la pantalla; funciona también al girar el móvil a horizontal. 5) El catálogo completo muestra miles de estrellas, las 88 constelaciones y objetos Messier (galaxias, nebulosas y cúmulos). Cuando MIRA está ON y la marca del visor se acerca a una constelación, las demás quedan más tenues, su figura se ilumina en amarillo y aparece su nombre; si mantienes el visor sobre ella un instante verás <b>Toca para información</b>. Tócalo para abrir la ficha en <b>INFO</b>. Usa <b>⭐ DETALLE</b> para cambiar entre Simple, Normal y Completo. En Gran Vista puedes ampliar como una carta telescópica con <b>＋ / −</b> o pellizcando con dos dedos hasta ×8; el zoom amplía la carta, no la capacidad óptica del móvil. La brújula del cielo es polar/altazimutal: indica AZ (azimut) y ALT (altura sobre el horizonte). Con <b>🔎 BUSCAR CIELO</b> puedes localizar planetas, estrellas con nombre y constelaciones: al elegir un objetivo aparece una flecha que indica si debes girar a izquierda/derecha y subir/bajar el móvil hasta encontrarlo.</p><p>En la vista normal y en la grande tienes <b>🏠 INICIO · 👣 ETAPA · ← VOLVER</b>. VOLVER recupera la pantalla y el desplazamiento desde donde abriste esta herramienta.</p></div></section>`);
 const sel=document.getElementById('skyStageSelect');sel.onchange=()=>{currentStageNo=+sel.value;loadSkyStage(+sel.value)};document.getElementById('skyGpsBtn').onclick=useGpsForSky;document.getElementById('skyTrackBtn').onclick=startSkyTracking;document.getElementById('skyFullBtn').onclick=openSkyFullView;document.getElementById('skyInfoBtn').onclick=()=>{const p=skyInfoPanel('normal');p?.hidden?skyInfoIntro('normal'):p.hidden=true};document.getElementById('skyAimBtn').onclick=skyToggleAim;document.getElementById('skySearchBtn').onclick=()=>openSkySearch('normal');document.getElementById('skyDetailBtn').onclick=skyCycleDetail;skyUpdateControls();document.getElementById('skyLocalBack').onclick=returnFromFeature;document.getElementById('skyLocalStage').onclick=skyGoStage;document.getElementById('skyLocalHome').onclick=skyGoHome;loadSkyCatalog();loadSkyStage(n);const wrap=document.getElementById('skyWrap');skyCompassCtrl=attachSkyPolarCompass(wrap,document.getElementById('skyCompassBtn'));skyTimer=setInterval(()=>{if(skyLocation&&document.getElementById('skyCanvas'))renderSky(skyLocation.lat,skyLocation.lon,skyLocation.label)},60000);
}

function home(){try{cleanupMaps()}catch(e){console.warn('Inicio: limpieza previa',e)}currentView='home';currentStageNo=null;setActive('home');const installed=isStandalone();app.innerHTML=`<section class="home-intro"><div><strong>33 etapas · mapas · servicios · gratis</strong><span>Camino Francés</span></div><div class="home-version-badge">V10.0.5 PRO · 2D SIN CAPAS</div><div class="home-share-row"><button id="whatsappShare" class="whatsapp-share">🟢 WHATSAPP</button><button id="homeComments" class="comments-main">💬 COMENTARIOS</button></div><div class="home-public-actions"><button id="installAppBtn" class="install-app-main ${installed?'installed':''}">${installed?'✓ APP INSTALADA':'📲 INSTALAR APP'}</button><button id="appGuideBtn" class="app-guide-main">📖 GUÍA APP</button></div><div class="home-feature-actions" aria-label="Herramientas del Camino"><button data-feature="walk"><span>🚶</span><b>A PIE · RUTAS</b></button><button data-feature="bike"><span>🚴</span><b>RUTA BICI</b></button><button data-feature="compass"><span>🧭</span><b>BRÚJULA</b></button><button data-feature="sky"><span>✨</span><b>ESTRELLAS</b><small>Constelaciones</small></button></div><button id="aboutPrivacyBtn" class="about-privacy-main">ℹ ACERCA DE · PRIVACIDAD · DATOS</button><div class="connection-row"><span id="connectionStatus" role="status" aria-live="polite" class="connection-pill ${navigator.onLine?'online':'offline'}">${navigator.onLine?'● Con conexión':'● Sin conexión'}</span><small>La app básica funciona instalada; mapas, tiempo y servicios externos pueden necesitar internet.</small></div><div class="home-counters compact"><div>👁 <strong>${appVisitCount}</strong><span>visitas</span></div><div>👍 <strong>${likedStages.size}</strong><span>me gusta</span></div><div>💬 <strong>${comments.length}</strong><span>comentarios</span></div></div><small class="counter-note">Contadores de este dispositivo</small></section><section class="section home-first"><div class="section-head"><h2>Las 33 etapas</h2><button class="link-btn" data-nav="stages">Ver todas</button></div>${STAGES.slice(0,5).map(stageCard).join('')}</section><section class="section"><h2>Accesos rápidos</h2><div class="service-grid">${SERVICE_TYPES.slice(0,9).map(x=>`<button class="service-btn" data-quick="${x[1]}"><span>${x[0]}</span>${x[2]}</button>`).join('')}</div></section><section class="section support-card compact-support"><div><b>💛 Donación solidaria</b><span>Creador: ${esc(DONATION_CREATOR)}</span></div><button class="donate-main" data-donate>Donar</button></section>`;bindCommon();document.getElementById('whatsappShare').onclick=shareAppWhatsApp;document.getElementById('homeComments').onclick=openComments;document.getElementById('appGuideBtn').onclick=openAppGuide;document.getElementById('installAppBtn').onclick=installApp;document.getElementById('aboutPrivacyBtn').onclick=openAboutPrivacy;updateConnectionStatus();}
...
class BCMap{
 ...
 _bind(){
  // V10.0.4: motor 2D táctil Android/Samsung.
  // Touch Events son la vía principal en móvil; Pointer Events quedan para ratón/lápiz.
  const surface=this.el.querySelector('.bc-gesture-layer');
  const controls='button,a,.route-popup,.bc-map-legend,.bc-service-filters,.map-compass-overlay';
  ...
  const nearestYellow=(clientX,clientY)=>{if(!this.routeOn||!this.route?.length)return-1;const r=this.el.getBoundingClientRect(),x=clientX-r.left,y=clientY-r.top,target=this.zoom>=16?102:this.zoom>=14?68:41,step=Math.max(1,Math.ceil(this.route.length/target));let best=-1,bd=Infinity;for(let i=0;i<this.route.length;i+=step){const p=this.pixel(this.route[i][0],this.route[i][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=i}}const last=this.route.length-1;if(last%step!==0){const p=this.pixel(this.route[last][0],this.route[last][1]),d=Math.hypot(p.x-x,p.y-y);if(d<bd){bd=d;best=last}}return bd<=40?best:-1;};

  // Controles: ejecutar al PRESIONAR. Evita el click/pointerup cancelado por Chrome Android.
  this.el.querySelectorAll('[data-z]').forEach(b=>{
    let last=0;
    const act=e=>{const now=Date.now();if(now-last<350)return;last=now;e?.preventDefault?.();e?.stopPropagation?.();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));};
    b.addEventListener('touchstart',act,{passive:false});
    b.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||e.pointerType==='pen')act(e)},{passive:false});
    b.addEventListener('click',act);
  });
  this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};
  this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};
  const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};
  const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};
  const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};
  this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});

  // Tacto real sobre una superficie dedicada que queda por debajo de los controles.
  let ts=null;
  surface.addEventListener('touchstart',e=>{
    const a=[...e.touches];if(!a.length)return;e.preventDefault();e.stopPropagation();this.userInteracted=true;this.closePopup();resetTransforms();
    if(a.length===1){const p=tp(a[0]);ts={mode:'pan',last:p,start:p,moved:false,t:performance.now()};}
    else{const p1=tp(a[0]),p2=tp(a[1]);ts={mode:'pinch',lastMid:mid(p1,p2),lastDist:Math.max(1,dist(p1,p2)),moved:true};}
  },{passive:false});
  surface.addEventListener('touchmove',e=>{
    if(!ts)return;const a=[...e.touches];if(!a.length)return;e.preventDefault();e.stopPropagation();
    if(a.length===1){const p=tp(a[0]);if(ts.mode!=='pan'){ts={mode:'pan',last:p,start:p,moved:true,t:performance.now()};return}const dx=p.x-ts.last.x,dy=p.y-ts.last.y;if(Math.abs(dx)+Math.abs(dy)>.25){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-dx,cp.y-dy,this.zoom);ts.last=p;if(Math.hypot(p.x-ts.start.x,p.y-ts.start.y)>4)ts.moved=true;this.requestRender();}return;}
    const p1=tp(a[0]),p2=tp(a[1]),m=mid(p1,p2),d=Math.max(1,dist(p1,p2));if(ts.mode!=='pinch'){ts={mode:'pinch',lastMid:m,lastDist:d,moved:true};return}const mdx=m.x-ts.lastMid.x,mdy=m.y-ts.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.15){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/ts.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;this.clearTileNodes();ts.lastDist=d;}else if(ratio<.92&&this.zoom>5){this.zoom--;this.clearTileNodes();ts.lastDist=d;}ts.lastMid=m;ts.moved=true;this.requestRender();
  },{passive:false});
  surface.addEventListener('touchend',e=>{
    if(!ts)return;e.preventDefault();e.stopPropagation();const left=[...e.touches];
    if(left.length>=2){const p1=tp(left[0]),p2=tp(left[1]);ts={mode:'pinch',lastMid:mid(p1,p2),lastDist:Math.max(1,dist(p1,p2)),moved:true};return;}
    if(left.length===1){const p=tp(left[0]);ts={mode:'pan',last:p,start:p,moved:true,t:performance.now()};return;}
    const was=ts,ch=e.changedTouches?.[0],end=ch?tp(ch):was.last;ts=null;
    if(was.mode==='pan'&&!was.moved&&end&&performance.now()-was.t<900){const i=nearestYellow(end.x,end.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);return;}}
    this.render();
  },{passive:false});
  surface.addEventListener('touchcancel',()=>{ts=null;this.render()},{passive:false});

  // Ratón y lápiz: Pointer Events sencillos.
  let pan=null;
  surface.addEventListener('pointerdown',e=>{if(e.pointerType==='touch'||(e.pointerType==='mouse'&&e.button!==0))return;e.preventDefault();this.userInteracted=true;this.closePopup();const cp=ll2world(this.center.lat,this.center.lon,this.zoom);pan={id:e.pointerId,start:{x:e.clientX,y:e.clientY},last:{x:e.clientX,y:e.clientY},cp,moved:false};try{surface.setPointerCapture(e.pointerId)}catch{}},{passive:false});
  surface.addEventListener('pointermove',e=>{if(!pan||e.pointerId!==pan.id)return;e.preventDefault();const dx=e.clientX-pan.last.x,dy=e.clientY-pan.last.y;const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-dx,cp.y-dy,this.zoom);pan.last={x:e.clientX,y:e.clientY};if(Math.hypot(e.clientX-pan.start.x,e.clientY-pan.start.y)>4)pan.moved=true;this.requestRender();},{passive:false});
  const finishPointer=e=>{if(!pan||e.pointerId!==pan.id)return;e.preventDefault();const was=pan;pan=null;if(!was.moved){const i=nearestYellow(e.clientX,e.clientY);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);return;}}this.render();};
  surface.addEventListener('pointerup',finishPointer,{passive:false});surface.addEventListener('pointercancel',()=>{pan=null;this.render()},{passive:false});
  surface.addEventListener('dblclick',e=>{e.preventDefault();this.setZoom(this.zoom+1)},{passive:false});
 ...
 selectRoutePoint(i,x,y){this.selectedRouteIndex=i;this.renderOverlay();this.onRoutePoint?.(i);this.closePopup();const p=this.route[i],samples=buildElevationSamples(this.stageNo||currentStageNo||1,[this.route]),idx=samples.length?nearestSample(samples,p[0],p[1]):0,s=samples[idx],detail=s?`Km ${s.km.toFixed(1)} · ${Math.round(s.ele)} m`:'Punto amarillo del trazado';openRoutePointSheet(p[0],p[1],this.stageNo||currentStageNo||1,{title:'Punto amarillo',detail});}
...
function cleanupMaps(){try{closeRoutePointSheet()}catch{}try{stopFeatureSensors()}catch(e){console.warn('Limpieza de sensores',e)}mapCompassActive=null;mapCompassResume=null;mainMapCompass=null;focusMapCompass=null;try{mainMap?.destroy?.()}catch{}mainMap=null;try{focusMap?.destroy?.()}catch{}focusMap=null;try{if(focusOverlay&&!focusOverlay.hidden)closeFocusMap()}catch(e){console.warn('Cierre mapa grande',e)}try{close3DMap()}catch(e){console.warn('Cierre 3D',e)}}
...
function open3DRoutePointPopup(map,ml,route,idx,stageNo){
 ...
 const samples=buildElevationSamples(stageNo||1,[route]),si=samples.length?nearestSample(samples,p[0],p[1]):0,sp=samples[si],detail=sp?`Km ${sp.km.toFixed(1)} · ${Math.round(sp.ele)} m · vista 3D`:'Punto amarillo · vista 3D';
 openRoutePointSheet(p[0],p[1],stageNo,{title:'Punto amarillo 3D',detail});return null;
}
...
window.addEventListener('resize',()=>{if(skyLocation&&(document.getElementById('skyCanvas')||document.getElementById('skyFullCanvas')))requestAnimationFrame(()=>renderSky(skyLocation.lat,skyLocation.lon,skyLocation.label));});
...
function openAppGuide(){
 ...
 <div class="guide-card"><strong>21. 🧭 Brújula</strong><p>Usa rumbo magnético/absoluto del móvil con media circular, zona muerta anti-temblor y bloqueo suave del Norte para que la aguja no baile al quedar orientada a N. Puede abrirse sola o flotar sobre mapa pequeño, MAPA GRANDE y mapas de rutas. El fondo es transparente y la brújula se arrastra con un dedo. Las marcas del aro dividen los 360 grados y la lectura central da el rumbo exacto. Normalmente la rosa es negra y la aguja roja; cuando el norte del mapa y el norte de la brújula coinciden (aprox. ±7°), la rosa pasa a rojo y la aguja a negro. Mantén el móvil lejos de metal/imanes y calibra con un movimiento en forma de 8 si oscila.</p></div>
 <div class="guide-card"><strong>22. ✨ Estrellas y constelaciones</strong><p>Primero pulsa <b>USAR MI GPS</b> y después <b>ACTIVAR MODO CIELO</b>. Apunta la parte trasera del móvil hacia el cielo; arriba se muestran <b>APUNTANDO</b> (azimut) y <b>ALT</b> (altura sobre el horizonte). Con <b>👁 MIRA ON</b> aparece una marca discreta de visor —sin diana circular— y la constelación cercana se resalta en amarillo; con <b>MIRA OFF</b> desaparece la marca y puedes explorar el cielo general tocando objetos directamente. Con conexión se carga un catálogo abierto con <b>miles de estrellas hasta magnitud 6, las 88 constelaciones IAU y objetos Messier</b>. La carta añade además <b>☀ Sol, ☾ Luna, Mercurio, Venus, Marte, Júpiter, Saturno, Urano y Neptuno</b>, calculados para la fecha y hora actuales y tocables para abrir su ficha. Las constelaciones no seleccionadas se dibujan tenues; al seleccionar una, sus estrellas y líneas brillan en amarillo y aparece su nombre. Usa <b>⭐ DETALLE</b> para elegir Simple, Normal o Completo. En <b>🌌 GRAN VISTA CIELO</b> dispones de zoom tipo telescopio <b>×1 · ×2 · ×4 · ×8</b> con botones ＋/− o pellizcando con dos dedos; doble toque o ↺ ×1 vuelve a la vista general. El zoom es una ampliación digital de la carta y no sustituye a un telescopio óptico. La brújula celeste polar/altazimutal es pequeña, transparente, movible y muestra AZ/ALT.</p></div>
 ...
 <div class="guide-card"><strong>24. 📷 Fotos Wikimedia cercanas</strong><p>Al tocar una bola amarilla se abre una ficha interna estable con Maps, Earth, OSM, Fotos y Servicios cerca. BUEN CAMINO carga una galería horizontal de hasta 12 fotografías cercanas de Wikimedia Commons y puedes deslizarla con el dedo. Si Commons no responde, puedes reintentar sin perder la información del punto. Se filtran archivos declarados con licencia libre o dominio público y se muestra autor/licencia; algunas licencias libres exigen atribución.</p></div>
...
if('serviceWorker' in navigator){
  navigator.serviceWorker.register('sw.js?v=1001',{updateViaCache:'none'})
    .then(reg=>reg.update().catch(()=>{}))
    .catch(()=>{});
}
function renderBootFallback(error){
 console.error('BUEN CAMINO · fallo de arranque',error);
 if(!app)return;
 app.innerHTML=`<section class="section"><div style="background:#fff;border:1px solid #d5e0ec;border-radius:16px;padding:18px;margin-top:12px"><h2 style="margin-top:0">BUEN CAMINO</h2><p>La aplicación no pudo completar el arranque. Tus Favoritos y Mi Camino no se han borrado.</p><button id="bootRepairBtn" style="width:100%;min-height:48px;border:0;border-radius:12px;background:#ffc400;color:#062b65;font-weight:950">↻ REPARAR Y RECARGAR</button><small style="display:block;margin-top:10px;color:#65758a">La reparación elimina únicamente cachés de BUEN CAMINO y vuelve a descargar los archivos de la versión actual.</small></div></section>`;
 document.getElementById('bootRepairBtn')?.addEventListener('click',()=>repairBuenCaminoBoot(true));
}
async function repairBuenCaminoBoot(force=false){
 try{
  if('serviceWorker' in navigator){const reg=await navigator.serviceWorker.getRegistration('./');if(reg)await reg.unregister().catch(()=>false)}
  if('caches' in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('buen-camino-')).map(k=>caches.delete(k)))}
 }catch(e){console.warn('Reparación de caché',e)}
 try{sessionStorage.setItem('bc-boot-repaired-v100','1')}catch{}
 const u=new URL('./',location.href);u.searchParams.set('bcv','100');u.searchParams.set('repair',Date.now().toString());location.replace(u.href);
}
function bootBuenCamino(){
 try{const initialStage=Number((location.hash.match(/^#stage-(\d+)/)||[])[1]);if(initialStage>=1&&initialStage<=33)detail(initialStage);else{home();setTimeout(openPublicWelcome,180)}}
 catch(e){renderBootFallback(e)}
}
bootBuenCamino();


/* V10.0.5 — controlador 2D de captura global, independiente de overlays. */
(()=>{
 if(window.__bc2dCaptureV1005)return; window.__bc2dCaptureV1005=true;
 let drag=null;
 const mapAt=(x,y)=>{
   const els=document.elementsFromPoint?document.elementsFromPoint(x,y):[document.elementFromPoint(x,y)];
   for(const el of els||[]){const root=el?.closest?.('.bc-map');if(root?.__bcMap)return root.__bcMap;}
   return null;
 };
 const nearest=(m,x,y)=>{if(!m?.routeOn||!m.route?.length)return-1;const r=m.el.getBoundingClientRect(),lx=x-r.left,ly=y-r.top,target=m.zoom>=16?102:m.zoom>=14?68:41,step=Math.max(1,Math.ceil(m.route.length/target));let best=-1,bd=1e9;for(let i=0;i<m.route.length;i+=step){const p=m.pixel(m.route[i][0],m.route[i][1]),d=Math.hypot(p.x-lx,p.y-ly);if(d<bd){bd=d;best=i}}return bd<=44?best:-1;};
 document.addEventListener('touchstart',e=>{
   const t=e.touches?.[0];if(!t)return;
   const z=e.target?.closest?.('.bc-map [data-z]');
   if(z){const m=z.closest('.bc-map')?.__bcMap;if(m){e.preventDefault();e.stopImmediatePropagation();m.userInteracted=true;m.setZoom(m.zoom+Number(z.dataset.z));}return;}
   if(e.target?.closest?.('.bc-map button,.bc-map a,.route-popup,.bc-map-legend,.bc-service-filters,.map-compass-overlay'))return;
   const m=mapAt(t.clientX,t.clientY);if(!m)return;
   e.preventDefault();e.stopImmediatePropagation();m.userInteracted=true;m.closePopup();
   drag={m,lastX:t.clientX,lastY:t.clientY,startX:t.clientX,startY:t.clientY,moved:false};
 },{capture:true,passive:false});
 document.addEventListener('touchmove',e=>{
   if(!drag)return;const t=e.touches?.[0];if(!t)return;e.preventDefault();e.stopImmediatePropagation();
   const dx=t.clientX-drag.lastX,dy=t.clientY-drag.lastY;drag.lastX=t.clientX;drag.lastY=t.clientY;
   if(Math.hypot(t.clientX-drag.startX,t.clientY-drag.startY)>5)drag.moved=true;
   const cp=ll2world(drag.m.center.lat,drag.m.center.lon,drag.m.zoom);drag.m.center=world2ll(cp.x-dx,cp.y-dy,drag.m.zoom);drag.m.requestRender();
 },{capture:true,passive:false});
 document.addEventListener('touchend',e=>{
   if(!drag)return;const d=drag;drag=null;const t=e.changedTouches?.[0];e.preventDefault();e.stopImmediatePropagation();
   if(!d.moved&&t){const i=nearest(d.m,t.clientX,t.clientY);if(i>=0){const p=d.m.pixel(d.m.route[i][0],d.m.route[i][1]);d.m.selectRoutePoint(i,p.x,p.y);return;}}
   d.m.render();
 },{capture:true,passive:false});
})();
