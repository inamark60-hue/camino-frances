...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v94')}catch(_){ }
...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>b.onclick=e=>{e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  ...
  // Ratón / lápiz: Pointer Events. En Android el tacto se gestiona abajo con Touch Events
  // para que el mapa incrustado de cada etapa conserve arrastre + pellizco de forma fiable.
  this.el.addEventListener('pointerdown',e=>{if(e.pointerType==='touch')return;if(e.target.closest('button,a,.route-popup'))return;e.preventDefault();this.userInteracted=true;this.closePopup();this.el.setPointerCapture?.(e.pointerId);this.activePointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(this.activePointers.size===1){this.panStart={x:e.clientX,y:e.clientY,cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0};this.pinch=null;}else if(this.activePointers.size===2){const pts=[...this.activePointers.values()];this.panStart=null;resetTransforms();this.pinch={lastDist:Math.max(1,dist(pts[0],pts[1]))};}});
  this.el.addEventListener('pointermove',e=>{if(e.pointerType==='touch'||!this.activePointers.has(e.pointerId))return;e.preventDefault();this.activePointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(this.activePointers.size>=2){const pts=[...this.activePointers.values()].slice(0,2),d=Math.max(1,dist(pts[0],pts[1]));if(!this.pinch)this.pinch={lastDist:d};const ratio=d/this.pinch.lastDist;if(ratio>1.15&&this.zoom<18){this.pinch.lastDist=d;this.setZoom(this.zoom+1);}else if(ratio<0.87&&this.zoom>5){this.pinch.lastDist=d;this.setZoom(this.zoom-1);}return;}if(this.panStart){const dx=e.clientX-this.panStart.x,dy=e.clientY-this.panStart.y;this.panStart.dx=dx;this.panStart.dy=dy;for(const node of [this.tiles,this.svg,this.markers])node.style.transform=`translate(${dx}px,${dy}px)`;}});
  const finish=e=>{if(e.pointerType==='touch')return;if(this.activePointers.has(e.pointerId))this.activePointers.delete(e.pointerId);if(this.activePointers.size===0){if(this.panStart){const ll=world2ll(this.panStart.cp.x-this.panStart.dx,this.panStart.cp.y-this.panStart.dy,this.zoom);this.center=ll;}this.panStart=null;this.pinch=null;resetTransforms();this.render();}else if(this.activePointers.size===1){resetTransforms();const p=[...this.activePointers.values()][0];this.panStart={x:p.x,y:p.y,cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0};this.pinch=null;}};
  this.el.addEventListener('pointerup',finish);this.el.addEventListener('pointercancel',finish);

  // Tacto Android/iOS: un dedo mueve; dos dedos mueven + pellizcan.
  // Se actualiza el centro directamente, evitando depender de pointer-capture multitáctil.
  let touchState=null;
  const touchPoint=t=>({x:t.clientX,y:t.clientY});
  const touchMid=(a,b)=>({x:(a.x+b.x)/2,y:(a.y+b.y)/2});
  const beginTouch=e=>{if(e.target.closest('button,a,.route-popup'))return;this.userInteracted=true;this.closePopup();const ts=[...e.touches];if(ts.length===1){const p=touchPoint(ts[0]);touchState={mode:'pan',last:p,moved:false};}else if(ts.length>=2){e.preventDefault();const a=touchPoint(ts[0]),b=touchPoint(ts[1]);touchState={mode:'pinch',lastMid:touchMid(a,b),lastDist:Math.max(1,dist(a,b)),moved:true};}};
  const moveTouch=e=>{if(!touchState)return;const ts=[...e.touches];if(ts.length===1&&touchState.mode==='pan'){e.preventDefault();const p=touchPoint(ts[0]),dx=p.x-touchState.last.x,dy=p.y-touchState.last.y;if(Math.abs(dx)+Math.abs(dy)>.5){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-dx,cp.y-dy,this.zoom);touchState.last=p;touchState.moved=true;this.requestRender();}return;}if(ts.length>=2){e.preventDefault();const a=touchPoint(ts[0]),b=touchPoint(ts[1]),mid=touchMid(a,b),d=Math.max(1,dist(a,b));if(touchState.mode!=='pinch'){touchState={mode:'pinch',lastMid:mid,lastDist:d,moved:true};return}const mdx=mid.x-touchState.lastMid.x,mdy=mid.y-touchState.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.2){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/touchState.lastDist;const max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;touchState.lastDist=d;this.clearTileNodes();}else if(ratio<.92&&this.zoom>5){this.zoom--;touchState.lastDist=d;this.clearTileNodes();}touchState.lastMid=mid;touchState.moved=true;this.requestRender();}};
  const endTouch=e=>{if(!touchState)return;const ts=[...e.touches];if(ts.length===0){touchState=null;this.render();return}if(ts.length===1){const p=touchPoint(ts[0]);touchState={mode:'pan',last:p,moved:true};return}const a=touchPoint(ts[0]),b=touchPoint(ts[1]);touchState={mode:'pinch',lastMid:touchMid(a,b),lastDist:Math.max(1,dist(a,b)),moved:true};};
  this.el.addEventListener('touchstart',beginTouch,{passive:false});this.el.addEventListener('touchmove',moveTouch,{passive:false});this.el.addEventListener('touchend',endTouch,{passive:false});this.el.addEventListener('touchcancel',()=>{touchState=null;this.render()},{passive:false});
  this.el.addEventListener('dblclick',e=>{e.preventDefault();this.setZoom(this.zoom+1)});window.addEventListener('resize',this._resizeHandler,{passive:true});}
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v94','1')}catch{}
