...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v97')}catch(_){ }
...
class BCMap{
 ...
 _bind(){this.el.querySelectorAll('[data-z]').forEach(b=>b.onclick=e=>{e.stopPropagation();this.userInteracted=true;this.setZoom(this.zoom+Number(b.dataset.z));});this.el.querySelector('.bc-layer').onclick=e=>{e.stopPropagation();this.userInteracted=true;this.toggleLayer();};this.el.querySelector('.bc-center-route').onclick=e=>{e.stopPropagation();if(!this.route?.length){toast('La ruta todavía se está cargando');return}this.userInteracted=false;this.fitRoute();toast('🎯 Ruta centrada');};const b3=this.el.querySelector('.bc-3d-mini');if(b3)b3.onclick=e=>{e.stopPropagation();open3DMap(this.stageNo||currentStageNo||1)};const street=this.el.querySelector('.bc-street-mini');if(street)street.onclick=e=>{e.stopPropagation();openStreetViewAt(this.center.lat,this.center.lon)};const symbols=this.el.querySelector('.bc-symbols-btn'),legend=this.el.querySelector('.bc-map-legend');if(symbols&&legend)symbols.onclick=e=>{e.stopPropagation();legend.hidden=!legend.hidden;symbols.classList.toggle('active',!legend.hidden)};this.el.querySelector('.bc-map-legend-close')?.addEventListener('click',e=>{e.stopPropagation();legend.hidden=true;symbols?.classList.remove('active')});
  // V9.7: gestos capturados por el propio contenedor del mapa.
  // La fase capture recibe el gesto aunque el dedo caiga sobre SVG, ruta, bolas o teselas.
  ...
  const pts=new Map();let gs=null;
  const pair=()=>[...pts.values()].slice(0,2);
  const startPan=p=>{gs={mode:'pan',start:{...p},cp:ll2world(this.center.lat,this.center.lon,this.zoom),dx:0,dy:0,moved:false,t:performance.now()};};
  const startPinch=()=>{const a=pair();if(a.length<2)return;gs={mode:'pinch',lastMid:mid(a[0],a[1]),lastDist:Math.max(1,dist(a[0],a[1])),moved:true,t:performance.now()};resetTransforms();};
  const onPointerDown=e=>{if(isControlTarget(e.target))return;if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();this.userInteracted=true;this.closePopup();try{this.el.setPointerCapture(e.pointerId)}catch{}pts.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pts.size===1)startPan({x:e.clientX,y:e.clientY});else if(pts.size===2)startPinch();};
  const onPointerMove=e=>{if(!pts.has(e.pointerId))return;e.preventDefault();pts.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pts.size===1&&gs?.mode==='pan'){const p=pts.values().next().value,dx=p.x-gs.start.x,dy=p.y-gs.start.y;gs.dx=dx;gs.dy=dy;if(Math.abs(dx)+Math.abs(dy)>3)gs.moved=true;translatePreview(dx,dy);return;}if(pts.size>=2){const a=pair(),m=mid(a[0],a[1]),d=Math.max(1,dist(a[0],a[1]));if(gs?.mode!=='pinch'){startPinch();return;}const mdx=m.x-gs.lastMid.x,mdy=m.y-gs.lastMid.y;if(Math.abs(mdx)+Math.abs(mdy)>.2){const cp=ll2world(this.center.lat,this.center.lon,this.zoom);this.center=world2ll(cp.x-mdx,cp.y-mdy,this.zoom);}const ratio=d/gs.lastDist,max=this.layer==='topo'?17:18;if(ratio>1.08&&this.zoom<max){this.zoom++;gs.lastDist=d;this.clearTileNodes();this.render();}else if(ratio<.92&&this.zoom>5){this.zoom--;gs.lastDist=d;this.clearTileNodes();this.render();}gs.lastMid=m;gs.moved=true;}};
  const finishPointer=(e,cancel=false)=>{if(!pts.has(e.pointerId))return;e.preventDefault();const last={x:e.clientX,y:e.clientY};pts.delete(e.pointerId);try{this.el.releasePointerCapture(e.pointerId)}catch{}if(pts.size>=2){startPinch();return;}if(pts.size===1){resetTransforms();startPan(pts.values().next().value);if(gs)gs.moved=true;return;}if(gs?.mode==='pan'){if(gs.moved&&!cancel)this.center=world2ll(gs.cp.x-gs.dx,gs.cp.y-gs.dy,this.zoom);else if(!cancel&&performance.now()-gs.t<650){const i=nearestYellow(last.x,last.y);if(i>=0){const q=this.pixel(this.route[i][0],this.route[i][1]);this.selectRoutePoint(i,q.x,q.y);}}}gs=null;resetTransforms();this.render();};
  this.el.addEventListener('pointerdown',onPointerDown,{passive:false,capture:true});
  this.el.addEventListener('pointermove',onPointerMove,{passive:false,capture:true});
  this.el.addEventListener('pointerup',e=>finishPointer(e,false),{passive:false,capture:true});
  this.el.addEventListener('pointercancel',e=>finishPointer(e,true),{passive:false,capture:true});
  this.el.addEventListener('dblclick',e=>{if(isControlTarget(e.target))return;e.preventDefault();this.setZoom(this.zoom+1)},{passive:false,capture:true});
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v97','1')}catch{}
