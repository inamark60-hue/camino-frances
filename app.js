...
let currentView='home', currentStageNo=null, mainMap=null, focusMap=null, terrain3DMap=null, terrain3DOverlay=null, terrain3DWatchId=null, routeVisible=localStorage.getItem('bc-route')!=='0';
...
const OFFICIAL_PUBLIC_HOSTELS='https://www.caminodesantiago.gal/documents/17639/1236272/Albergues.%20Hostels.pdf?version=1.5';
const OFFICIAL_CREDENTIAL='https://oficinadelperegrino.com/peregrinacion/la-credencial/';
const OFFICIAL_COMPOSTELA='https://oficinadelperegrino.com/peregrinacion/la-compostela/';
const OFFICIAL_PILGRIM_OFFICE='https://oficinadelperegrino.com/oficina-peregrino/';
...
function normalizeStageOsmElement(e,n,route){
 ...
 const details={
  ...
  pilgrims:t.pilgrims||'',
  network:t.network||'',
  credentialStamp:t.credential_stamp||t.stamp||''
 };
 return {
...
function officialAlbergueMeta(x){
 const d=x?.details||{},t=[x?.name,x?.address,d.operator,d.network,d.official].filter(Boolean).join(' ').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
 const xunta=/(xunta de galicia|xacobeo|turismo de galicia|rede publica)/.test(t);
 const explicit=String(d.official||'').toLowerCase()==='yes'||String(d.official||'').toLowerCase()==='true';
 return {official:xunta||explicit,publicNetwork:xunta,network:xunta?'Rede Pública de Albergues · Xunta de Galicia':(explicit?(d.network||d.operator||'Gestión oficial indicada'):'')};
}
function serviceLogoUrl(x){
 if(x?.logoUrl)return x.logoUrl;
 if(x?.type==='alojamiento'&&officialAlbergueMeta(x).publicNetwork)return 'https://www.google.com/s2/favicons?domain_url=https%3A%2F%2Fwww.caminodesantiago.gal&sz=64';
 if(!x?.website)return'';
 try{const u=new URL(x.website);return `https://www.google.com/s2/favicons?domain_url=${encodeURIComponent(u.origin)}&sz=64`}catch{return''}
}
function decoratePilgrimAlbergue(x){
 const meta=officialAlbergueMeta(x),d=x?.details||{},stamp=!!x.credentialStamp||['yes','true'].includes(String(d.credentialStamp||'').toLowerCase());
 return {...x,officialPilgrim:meta.official,officialNetwork:meta.network,credentialStamp:stamp,credentialRequired:meta.publicNetwork,logoUrl:serviceLogoUrl(x)};
}
function officialPilgrimDocsPanel(compact=false){return `<div class="official-pilgrim-info ${compact?'compact':''}"><div class="official-info-title"><span>🪪</span><div><b>Credencial · sellos · Compostela</b><small>Fuentes oficiales del Camino</small></div></div><p><b>Credencial:</b> identifica al peregrino y sirve para acreditar el paso. En los últimos 100 km a pie/caballo o 200 km en bicicleta, la Oficina del Peregrino indica al menos <b>2 sellos por día</b>.</p><p><b>Compostela:</b> se solicita presencialmente al finalizar en Santiago si se cumplen los requisitos vigentes. El sello final de la Catedral se pone en la Oficina de Acogida al Peregrino.</p><div class="official-info-actions"><a href="${OFFICIAL_CREDENTIAL}" target="_blank" rel="noopener">🪪 Credencial oficial</a><a href="${OFFICIAL_COMPOSTELA}" target="_blank" rel="noopener">📜 Compostela</a><a href="${OFFICIAL_PILGRIM_OFFICE}" target="_blank" rel="noopener">⛪ Oficina del Peregrino</a><a href="${OFFICIAL_PUBLIC_HOSTELS}" target="_blank" rel="noopener">🏛 Red pública Galicia</a></div><small class="official-caution">Los horarios, condiciones y requisitos pueden cambiar. Abre siempre la fuente oficial antes de desplazarte.</small></div>`}
...
function specialServiceItems(all,mode){
 if(mode==='albergues'){
  const exact=all.filter(isPilgrimAlbergue).sort((a,b)=>(a.km??9999)-(b.km??9999));
  const fallback=all.filter(x=>x.type==='alojamiento').sort((a,b)=>(a.km??9999)-(b.km??9999));
  return {items:(exact.length?exact:fallback).map(x=>{const y=decoratePilgrimAlbergue(x);return{...y,specialIcon:'🥾',specialBadge:y.officialPilgrim?'✓ Red pública / oficial indicada':(exact.length?'🥾 Albergue del Camino':'🛏 Alojamiento de la ruta')}}),verified:exact.length>0,exactCount:exact.length};
 }
 if(mode==='menu'){
...
async function showSpecialServiceList(n,mode){
 ...
 if(mode==='credential'){
  const stamps=all.filter(x=>x.credentialStamp).map(x=>({...x,specialIcon:'🔏',specialBadge:'Sello de credencial indicado'})).sort((a,b)=>(a.km??9999)-(b.km??9999));
  host.innerHTML=officialPilgrimDocsPanel()+`<div class="pilgrim-special-head credential"><div><b>🔏 PUNTOS DE SELLO EN LA ETAPA</b><span>${stamps.length?`${stamps.length} puntos marcados expresamente en los datos del Camino`:'No aparecen puntos de sello marcados expresamente en los datos abiertos de esta etapa'}</span></div></div>`+(stamps.length?stamps.slice(0,50).map(serviceCard).join(''):`<div class="empty compact-empty">Pregunta en albergues de peregrinos, iglesias, oficinas de turismo o establecimientos del Camino y confirma que pueden sellar la credencial.</div>`);return;
 }
 const pack=specialServiceItems(all,mode),items=pack.items;
 if(mode==='albergues'){
  if(!items.length){host.innerHTML=officialPilgrimDocsPanel(true)+'<div class="empty"><b>No aparecen alojamientos en los datos abiertos de esta etapa.</b></div>';return}
  const officialCount=items.filter(x=>x.officialPilgrim).length,stampCount=items.filter(x=>x.credentialStamp).length;
  host.innerHTML=`<div class="pilgrim-special-head albergue"><div><b>🥾 ALBERGUES DEL CAMINO</b><span>${pack.verified?`${pack.exactCount} albergues/refugios identificados`:'No se distinguen albergues explícitos: se muestran alojamientos de la ruta para confirmar'} · ${officialCount} con gestión oficial/red pública identificada${stampCount?` · ${stampCount} con sello indicado`:''}</span></div><button id="specialMapBtn">📍 VER EN MAPA</button></div>`
   +`<div class="official-hostel-source"><div><b>🏛 Albergues oficiales / públicos</b><span>Solo se marca como oficial cuando la fuente identifica la red o gestión. Para Galicia puedes consultar directamente la Red Pública de Albergues de la Xunta.</span></div><a href="${OFFICIAL_PUBLIC_HOSTELS}" target="_blank" rel="noopener">VER LISTADO OFICIAL ↗</a></div>`
   +items.slice(0,70).map(serviceCard).join('')
   +officialPilgrimDocsPanel(true)
   +`<p class="data-source-note">Ordenados por km de marcha. Los datos proceden de puntos abiertos del Camino y OpenStreetMap; confirma plazas, horarios, sello y condiciones antes de llegar.</p>`;
 }else{
  ...
  host.innerHTML=`<div class="pilgrim-special-head menu"><div><b>🍲 MENÚ PEREGRINO</b><span>${exact.length?`${exact.length} lugares indican menú de peregrino`:'No consta un menú de peregrino verificado en los datos abiertos de esta etapa'}</span></div><button id="specialMapBtn">📍 VER EN MAPA</button></div>`
   ...
   +`<a class="stage-menu-search" href="https://www.google.com/search?q=${encodeURIComponent(`menú peregrino ${st.from||''} ${st.to||''}`)}" target="_blank" rel="noopener">🔎 Buscar “menú peregrino” en esta etapa</a>`
...
function setActive(k){
 ...
 const earth=document.getElementById('homeEarthBtn');if(earth){earth.hidden=k!=='home';if(k==='home')earth.onclick=()=>window.open(wholeCaminoEarthUrl(),'_blank','noopener')}
}
...
class BCMap{
 constructor(el,opts={}){this.el=typeof el==='string'?document.getElementById(el):el;this.zoom=opts.zoom||11;this.center={lat:opts.lat||43.08,lon:opts.lon||-1.27};this.layer=opts.layer||'topo';this.route=[];this.routeOn=routeVisible;this.services=[];this.serviceFilter='all';this.extraLines=[];this.selectedRouteIndex=-1;this.onRoutePoint=null;this._raf=0;this.tileNodes=new Map();this.activePointers=new Map();this.panStart=null;this.pinch=null;this.userInteracted=false;this.userPosition=null;this.gpsWatchId=null;this.gpsCenteredOnce=false;this._resizeHandler=()=>this.requestRender();this._build();this._bind();this.render();}
 _build(){this.el.innerHTML='<div class="bc-tiles"></div><svg class="bc-overlay"></svg><div class="bc-markers"></div><div class="bc-service-filters" hidden></div><div class="bc-map-controls"><button data-z="1" aria-label="Acercar">+</button><button data-z="-1" aria-label="Alejar">−</button></div><div class="bc-center-controls"><button data-map-action="fit-route" title="Centrar toda la ruta" aria-label="Centrar toda la ruta">◎</button><button data-map-action="gps" title="Mi posición GPS" aria-label="Centrar en mi posición GPS">📍</button></div><button class="bc-layer" title="Cambiar mapa" aria-label="Cambiar tipo de mapa"></button><div class="bc-scale"></div><div class="bc-map-msg">Cargando mapa…</div>';this.tiles=this.el.querySelector('.bc-tiles');this.svg=this.el.querySelector('.bc-overlay');this.markers=this.el.querySelector('.bc-markers');this.scale=this.el.querySelector('.bc-scale');this.msg=this.el.querySelector('.bc-map-msg');this.popup=null;this.serviceFilters=this.el.querySelector('.bc-service-filters');this.updateLayerButton();}
 ...
 updateUserPosition(pos,center=false){const {latitude,longitude,accuracy}=pos.coords||{};if(!Number.isFinite(latitude)||!Number.isFinite(longitude))return;this.userPosition={lat:latitude,lon:longitude,accuracy:Number.isFinite(accuracy)?accuracy:null};if(center){this.center={lat:latitude,lon:longitude};this.zoom=Math.max(this.zoom,15);this.gpsCenteredOnce=true}this.render();}
 locateUser(){if(!navigator.geolocation){toast('GPS no disponible en este dispositivo');return}const btn=this.el.querySelector('[data-map-action="gps"]');if(this.userPosition){this.center={lat:this.userPosition.lat,lon:this.userPosition.lon};this.zoom=Math.max(this.zoom,15);this.render();toast(`Mi posición${this.userPosition.accuracy?` · ±${Math.round(this.userPosition.accuracy)} m`:''}`);return}if(btn)btn.classList.add('searching');toast('Buscando tu posición GPS…');const ok=p=>{this.updateUserPosition(p,!this.gpsCenteredOnce);if(btn)btn.classList.remove('searching');toast(`GPS activo${Number.isFinite(p.coords?.accuracy)?` · ±${Math.round(p.coords.accuracy)} m`:''}`)},fail=()=>{if(btn)btn.classList.remove('searching');toast('No se pudo obtener tu ubicación. Revisa el permiso GPS.')};navigator.geolocation.getCurrentPosition(ok,fail,{enableHighAccuracy:true,timeout:12000,maximumAge:5000});if(this.gpsWatchId==null){try{this.gpsWatchId=navigator.geolocation.watchPosition(p=>this.updateUserPosition(p,false),()=>{}, {enableHighAccuracy:true,maximumAge:5000,timeout:20000})}catch{}}}
 ...
 destroy(){window.removeEventListener('resize',this._resizeHandler);if(this.gpsWatchId!=null&&navigator.geolocation){try{navigator.geolocation.clearWatch(this.gpsWatchId)}catch{}}this.gpsWatchId=null;this.activePointers.clear();this.clearTileNodes();this.el.innerHTML='';}
}
...
function cleanupMaps(){mainMap?.destroy?.();mainMap=null;focusMap?.destroy?.();focusMap=null;if(!focusOverlay.hidden)closeFocusMap();close3DMap();}
...
function open3DRoutePointPopup(map,ml,route,idx,stageNo){
 ...
 node.innerHTML=`<b>🟨 Punto de ruta 3D</b><small>${p[0].toFixed(6)}, ${p[1].toFixed(6)}</small><div class="terrain-point-actions"><a href="${mapsUrl(p[0],p[1])}" target="_blank" rel="noopener">📍 Maps</a><a href="${earthUrl(p[0],p[1])}" target="_blank" rel="noopener">🌍 Earth</a><a href="${osmUrl(p[0],p[1])}" target="_blank" rel="noopener">🗺 OSM</a><a href="${commonsNearbyUrl(p[0],p[1])}" target="_blank" rel="noopener">📷 Fotos</a><button type="button" class="terrain-nearby">📌 SERVICIOS CERCA</button></div>`;
 ...
 const btn=node.querySelector('.terrain-nearby');
...
function close3DMap(){if(terrain3DWatchId!=null&&navigator.geolocation){try{navigator.geolocation.clearWatch(terrain3DWatchId)}catch{}}terrain3DWatchId=null;try{terrain3DMap?.remove?.()}catch{}terrain3DMap=null;try{screen.orientation?.unlock?.()}catch{}if(terrain3DRequestedFullscreen&&document.fullscreenElement){document.exitFullscreen?.().catch?.(()=>{})}terrain3DRequestedFullscreen=false;if(terrain3DOverlay){terrain3DOverlay.remove();terrain3DOverlay=null}document.body.classList.remove('terrain3d-open')}
...
async function open3DMap(n){
 ...
 const style={version:8,sources:{topo:{type:'raster',tiles:['https://a.tile.opentopomap.org/{z}/{x}/{y}.png','https://b.tile.opentopomap.org/{z}/{x}/{y}.png','https://c.tile.opentopomap.org/{z}/{x}/{y}.png'],tileSize:256,maxzoom:17,attribution:'© OpenStreetMap · SRTM · OpenTopoMap'},osm:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,maxzoom:19,attribution:'© OpenStreetMap contributors'},terrainSource:{type:'raster-dem',url:'https://tiles.mapterhorn.com/tilejson.json'},hillshadeSource:{type:'raster-dem',url:'https://tiles.mapterhorn.com/tilejson.json'}},layers:[{id:'osm-base',type:'raster',source:'osm',layout:{visibility:'none'}},{id:'topo-base',type:'raster',source:'topo'},{id:'terrain-hillshade',type:'hillshade',source:'hillshadeSource',paint:{'hillshade-exaggeration':0.28,'hillshade-shadow-color':'#3d3328','hillshade-highlight-color':'#f6f0dc','hillshade-accent-color':'#5f5646'}}]};
 try{
  ...
  terrain3DMap.on('load',()=>{
   ...
   try{terrain3DMap.setTerrain({source:'terrainSource',exaggeration:1.2})}catch(e){toast('Relieve 3D limitado en este dispositivo')}
 ...
 document.getElementById('terrainBase').onclick=()=>{if(!terrain3DMap?.getLayer('topo-base'))return;topo=!topo;terrain3DMap.setLayoutProperty('topo-base','visibility',topo?'visible':'none');terrain3DMap.setLayoutProperty('osm-base','visibility',topo?'none':'visible');document.getElementById('terrainBase').textContent=topo?'⛰ TOPO':'🛣 CALLES'};
 ...
 document.getElementById('terrainCenterRoute').onclick=()=>{if(!terrain3DMap||!route?.length)return;const bounds=new ml.LngLatBounds();route.forEach(p=>bounds.extend([p[1],p[0]]));terrain3DMap.fitBounds(bounds,{padding:{top:75,bottom:65,left:50,right:50},duration:650,maxZoom:14});toast('Ruta centrada en 3D')};
 ...
 document.getElementById('terrainLocate').onclick=()=>{
  ...
  const btn=document.getElementById('terrainLocate');btn.textContent='📍 BUSCANDO…';let first=true;
  const update=pos=>{if(!terrain3DMap)return;const {latitude,longitude,accuracy}=pos.coords,data={type:'Feature',properties:{accuracy:Math.round(accuracy||0)},geometry:{type:'Point',coordinates:[longitude,latitude]}};if(terrain3DMap.getSource('bc-user-position'))terrain3DMap.getSource('bc-user-position').setData(data);else{terrain3DMap.addSource('bc-user-position',{type:'geojson',data});terrain3DMap.addLayer({id:'bc-user-position-halo',type:'circle',source:'bc-user-position',paint:{'circle-radius':12,'circle-color':'#ffffff','circle-opacity':0.88,'circle-stroke-color':'#0b63b6','circle-stroke-width':2}});terrain3DMap.addLayer({id:'bc-user-position',type:'circle',source:'bc-user-position',paint:{'circle-radius':6,'circle-color':'#0b63b6','circle-stroke-color':'#ffffff','circle-stroke-width':2}})}if(first){terrain3DMap.easeTo({center:[longitude,latitude],zoom:15.5,pitch:70,duration:900});first=false}btn.textContent='📍 MI POSICIÓN';toast(`GPS activo en 3D · ±${Math.round(accuracy||0)} m`)};
  navigator.geolocation.getCurrentPosition(update,()=>{btn.textContent='📍 MI POSICIÓN';toast('No se pudo obtener tu ubicación. Revisa el permiso GPS.')},{enableHighAccuracy:true,timeout:12000,maximumAge:5000});
  if(terrain3DWatchId==null){try{terrain3DWatchId=navigator.geolocation.watchPosition(update,()=>{}, {enableHighAccuracy:true,maximumAge:5000,timeout:20000})}catch{}}
 };
...
function mapView(){cleanupMaps();currentView='map';currentStageNo=1;setActive('map');app.innerHTML=`<section class="section"><h2>Mapa por etapa</h2><p class="map-note">Las 33 etapas cargan su tramo concreto del Camino Francés. Selecciona una etapa: el mapa se ajustará automáticamente a su recorrido.</p><select id="globalStageSelect" class="stage-select">${STAGES.map(s=>`<option value="${s.n}">${s.n}. ${esc(s.from)} → ${esc(s.to)}</option>`).join('')}</select><div class="map-toolbar"><button id="global3D" class="map-chip terrain-chip">🏔 MAPA 3D</button><button id="globalBig" class="map-chip primary">⛶ MAPA GRANDE / HORIZONTAL</button></div><div id="globalMap" class="bc-map tall" style="height:60dvh;min-height:430px"></div></section>`;const load=n=>{mainMap?.destroy?.();currentStageNo=n;mainMap=buildStageMap('globalMap',n,false);document.getElementById('globalBig').onclick=()=>openFocusMap(n);document.getElementById('global3D').onclick=()=>open3DMap(n)};setTimeout(()=>load(1),0);document.getElementById('globalStageSelect').onchange=e=>load(+e.target.value);}
...
function openDoubts(){document.getElementById('doubtsTitle').textContent='DUDAS PEREGRINO · Consejos prácticos';document.getElementById('doubtsBody').innerHTML=`
...
function bindStageCards(){
 ...
 document.querySelectorAll('[data-share-stage]').forEach(b=>b.onclick=e=>{e.stopPropagation();shareStage(+b.dataset.shareStage)});
}
