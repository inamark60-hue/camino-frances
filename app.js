...
async function skyFetchDataFile(file){
 let cache=null;try{if('caches'in window)cache=await caches.open('buen-camino-sky-v90')}catch(_){ }
...
async function repairBuenCaminoBoot(force=false){
 ...
 try{sessionStorage.setItem('bc-boot-repaired-v90','1')}catch{}
 const u=new URL('./',location.href);u.searchParams.set('bcv','87');u.searchParams.set('repair',Date.now().toString());location.replace(u.href);
}
